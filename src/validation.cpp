...
static void LimitMempoolSize(CTxMemPool& pool, size_t limit, unsigned long age) {
    ...
    if (expired != 0) {
        LogPrintG(BCLogLevel::DEBUG, BCLog::MEMPOOL, "[MemPool] Expired %i transactions from the memory pool\n", expired);
    }
                              ...
                              bool bypass_limits, const CAmount& nAbsurdFee, std::vector<COutPoint>& coins_to_uncache)
{
...

    {
        ...
        if (!CheckInputsFromMempoolAndCache(tx, state, view, pool, currentBlockScriptVerifyFlags, true, txdata))
        {
            ...
            if (!(~scriptVerifyFlags & currentBlockScriptVerifyFlags)) {
                ...
                if (!CheckInputs(tx, state, view, true, MANDATORY_SCRIPT_VERIFY_FLAGS, true, false, txdata)) {
                    ...
                    LogPrintG(BCLogLevel::DEBUG, BCLog::BLOCKVALID, "[BlockValidation] Warning: -promiscuousmempool flags set to not include currently enforced soft forks, this may break mining or otherwise cause instability!\n");
                }
            }
        }
        ...
        {
            LogPrintG(BCLogLevel::DEBUG, BCLog::MEMPOOL, "[MemPool] Replacing tx %s with %s for %s GENX additional fees, %d delta bytes\n",
...
CAmount GetBlockSubsidy(int nHeight, const Consensus::Params& consensusParams, bool fGovernanceBlockPartOnly)
{
    ...
    //LogPrintG(BCLogLevel::DEBUG, BCLog::BLOCKVALID, "[BlockValidation] Subsidy %i acceptable for block %i \n", subsidy, nHeight);
...
bool IsInitialBlockDownload()
{
    ...
    if (chainActive.Tip()->nHeight > 0  && chainActive.Tip()->GetBlockTime() < (GetTime() - nMaxTipAge))
        return true;
    LogPrintG(BCLogLevel::DEBUG, BCLog::BLOCKVALID, "[BlockValidation] Leaving InitialBlockDownload (latching to false)\n");
...
static void CheckForkWarningConditions()
{
    ...
    if (pindexBestForkTip || (pindexBestInvalid && pindexBestInvalid->nChainWork > chainActive.Tip()->nChainWork + (GetBlockProof(*chainActive.Tip()) * 6)))
    {
        ...
        {
            LogPrintG(BCLogLevel::DEBUG, BCLog::BLOCKVALID, "[BlockValidation] %s: Warning: Large valid fork found\n  forking the chain at height %d (%s)\n  lasting to height %d (%s).\nChain state database corruption likely.\n", __func__,
        ...
        {
            LogPrintG(BCLogLevel::DEBUG, BCLog::BLOCKVALID, "[BlockValidation] %s: Warning: Found invalid chain at least ~6 blocks longer than our best chain.\nChain state database corruption likely.\n", __func__);
...
void static InvalidChainFound(CBlockIndex* pindexNew)
{
    ...
    LogPrintG(BCLogLevel::DEBUG, BCLog::BLOCKVALID, "[BlockValidation] %s: invalid block=%s  height=%d  log2_work=%.8g  date=%s\n", __func__,
      pindexNew->GetBlockHash().ToString(), pindexNew->nHeight,
      log(pindexNew->nChainWork.getdouble())/log(2.0), DateTimeStrFormat("%Y-%m-%d %H:%M:%S",
      pindexNew->GetBlockTime()));
    CBlockIndex *tip = chainActive.Tip();
    assert (tip);
    LogPrintG(BCLogLevel::DEBUG, BCLog::BLOCKVALID, "[BlockValidation] %s:  current best=%s  height=%d  log2_work=%.8g  date=%s\n", __func__,
...
void InitScriptExecutionCache() {
    ...
    LogPrintG(BCLogLevel::DEBUG, BCLog::BLOCKVALID, "[BlockValidation] Using %zu MiB out of %zu/2 requested for script execution cache, able to store %zu elements\n",
...
bool AbortNode(const std::string& strMessage, const std::string& userMessage="")
{
    ...
    LogPrintG(BCLogLevel::DEBUG, BCLog::BLOCKVALID, "[BlockValidation] *** %s\n", strMessage);
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck)
{
    ...
    LogPrintG(BCLogLevel::DEBUG, BCLog::BENCH, "[Benchmarking]    - Sanity checks: %.2fms [%.2fs (%.2fms/blk)]\n", MILLI * (nTime1 - nTimeStart), nTimeCheck * MICRO, nTimeCheck * MILLI / nBlocksTotal);
    ...
    LogPrintG(BCLogLevel::DEBUG, BCLog::BENCH, "[Benchmarking]    - Fork checks: %.2fms [%.2fs (%.2fms/blk)]\n", MILLI * (nTime2 - nTime1), nTimeForks * MICRO, nTimeForks * MILLI / nBlocksTotal);
    ...
    LogPrintG(BCLogLevel::DEBUG, BCLog::BENCH, "[Benchmarking]      - Connect %u transactions: %.2fms (%.3fms/tx, %.3fms/txin) [%.2fs (%.2fms/blk)]\n", (unsigned)block.vtx.size(), MILLI * (nTime3 - nTime2), MILLI * (nTime3 - nTime2) / block.vtx.size(), nInputs <= 1 ? 0 : MILLI * (nTime3 - nTime2) / (nInputs-1), nTimeConnect * MICRO, nTimeConnect * MILLI / nBlocksTotal);
    ...
    LogPrintG(BCLogLevel::DEBUG, BCLog::BENCH, "[Benchmarking]    - Verify %u txins: %.2fms (%.3fms/txin) [%.2fs (%.2fms/blk)]\n", nInputs - 1, MILLI * (nTime4 - nTime2), nInputs <= 1 ? 0 : MILLI * (nTime4 - nTime2) / (nInputs-1), nTimeVerify * MICRO, nTimeVerify * MILLI / nBlocksTotal);
    ...
    LogPrintG(BCLogLevel::DEBUG, BCLog::BENCH, "[Benchmarking]    - Index writing: %.2fms [%.2fs (%.2fms/blk)]\n", MILLI * (nTime5 - nTime4), nTimeIndex * MICRO, nTimeIndex * MILLI / nBlocksTotal);

    int64_t nTime6 = GetTimeMicros(); nTimeCallbacks += nTime6 - nTime5;
    LogPrintG(BCLogLevel::DEBUG, BCLog::BENCH, "[Benchmarking]    - Callbacks: %.2fms [%.2fs (%.2fms/blk)]\n", MILLI * (nTime6 - nTime5), nTimeCallbacks * MICRO, nTimeCallbacks * MILLI / nBlocksTotal);
...
void static UpdateTip(const CBlockIndex *pindexNew, const CChainParams& chainParams) {
    ...
    LogPrintG(BCLogLevel::DEBUG, BCLog::BLOCKVALID, "[BlockValidation] %s: new best=%s height=%d version=0x%08x log2_work=%.8g tx=%lu date='%s' progress=%f cache=%.1fMiB(%utxo)", __func__,
      pindexNew->GetBlockHash().ToString(), pindexNew->nHeight, pindexNew->nVersion,
      log(pindexNew->nChainWork.getdouble())/log(2.0), (unsigned long)pindexNew->nChainTx,
      DateTimeStrFormat("%Y-%m-%d %H:%M:%S", pindexNew->GetBlockTime()),
      GuessVerificationProgress(chainParams.TxData(), pindexNew), pcoinsTip->DynamicMemoryUsage() * (1.0 / (1<<20)), pcoinsTip->GetCacheSize());
    if (!warningMessages.empty())
        LogPrintG(BCLogLevel::DEBUG, BCLog::BLOCKVALID, "[BlockValidation]  warning='%s'", boost::algorithm::join(warningMessages, ", "));
    LogPrintG(BCLogLevel::DEBUG, BCLog::BLOCKVALID, "[BlockValidation] \n");
...
bool CChainState::DisconnectTip(CValidationState& state, const CChainParams& chainparams, DisconnectedBlockTransactions *disconnectpool)
{
    ...
    LogPrintG(BCLogLevel::DEBUG, BCLog::BENCH, "[Benchmarking] - Disconnect block: %.2fms\n", (GetTimeMicros() - nStart) * MILLI);
...
bool CChainState::ConnectTip(CValidationState& state, const CChainParams& chainparams, CBlockIndex* pindexNew, const std::shared_ptr<const CBlock>& pblock, ConnectTrace& connectTrace, DisconnectedBlockTransactions &disconnectpool)
{
    ...
    int64_t nTime3;
    LogPrintG(BCLogLevel::DEBUG, BCLog::BENCH, "[Benchmarking] - Load block from disk: %.2fms [%.2fs]\n", (nTime2 - nTime1) * MILLI, nTimeReadFromDisk * MICRO);
    {
        CCoinsViewCache view(pcoinsTip.get());
        ...
        LogPrintG(BCLogLevel::DEBUG, BCLog::BENCH, "[Benchmarking] - Connect total: %.2fms [%.2fs (%.2fms/blk)]\n", (nTime3 - nTime2) * MILLI, nTimeConnectTotal * MICRO, nTimeConnectTotal * MILLI / nBlocksTotal);
        bool flushed = view.Flush();
        assert(flushed);
    }
    int64_t nTime4 = GetTimeMicros(); nTimeFlush += nTime4 - nTime3;
    LogPrintG(BCLogLevel::DEBUG, BCLog::BENCH, "[Benchmarking] - Flush: %.2fms [%.2fs (%.2fms/blk)]\n", (nTime4 - nTime3) * MILLI, nTimeFlush * MICRO, nTimeFlush * MILLI / nBlocksTotal);
    // Write the chain state to disk, if necessary.
    if (!FlushStateToDisk(chainparams, state, FLUSH_STATE_IF_NEEDED))
        return false;
    int64_t nTime5 = GetTimeMicros(); nTimeChainState += nTime5 - nTime4;
    LogPrintG(BCLogLevel::DEBUG, BCLog::BENCH, "[Benchmarking] - Writing chainstate: %.2fms [%.2fs (%.2fms/blk)]\n", (nTime5 - nTime4) * MILLI, nTimeChainState * MICRO, nTimeChainState * MILLI / nBlocksTotal);
    // Remove conflicting transactions from the mempool.;
    ...
    LogPrintG(BCLogLevel::DEBUG, BCLog::BENCH, "[Benchmarking] - Connect postprocess: %.2fms [%.2fs (%.2fms/blk)]\n", (nTime6 - nTime5) * MILLI, nTimePostConnect * MICRO, nTimePostConnect * MILLI / nBlocksTotal);
    LogPrintG(BCLogLevel::DEBUG, BCLog::BENCH, "[Benchmarking] - Connect block: %.2fms [%.2fs (%.2fms/blk)]\n", (nTime6 - nTime1) * MILLI, nTimeTotal * MICRO, nTimeTotal * MILLI / nBlocksTotal);
...
static bool CheckBlockHeader(const CBlockHeader& block, CValidationState& state, const Consensus::Params& consensusParams, bool fCheckPOW = true)
{
    ...
    if (fCheckPOW)
    {
        ...
        {
            // LogPrintG(BCLogLevel::DEBUG, BCLog::BLOCKVALID, "[BlockValidation] CheckBlockHeader(): Found solution using GENX_PoW at height: %d\n", block.nHeight);
        }
        ...
        {
            // LogPrintG(BCLogLevel::DEBUG, BCLog::BLOCKVALID, "[BlockValidation] CheckBlockHeader(): Found solution using SafeCash at height: %d\n", block.nHeight);
        }
        else if (block.GetHash() == Params().GetConsensus().hashGenesisBlock)
        {
            // LogPrintG(BCLogLevel::DEBUG, BCLog::BLOCKVALID, "[BlockValidation] Skipping genesis block\n");
        }
        else
        {
            // Nothing worked... bugger.
            LogPrintG(BCLogLevel::DEBUG, BCLog::BLOCKVALID, "[BlockValidation] CheckBlockHeader(): Equihash solution invalid at height %d\n", block.nHeight);
...
static bool ContextualCheckBlock(const CBlock& block, CValidationState& state, const Consensus::Params& consensusParams, const CBlockIndex* pindexPrev)
{
    ...
    if (nHeight >= consensusParams.BIP34Height)
    {
        ...
        if (block.vtx[0]->vin[0].scriptSig.size() < expect.size() ||
            ...
            LogPrintG(BCLogLevel::DEBUG, BCLog::BLOCKVALID, "[BlockValidation] BIP34 Height Validation failed at: %d\n", nHeight);
...
void UnlinkPrunedFiles(const std::set<int>& setFilesToPrune)
{
    for (std::set<int>::iterator it = setFilesToPrune.begin(); it != setFilesToPrune.end(); ++it) {
        ...
        LogPrintG(BCLogLevel::DEBUG, BCLog::PRUNE, "[Prune] %s deleted blk/rev (%05u)\n", __func__, *it);
    }
}
...
static void FindFilesToPruneManual(std::set<int>& setFilesToPrune, int nManualPruneHeight)
{
    ...
    LogPrintG(BCLogLevel::DEBUG, BCLog::PRUNE, "[Prune] (Manual) prune_height=%d removed %d blk/rev pairs\n", nLastBlockWeCanPrune, count);
}
...
static void FindFilesToPrune(std::set<int>& setFilesToPrune, uint64_t nPruneAfterHeight)
{
    ...
    LogPrintG(BCLogLevel::DEBUG, BCLog::PRUNE, "[Prune] target=%dMiB actual=%dMiB diff=%dMiB max_prune_height=%d removed %d blk/rev pairs\n",
...
bool CVerifyDB::VerifyDB(const CChainParams& chainparams, CCoinsView *coinsview, int nCheckLevel, int nCheckDepth)
{
    ...
    LogPrintG(BCLogLevel::DEBUG, BCLog::BLOCKVALID, "[BlockValidation] Verifying last %i blocks at level %i\n", nCheckDepth, nCheckLevel);
    ...
    CValidationState state;
    int reportDone = 0;
    LogPrintG(BCLogLevel::DEBUG, BCLog::BLOCKVALID, "[BlockValidation] [0%%]...");
    for (CBlockIndex* pindex = chainActive.Tip(); pindex && pindex->pprev; pindex = pindex->pprev)
    {
        ...
        if (reportDone < percentageDone/10) {
            ...
            LogPrintG(BCLogLevel::DEBUG, BCLog::BLOCKVALID, "[BlockValidation] [%d%%]...", percentageDone);
        ...
        if (fPruneMode && !(pindex->nStatus & BLOCK_HAVE_DATA)) {
            ...
            LogPrintG(BCLogLevel::DEBUG, BCLog::BLOCKVALID, "[BlockValidation] VerifyDB(): block verification stopping at height %d (pruning, no data)\n", pindex->nHeight);
            break;
        }
        CBlock block;
    ...
    LogPrintG(BCLogLevel::DEBUG, BCLog::BLOCKVALID, "[BlockValidation] [DONE].\n");
    LogPrintG(BCLogLevel::DEBUG, BCLog::BLOCKVALID, "[BlockValidation] No coin database inconsistencies in last %i blocks (%i transactions)\n", chainActive.Height() - pindexState->nHeight, nGoodTransactions);
...
bool CChainState::ReplayBlocks(const CChainParams& params, CCoinsView* view)
{
    ...
    LogPrintG(BCLogLevel::DEBUG, BCLog::REINDEX, "[REINDEX] Replaying blocks\n");
    ...
    while (pindexOld != pindexFork) {
        if (pindexOld->nHeight > 0) { // Never disconnect the genesis block.
            ...
            LogPrintG(BCLogLevel::DEBUG, BCLog::REINDEX, "[REINDEX] Rolling back %s (%i)\n", pindexOld->GetBlockHash().ToString(), pindexOld->nHeight);
...
bool LoadBlockIndex(const CChainParams& chainparams)
{
    ...
    if (needs_init) {
        ...
        LogPrintG(BCLogLevel::DEBUG, BCLog::DB, "[WalletDatabase] Initializing databases...\n");
...
bool LoadExternalBlockFile(const CChainParams& chainparams, FILE* fileIn, CDiskBlockPos *dbp)
{
        ...
        while (!blkdat.eof()) {
            ...
            try {
                ...
                if (hash != chainparams.GetConsensus().hashGenesisBlock && mapBlockIndex.find(block.hashPrevBlock) == mapBlockIndex.end()) {
                    LogPrintG(BCLogLevel::DEBUG, BCLog::REINDEX, "[REINDEX] %s: Out of order block %s, parent %s not known\n", __func__, hash.ToString(),
                ...
                if (mapBlockIndex.count(hash) == 0 || (mapBlockIndex[hash]->nStatus & BLOCK_HAVE_DATA) == 0) {
                    ...
                    LogPrintG(BCLogLevel::DEBUG, BCLog::REINDEX, "[REINDEX] Block Import: already had block %s at height %d\n", hash.ToString(), mapBlockIndex[hash]->nHeight);
                }
                ...
                while (!queue.empty()) {
                    ...
                    while (range.first != range.second) {
                        ...
                        {
                            LogPrintG(BCLogLevel::DEBUG, BCLog::REINDEX, "[REINDEX] %s: Processing out of order child %s of %s\n", __func__, pblockrecursive->GetHash().ToString(),
                                    head.ToString());
                            LOCK(cs_main);
                ...
                LogPrintG(BCLogLevel::DEBUG, BCLog::REINDEX, "[REINDEX] %s: Deserialize or I/O error - %s\n", __func__, e.what());
            }
        }
    } catch (const std::runtime_error& e) {
    ...
    if (nLoaded > 0)
        LogPrintG(BCLogLevel::DEBUG, BCLog::REINDEX, "[REINDEX] Loaded %i blocks from external file in %dms\n", nLoaded, GetTimeMillis() - nStart);
    return nLoaded > 0;
}
...
bool LoadMempool(void)
{
    ...
    if (file.IsNull()) {
        LogPrintG(BCLogLevel::DEBUG, BCLog::MEMPOOL, "[MemPool] Failed to open mempool file from disk. Continuing anyway.\n");
        return false;
    }
    ...
    try {
        ...
        LogPrintG(BCLogLevel::DEBUG, BCLog::MEMPOOL, "[MemPool] Failed to deserialize mempool data on disk: %s. Continuing anyway.\n", e.what());
        return false;
    }

    LogPrintG(BCLogLevel::DEBUG, BCLog::MEMPOOL, "[MemPool] Imported mempool transactions from disk: %i succeeded, %i failed, %i expired, %i already there\n", count, failed, expired, already_there);
    return true;
}
...
bool DumpMempool(void)
{
    ...
    try {
        ...
        LogPrintG(BCLogLevel::DEBUG, BCLog::MEMPOOL, "[MemPool] Dumped mempool: %gs to copy, %gs to dump\n", (mid-start)*MICRO, (last-mid)*MICRO);
    } catch (const std::exception& e) {
        LogPrintG(BCLogLevel::DEBUG, BCLog::MEMPOOL, "[MemPool] Failed to dump mempool: %s. Continuing anyway.\n", e.what());
        return false;
    }
    return true;
}
