        ...
        if (!CheckInputsFromMempoolAndCache(tx, state, view, pool, currentBlockScriptVerifyFlags, true, txdata))
        {
            ...
            if (!(~scriptVerifyFlags & currentBlockScriptVerifyFlags)) {
                ...
                if (!CheckInputs(tx, state, view, true, MANDATORY_SCRIPT_VERIFY_FLAGS, true, false, txdata)) {
                    ...
                    LogPrintG(BCLogLevel::LOG_WARNING, BCLog::BLOCKVALID, "[BlockValidation] Warning: -promiscuousmempool flags set to not include currently enforced soft forks, this may break mining or otherwise cause instability!\n");
                }
            }
        }
...
static void CheckForkWarningConditions()
{
    ...
    if (pindexBestForkTip || (pindexBestInvalid && pindexBestInvalid->nChainWork > chainActive.Tip()->nChainWork + (GetBlockProof(*chainActive.Tip()) * 6)))
    {
        ...
        {
            LogPrintG(BCLogLevel::LOG_WARNING, BCLog::BLOCKVALID, "[BlockValidation] %s: Warning: Large valid fork found\n  forking the chain at height %d (%s)\n  lasting to height %d (%s).\nChain state database corruption likely.\n", __func__,
        ...
        {
            LogPrintG(BCLogLevel::LOG_WARNING, BCLog::BLOCKVALID, "[BlockValidation] %s: Warning: Found invalid chain at least ~6 blocks longer than our best chain.\nChain state database corruption likely.\n", __func__);
...
static bool CheckBlockHeader(const CBlockHeader& block, CValidationState& state, const Consensus::Params& consensusParams, bool fCheckPOW = true)
{
    ...
    if (fCheckPOW)
    {
        ...
        {
            // LogPrintG(BCLogLevel::LOG_INFO, BCLog::BLOCKVALID, "[BlockValidation] CheckBlockHeader(): Found solution using SafeCash at height: %d\n", block.nHeight);
        }
        ...
        else
        {
            ...
            LogPrintG(BCLogLevel::LOG_ERROR, BCLog::BLOCKVALID, "[BlockValidation] CheckBlockHeader(): Equihash solution invalid at height %d\n", block.nHeight);
...
static bool ContextualCheckBlock(const CBlock& block, CValidationState& state, const Consensus::Params& consensusParams, const CBlockIndex* pindexPrev)
{
    ...
    if (nHeight >= consensusParams.BIP34Height)
    {
        ...
        if (block.vtx[0]->vin[0].scriptSig.size() < expect.size() ||
            ...
            LogPrintG(BCLogLevel::LOG_ERROR, BCLog::BLOCKVALID, "[BlockValidation] BIP34 Height Validation failed at: %d\n", nHeight);
...
void UnlinkPrunedFiles(const std::set<int>& setFilesToPrune)
{
    for (std::set<int>::iterator it = setFilesToPrune.begin(); it != setFilesToPrune.end(); ++it) {
        ...
        LogPrintG(BCLogLevel::LOG_INFO, BCLog::PRUNE, "[Prune] %s deleted blk/rev (%05u)\n", __func__, *it);
    }
}
...
static void FindFilesToPruneManual(std::set<int>& setFilesToPrune, int nManualPruneHeight)
{
    ...
    LogPrintG(BCLogLevel::LOG_INFO, BCLog::PRUNE, "[Prune] (Manual) prune_height=%d removed %d blk/rev pairs\n", nLastBlockWeCanPrune, count);
}
...
static void FindFilesToPrune(std::set<int>& setFilesToPrune, uint64_t nPruneAfterHeight)
{
    ...
    LogPrintG(BCLogLevel::LOG_INFO, BCLog::PRUNE, "[Prune] target=%dMiB actual=%dMiB diff=%dMiB max_prune_height=%d removed %d blk/rev pairs\n",
...
bool CVerifyDB::VerifyDB(const CChainParams& chainparams, CCoinsView *coinsview, int nCheckLevel, int nCheckDepth)
{
    ...
    LogPrintG(BCLogLevel::LOG_INFO, BCLog::BLOCKVALID, "[BlockValidation] Verifying last %i blocks at level %i\n", nCheckDepth, nCheckLevel);
    ...
    CValidationState state;
    int reportDone = 0;
    LogPrintG(BCLogLevel::LOG_INFO, BCLog::BLOCKVALID, "[BlockValidation] [0%%]...");
    for (CBlockIndex* pindex = chainActive.Tip(); pindex && pindex->pprev; pindex = pindex->pprev)
    {
        ...
        if (reportDone < percentageDone/10) {
            ...
            LogPrintG(BCLogLevel::LOG_INFO, BCLog::BLOCKVALID, "[BlockValidation] [%d%%]...", percentageDone);
        ...
        if (fPruneMode && !(pindex->nStatus & BLOCK_HAVE_DATA)) {
            ...
            LogPrintG(BCLogLevel::LOG_INFO, BCLog::BLOCKVALID, "[BlockValidation] VerifyDB(): block verification stopping at height %d (pruning, no data)\n", pindex->nHeight);
            break;
        }
        CBlock block;
...
bool LoadExternalBlockFile(const CChainParams& chainparams, FILE* fileIn, CDiskBlockPos *dbp)
{
        ...
        while (!blkdat.eof()) {
            ...
            try {
                ...
                if (hash != chainparams.GetConsensus().hashGenesisBlock && mapBlockIndex.find(block.hashPrevBlock) == mapBlockIndex.end()) {
                    LogPrintG(BCLogLevel::LOG_WARNING, BCLog::REINDEX, "[REINDEX] %s: Out of order block %s, parent %s not known\n", __func__, hash.ToString(),
                ...
                if (mapBlockIndex.count(hash) == 0 || (mapBlockIndex[hash]->nStatus & BLOCK_HAVE_DATA) == 0) {
                    ...
                    LogPrintG(BCLogLevel::LOG_WARNING, BCLog::REINDEX, "[REINDEX] Block Import: already had block %s at height %d\n", hash.ToString(), mapBlockIndex[hash]->nHeight);
                }
                ...
                while (!queue.empty()) {
                    ...
                    while (range.first != range.second) {
                        ...
                        {
                            LogPrintG(BCLogLevel::LOG_INFO, BCLog::REINDEX, "[REINDEX] %s: Processing out of order child %s of %s\n", __func__, pblockrecursive->GetHash().ToString(),
                                    head.ToString());
                            LOCK(cs_main);
                ...
                LogPrintG(BCLogLevel::LOG_ERROR, BCLog::REINDEX, "[REINDEX] %s: Deserialize or I/O error - %s\n", __func__, e.what());
            }
        }
    } catch (const std::runtime_error& e) {
    ...
    if (nLoaded > 0)
        LogPrintG(BCLogLevel::LOG_INFO, BCLog::REINDEX, "[REINDEX] Loaded %i blocks from external file in %dms\n", nLoaded, GetTimeMillis() - nStart);
    return nLoaded > 0;
}
...
bool LoadMempool(void)
{
    ...
    if (file.IsNull()) {
        LogPrintG(BCLogLevel::LOG_ERROR, BCLog::MEMPOOL, "[MemPool] Failed to open mempool file from disk. Continuing anyway.\n");
        return false;
    }
    ...
    try {
        ...
        LogPrintG(BCLogLevel::LOG_ERROR, BCLog::MEMPOOL, "[MemPool] Failed to deserialize mempool data on disk: %s. Continuing anyway.\n", e.what());
        return false;
    }

    LogPrintG(BCLogLevel::LOG_INFO, BCLog::MEMPOOL, "[MemPool] Imported mempool transactions from disk: %i succeeded, %i failed, %i expired, %i already there\n", count, failed, expired, already_there);
    return true;
}
...
bool DumpMempool(void)
{
    ...
    try {
        ...
        LogPrintG(BCLogLevel::LOG_INFO, BCLog::MEMPOOL, "[MemPool] Dumped mempool: %gs to copy, %gs to dump\n", (mid-start)*MICRO, (last-mid)*MICRO);
    } catch (const std::exception& e) {
        LogPrintG(BCLogLevel::LOG_ERROR, BCLog::MEMPOOL, "[MemPool] Failed to dump mempool: %s. Continuing anyway.\n", e.what());
        return false;
    }
    return true;
}
