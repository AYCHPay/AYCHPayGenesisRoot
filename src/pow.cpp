...
unsigned int LwmaGetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock, const Consensus::Params& params)
{
    ...
    {
        //LogPrintG(BCLogLevel::LOG_DEBUG, BCLog::POW, "[ProofOfWork] Using minimum diff (%d), because we are in testnet and the last block was a long time ago \n", UintToArith256(params.powLimit).GetCompact());
        return UintToArith256(params.powLimit).GetCompact();
    }
    // Special difficulty rule for if we don't have enough blocks:
    if (pindexLast->nHeight <= params.nZawyLwmaAveragingWindow )
    {
        //LogPrintG(BCLogLevel::LOG_DEBUG, BCLog::POW, "[ProofOfWork] Using minimum diff (%d), because we don't have more than %d blocks yet %d to go... \n", UintToArith256(params.powLimit).GetCompact(), params.nZawyLwmaAveragingWindow, pindexLast->nHeight - params.nZawyLwmaAveragingWindow);
        return UintToArith256(params.powLimit).GetCompact();
    }
...
{   
    ...
    if (t < k/10 ) 
    {
        //LogPrintG(BCLogLevel::LOG_DEBUG, BCLog::POW, "[ProofOfWork] t adjusted from %d to %d, as it was too low \n", t, k/10);
        t = k/10;  
    }
    arith_uint256 next_target = t * sum_target;

    //LogPrintG(BCLogLevel::LOG_DEBUG, BCLog::POW, "[ProofOfWork] LWMA diff: %d \n", next_target.GetCompact());
...
bool CheckProofOfWork(uint256 hash, unsigned int nBits, const Consensus::Params& params)
{
    ...
    {
        LogPrintG(BCLogLevel::LOG_DEBUG, BCLog::POW, "[ProofOfWork] CheckProofOfWork failed the first check: \n");
        if (fNegative)
        {
            LogPrintG(BCLogLevel::LOG_DEBUG, BCLog::POW, "[ProofOfWork] fNegative is true \n");
        }
        else if (bnTarget == 0)
        {
            LogPrintG(BCLogLevel::LOG_DEBUG, BCLog::POW, "[ProofOfWork] bnTarget is zero \n");
        }
        else if (fOverflow)
        {
            LogPrintG(BCLogLevel::LOG_DEBUG, BCLog::POW, "[ProofOfWork] fOverflow is true \n");
        }
        else if (bnTarget > UintToArith256(params.powLimit))
        {
            LogPrintG(BCLogLevel::LOG_DEBUG, BCLog::POW, "[ProofOfWork] bnTarget (%d) is greater than the minimum difficulty (%d) \n", bnTarget.GetCompact(), UintToArith256(params.powLimit).GetCompact());
        }
        return false;
    }
    ...
    {
        LogPrintG(BCLogLevel::LOG_DEBUG, BCLog::POW, "[ProofOfWork] CheckProofOfWork failed the second check: the hash (%d) is greater than the target (%d) \n", UintToArith256(hash).GetCompact(), bnTarget.GetCompact());
        return false;
    }

    //LogPrintG(BCLogLevel::LOG_DEBUG, BCLog::POW, "[ProofOfWork] CheckProofOfWork success! \n");
    return true;
}
