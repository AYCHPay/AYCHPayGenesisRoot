...
#define GENESIS_TIMEDATA_MAX_SAMPLES 200
...
void AddTimeData(const CNetAddr& ip, int64_t nOffsetSample)
{
    ...
    if (setKnown.size() == GENESIS_TIMEDATA_MAX_SAMPLES)
        return;
    ...
    LogPrintG(BCLogLevel::DEBUG, BCLog::NET,"added time data, samples %d, offset %+d (%+d minutes)\n", vTimeOffsets.size(), nOffsetSample, nOffsetSample/60);
    ...
    if (vTimeOffsets.size() >= 5 && vTimeOffsets.size() % 2 == 1)
    {
        ...
        if (LogAcceptCategory(BCLog::NET)) {
            for (int64_t n : vSorted) {
                LogPrintG(BCLogLevel::DEBUG, BCLog::NET, "[Networking] %+d  ", n);
            }
            LogPrintG(BCLogLevel::DEBUG, BCLog::NET, "[Networking] |  ");

            LogPrintG(BCLogLevel::DEBUG, BCLog::NET, "[Networking] nTimeOffset = %+d  (%+d minutes)\n", nTimeOffset, nTimeOffset/60);
        }
    }
}
