...
#include <masternodes/cachemultimap.h>
#include <masternodes/governance-exceptions.h>
#include <masternodes/governance-vote.h>
#include <masternodes/governance-votedb.h>
#include <key.h>
#include <net.h>
#include <sync.h>
#include <util.h>
...
static const int MIN_GOVERNANCE_PEER_PROTO_VERSION = BLOCKRESTRUCTURE_AND_MASTERNODES;
static const int GOVERNANCE_FILTER_PROTO_VERSION = BLOCKRESTRUCTURE_AND_MASTERNODES;
...
public:
    ...
    inline void SerializationOp(Stream& s, Operation ser_action)
    {
        ...
        if (s.GetType() & SER_DISK) {
            ...
            LogPrintG(BCLogLevel::LOG_NOTICE, BCLog::GOV, "[Governance] CGovernanceObject::SerializationOp Reading/writing votes from/to disk\n");
            READWRITE(nDeletionTime);
            READWRITE(fExpired);
            READWRITE(mapCurrentMNVotes);
            READWRITE(fileVotes);
            LogPrintG(BCLogLevel::LOG_NOTICE, BCLog::GOV, "[Governance] CGovernanceObject::SerializationOp hash = %s, vote count = %d\n", GetHash().ToString(), fileVotes.GetVoteCount());
        }
