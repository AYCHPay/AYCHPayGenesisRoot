...
void CGovernanceManager::ProcessMessage(CNode* pfrom, const std::string& strCommand, CDataStream& vRecv, CConnman& connman)
{
    ...
    if (strCommand == NetMsgType::MNGOVERNANCESYNC)
    {
        ...
        if (pfrom->nVersion < MIN_GOVERNANCE_PEER_PROTO_VERSION) {
            LogPrintG(BCLogLevel::LOG_WARNING, BCLog::GOV, "[Governance] MNGOVERNANCESYNC -- peer=%d using obsolete version %i\n", pfrom->GetId(), pfrom->nVersion);
        ...
        LogPrintG(BCLogLevel::LOG_INFO, BCLog::GOV, "[Governance] MNGOVERNANCESYNC -- syncing governance objects to our peer at %s\n", pfrom->addr.ToString());
    }
    ...
    else if (strCommand == NetMsgType::MNGOVERNANCEOBJECT)
    {
        ...
        if (pfrom->nVersion < MIN_GOVERNANCE_PEER_PROTO_VERSION) {
            LogPrintG(BCLogLevel::LOG_WARNING, BCLog::GOV, "[Governance] MNGOVERNANCEOBJECT -- peer=%d using obsolete version %i\n", pfrom->GetId(), pfrom->nVersion);
            connman.PushMessage(pfrom, CNetMsgMaker(pfrom->GetSendVersion()).Make(NetMsgType::REJECT, strCommand, REJECT_OBSOLETE,
                               strprintf("Version must be %d or greater", MIN_GOVERNANCE_PEER_PROTO_VERSION)));
            return;
        }
        ...
        if (!masternodeSync.IsMasternodeListSynced()) {
            LogPrintG(BCLogLevel::LOG_WARNING, BCLog::GOV, "[Governance] MNGOVERNANCEOBJECT -- masternode list not synced\n");
            return;
        }

        std::string strHash = nHash.ToString();

        LogPrintG(BCLogLevel::LOG_WARNING, BCLog::GOV, "[Governance] MNGOVERNANCEOBJECT -- Received object: %s\n", strHash);

        if (!AcceptObjectMessage(nHash)) {
            LogPrintG(BCLogLevel::LOG_WARNING, BCLog::GOV, "[Governance] MNGOVERNANCEOBJECT -- Received unrequested object: %s\n", strHash);
            return;
        }
        ...
        if (mapObjects.count(nHash) || mapPostponedObjects.count(nHash) ||
           mapErasedGovernanceObjects.count(nHash) || mapMasternodeOrphanObjects.count(nHash)) {
            ...
            LogPrintG(BCLogLevel::LOG_WARNING, BCLog::GOV, "[Governance] MNGOVERNANCEOBJECT -- Received already seen object: %s\n", strHash);
            return;
        }

        bool fRateCheckBypassed = false;
        if (!MasternodeRateCheck(govobj, true, false, fRateCheckBypassed)) {
            LogPrintG(BCLogLevel::LOG_ERROR, BCLog::GOV, "[Governance] MNGOVERNANCEOBJECT -- masternode rate check failed - %s - (current block height %d) \n", strHash, nCachedBlockHeight);
            return;
        }
        ...
        if (fRateCheckBypassed && (fIsValid || fMasternodeMissing)) {
            if (!MasternodeRateCheck(govobj, true)) {
                LogPrintG(BCLogLevel::LOG_ERROR, BCLog::GOV, "[Governance] MNGOVERNANCEOBJECT -- masternode rate check failed (after signature verification) - %s - (current block height %d) \n", strHash, nCachedBlockHeight);
                return;
            }
        }
        ...
        if (!fIsValid) {
            if (fMasternodeMissing) {
                ...
                if (count >= 10) {
                    LogPrintG(BCLogLevel::LOG_ERROR, BCLog::GOV, "[Governance] MNGOVERNANCEOBJECT -- Too many orphan objects, missing masternode=%s\n", govobj.GetMasternodeOutpoint().ToStringShort());           // ask for this object again in 2 minutes
                ...
                LogPrintG(BCLogLevel::LOG_ERROR, BCLog::GOV, "[Governance] MNGOVERNANCEOBJECT -- Missing masternode for: %s, strError = %s\n", strHash, strError);
            } else if (fMissingConfirmations) {
                AddPostponedObject(govobj);
                LogPrintG(BCLogLevel::LOG_ERROR, BCLog::GOV, "[Governance] MNGOVERNANCEOBJECT -- Not enough fee confirmations for: %s, strError = %s\n", strHash, strError);
            } else {
                LogPrintG(BCLogLevel::LOG_ERROR, BCLog::GOV, "[Governance] MNGOVERNANCEOBJECT -- Governance object is invalid - %s\n", strError);
    ...
    else if (strCommand == NetMsgType::MNGOVERNANCEOBJECTVOTE)
    {
        ...
        if (pfrom->nVersion < MIN_GOVERNANCE_PEER_PROTO_VERSION) {
            LogPrintG(BCLogLevel::LOG_WARNING, BCLog::GOV, "[Governance] MNGOVERNANCEOBJECTVOTE -- peer=%d using obsolete version %i\n", pfrom->GetId(), pfrom->nVersion);
            connman.PushMessage(pfrom, CNetMsgMaker(pfrom->GetSendVersion()).Make(NetMsgType::REJECT, strCommand, REJECT_OBSOLETE,
                               strprintf("Version must be %d or greater", MIN_GOVERNANCE_PEER_PROTO_VERSION)));
        }
        ...
        if (!masternodeSync.IsMasternodeListSynced()) {
            LogPrintG(BCLogLevel::LOG_WARNING, BCLog::GOV, "[Governance] MNGOVERNANCEOBJECTVOTE -- masternode list not synced\n");
            return;
        }

        LogPrintG(BCLogLevel::LOG_INFO, BCLog::GOV, "[Governance] MNGOVERNANCEOBJECTVOTE -- Received vote: %s\n", vote.ToString());

        std::string strHash = nHash.ToString();

        if (!AcceptVoteMessage(nHash)) {
            LogPrintG(BCLogLevel::LOG_WARNING, BCLog::GOV, "[Governance] MNGOVERNANCEOBJECTVOTE -- Received unrequested vote object: %s, hash: %s, peer = %d\n",
        ...
        if (ProcessVote(pfrom, vote, exception, connman)) {
            LogPrintG(BCLogLevel::LOG_WARNING, BCLog::GOV, "[Governance] MNGOVERNANCEOBJECTVOTE -- %s new\n", strHash);
            masternodeSync.BumpAssetLastTime("MNGOVERNANCEOBJECTVOTE");
            vote.Relay(connman);
        }
        else {
            LogPrintG(BCLogLevel::LOG_ERROR, BCLog::GOV, "[Governance] MNGOVERNANCEOBJECTVOTE -- Rejected vote, error = %s\n", exception.what());
...
void CGovernanceManager::AddGovernanceObject(CGovernanceObject& govobj, CConnman& connman, CNode* pfrom)
{
    ...
    if (!govobj.IsValidLocally(strError, true)) {
        LogPrintG(BCLogLevel::LOG_ERROR, BCLog::GOV, "[Governance] CGovernanceManager::AddGovernanceObject -- invalid governance object - %s - (nCachedBlockHeight %d) \n", strError, nCachedBlockHeight);
        return;
    }
    
    LogPrintG(BCLogLevel::LOG_INFO, BCLog::GOV, "[Governance] CGovernanceManager::AddGovernanceObject -- Adding object: hash = %s, type = %d\n", nHash.ToString(), govobj.GetObjectType());
    ...
    LogPrintG(BCLogLevel::LOG_INFO, BCLog::GOV, "[Governance] CGovernanceManager::AddGovernanceObject -- %s new, received from %s\n", strHash, pfrom? pfrom->GetAddrName() : "NULL");
...
{
    LogPrintG(BCLogLevel::LOG_INFO, BCLog::GOV, "[Governance] CGovernanceManager::UpdateCachesAndClean\n");
    ...
    while(it != mapObjects.end())
    {
        ...
        LogPrintG(BCLogLevel::LOG_INFO, BCLog::GOV, "[Governance] CGovernanceManager::UpdateCachesAndClean -- Checking object for deletion: %s, deletion time = %d, time since deletion = %d, delete flag = %d, expired flag = %d\n",
                 strHash, pObj->GetDeletionTime(), nTimeSinceDeletion, pObj->IsSetCachedDelete(), pObj->IsSetExpired());

        if ((pObj->IsSetCachedDelete() || pObj->IsSetExpired()) &&
           (nTimeSinceDeletion >= GOVERNANCE_DELETION_DELAY)) {
            LogPrintG(BCLogLevel::LOG_INFO, BCLog::GOV, "[Governance] CGovernanceManager::UpdateCachesAndClean -- erase obj %s\n", (*it).first.ToString());
            ...
            if (pObj->GetObjectType() == GOVERNANCE_OBJECT_PROPOSAL) {
                ...
                if (!validator.Validate()) {
                    LogPrintG(BCLogLevel::LOG_INFO, BCLog::GOV, "[Governance] CGovernanceManager::UpdateCachesAndClean -- set for deletion expired obj %s\n", (*it).first.ToString());
    ...
    LogPrintG(BCLogLevel::LOG_INFO, BCLog::GOV, "[Governance] CGovernanceManager::UpdateCachesAndClean -- %s\n", ToString());
}
...
bool CGovernanceManager::ConfirmInventoryRequest(const CInv& inv)
{
    ...
    LogPrintG(BCLogLevel::LOG_INFO, BCLog::GOV, "[Governance] CGovernanceManager::ConfirmInventoryRequest inv = %s\n", inv.ToString());
...
void CGovernanceManager::SyncSingleObjAndItsVotes(CNode* pnode, const uint256& nProp, const CBloomFilter& filter, CConnman& connman)
{
    ...
    LogPrintG(BCLogLevel::LOG_DEBUG, BCLog::GOV, "[Governance] CGovernanceManager::%s -- syncing single object to peer=%d, nProp = %s\n", __func__, pnode->GetId(), nProp.ToString());
    ...
    if (it == mapObjects.end()) {
        LogPrintG(BCLogLevel::LOG_WARNING, BCLog::GOV, "[Governance] CGovernanceManager::%s -- no matching object for hash %s, peer=%d\n", __func__, nProp.ToString(), pnode->GetId());
        return;
    }
    CGovernanceObject& govobj = it->second;
    std::string strHash = it->first.ToString();

    LogPrintG(BCLogLevel::LOG_DEBUG, BCLog::GOV, "[Governance] CGovernanceManager::%s -- attempting to sync govobj: %s, peer=%d\n", __func__, strHash, pnode->GetId());

    if (govobj.IsSetCachedDelete() || govobj.IsSetExpired()) {
        LogPrintG(BCLogLevel::LOG_WARNING, BCLog::GOV, "[Governance] CGovernanceManager::%s -- not syncing deleted/expired govobj: %s, peer=%d\n", __func__,
                  strHash, pnode->GetId());
        return;
    }

    // Push the govobj inventory message over to the other client
    LogPrintG(BCLogLevel::LOG_DEBUG, BCLog::GOV, "[Governance] CGovernanceManager::%s -- syncing govobj: %s, peer=%d\n", __func__, strHash, pnode->GetId());
    ...
    LogPrintG(BCLogLevel::LOG_DEBUG, BCLog::GOV, "[Governance] CGovernanceManager::%s -- sent 1 object and %d votes to peer=%d\n", __func__, nVoteCount, pnode->GetId());
}
...
void CGovernanceManager::SyncAll(CNode* pnode, CConnman& connman) const
{
    ...
    if (netfulfilledman.HasFulfilledRequest(pnode->addr, NetMsgType::MNGOVERNANCESYNC)) {
        ...
        LogPrintG(BCLogLevel::LOG_WARNING, BCLog::GOV, "[Governance] CGovernanceManager::%s -- peer already asked me for the list\n", __func__);
    ...
    LogPrintG(BCLogLevel::LOG_DEBUG, BCLog::GOV, "[Governance] CGovernanceManager::%s -- syncing all objects to peer=%d\n", __func__, pnode->GetId());
    ...
    for(object_m_cit it = mapObjects.begin(); it != mapObjects.end(); ++it) {
        ...
        if (govobj.IsSetCachedDelete() || govobj.IsSetExpired()) {
            LogPrintG(BCLogLevel::LOG_DEBUG, BCLog::GOV, "[Governance] CGovernanceManager::%s -- not syncing deleted/expired govobj: %s, peer=%d\n", __func__,
                      strHash, pnode->GetId());
            continue;
        }
    ...
    connman.PushMessage(pnode, msgMaker.Make(NetMsgType::SYNCSTATUSCOUNT, MASTERNODE_SYNC_GOVOBJ_VOTE, nVoteCount));
    LogPrintG(BCLogLevel::LOG_DEBUG, BCLog::GOV, "[Governance] CGovernanceManager::%s -- sent %d objects and %d votes to peer=%d\n", __func__, nObjCount, nVoteCount, pnode->GetId());
}
...
bool CGovernanceManager::MasternodeRateCheck(const CGovernanceObject& govobj, bool fUpdateFailStatus, bool fForce, bool& fRateCheckBypassed)
{
    ...
    if (nTimestamp < nNow - 2 * nGovernanceBlockOffsetSeconds) {
        LogPrintG(BCLogLevel::LOG_WARNING, BCLog::GOV, "[Governance] CGovernanceManager::MasternodeRateCheck -- object %s rejected due to too old timestamp, masternode = %s, timestamp = %d, current time = %d\n",
                 strHash, masternodeOutpoint.ToStringShort(), nTimestamp, nNow);
        return false;
    }

    if (nTimestamp > nNow + MAX_TIME_FUTURE_DEVIATION) {
        LogPrintG(BCLogLevel::LOG_WARNING, BCLog::GOV, "[Governance] CGovernanceManager::MasternodeRateCheck -- object %s rejected due to too new (future) timestamp, masternode = %s, timestamp = %d, current time = %d\n",
                 strHash, masternodeOutpoint.ToStringShort(), nTimestamp, nNow);
        return false;
    }
    ...
    LogPrintG(BCLogLevel::LOG_WARNING, BCLog::GOV, "[Governance] CGovernanceManager::MasternodeRateCheck -- Rate too high: object hash = %s, masternode = %s, object timestamp = %d, rate = %f, max rate = %f\n",
...
bool CGovernanceManager::ProcessVote(CNode* pfrom, const CGovernanceVote& vote, CGovernanceException& exception, CConnman& connman)
{
    ...
    if (cmapVoteToObject.HasKey(nHashVote)) {
        LogPrintG(BCLogLevel::LOG_INFO, BCLog::GOV, "[Governance] CGovernanceObject::ProcessVote -- skipping known valid vote %s for object %s\n", nHashVote.ToString(), nHashGovobj.ToString());
    ...
    if (govobj.IsSetCachedDelete() || govobj.IsSetExpired()) {
        LogPrintG(BCLogLevel::LOG_INFO, BCLog::GOV, "[Governance] CGovernanceObject::ProcessVote -- ignoring vote for expired or deleted object, hash = %s\n", nHashGovobj.ToString());
        LEAVE_CRITICAL_SECTION(cs);
        return false;
    }
...
void CGovernanceManager::CheckPostponedObjects(CConnman& connman)
{
    ...
    for(object_m_it it = mapPostponedObjects.begin(); it != mapPostponedObjects.end();) {
        ...
        if (govobj.IsCollateralValid(strError, fMissingConfirmations))
        {
            if (govobj.IsValidLocally(strError, false))
                ...
                LogPrintG(BCLogLevel::LOG_WARNING, BCLog::GOV, "[Governance] CGovernanceManager::CheckPostponedObjects -- %s invalid\n", nHash.ToString());
    ...
    for(hash_s_it it = setAdditionalRelayObjects.begin(); it != setAdditionalRelayObjects.end();) {
        ...
        if (itObject != mapObjects.end()) {
            ...
            if (fValid) {
                if (fReady) {
                    LogPrintG(BCLogLevel::LOG_DEBUG, BCLog::GOV, "[Governance] CGovernanceManager::CheckPostponedObjects -- additional relay: hash = %s\n", govobj.GetHash().ToString());
                    govobj.Relay(connman);
                } else {
                    it++;
                    continue;
            ...
            LogPrintG(BCLogLevel::LOG_DEBUG, BCLog::GOV, "[Governance] CGovernanceManager::CheckPostponedObjects -- additional relay of unknown object: %s\n", it->ToString());
        }
...
void CGovernanceManager::RequestGovernanceObject(CNode* pfrom, const uint256& nHash, CConnman& connman, bool fUseFilter)
{
    ...
    LogPrintG(BCLogLevel::LOG_DEBUG, BCLog::GOV, "[Governance] CGovernanceObject::RequestGovernanceObject -- hash = %s (peer=%d)\n", nHash.ToString(), pfrom->GetId());
    ...
    LogPrintG(BCLogLevel::LOG_DEBUG, BCLog::GOV, "[Governance] CGovernanceManager::RequestGovernanceObject -- nHash %s nVoteCount %d peer=%d\n", nHash.ToString(), nVoteCount, pfrom->GetId());
...
int CGovernanceManager::RequestGovernanceObjectVotes(const std::vector<CNode*>& vNodesCopy, CConnman& connman)
{
    ...
    LogPrintG(BCLogLevel::LOG_DEBUG, BCLog::GOV, "[Governance] CGovernanceManager::RequestGovernanceObjectVotes -- start: vpGovObjsTriggersTmp %d vpGovObjsTmp %d mapAskedRecently %d\n",
    ...
    LogPrintG(BCLogLevel::LOG_DEBUG, BCLog::GOV, "[Governance] CGovernanceManager::RequestGovernanceObjectVotes -- end: vpGovObjsTriggersTmp %d vpGovObjsTmp %d mapAskedRecently %d\n",
                vpGovObjsTriggersTmp.size(), vpGovObjsTmp.size(), mapAskedRecently.size());
...
void CGovernanceManager::InitOnLoad()
{
    ...
    LogPrintG(BCLogLevel::LOG_INFO, BCLog::GOV, "[Governance] Preparing masternode indexes and governance triggers...\n");
    RebuildIndexes();
    AddCachedTriggers();
    LogPrintG(BCLogLevel::LOG_DEBUG, BCLog::GOV, "[Governance] Masternode indexes and governance triggers prepared  %dms\n", GetTimeMillis() - nStart);
...
void CGovernanceManager::UpdatedBlockTip(const CBlockIndex *pindex, CConnman& connman)
{
    ...
    LogPrintG(BCLogLevel::LOG_DEBUG, BCLog::GOV, "[Governance] CGovernanceManager::UpdatedBlockTip -- nCachedBlockHeight: %d\n", nCachedBlockHeight);
...
void CGovernanceManager::RequestOrphanObjects(CConnman& connman)
{
    ...
    LogPrintG(BCLogLevel::LOG_DEBUG, BCLog::GOV, "[Governance] CGovernanceObject::RequestOrphanObjects -- number objects = %d\n", vecHashesFiltered.size());
