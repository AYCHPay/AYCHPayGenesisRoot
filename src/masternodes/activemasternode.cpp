...
{
    LogPrint(BCLog::MN, "[Masterenodes] CActiveMasternode::ManageState -- Start\n");
    if (!fMasternodeMode) {
        LogPrint(BCLog::MN, "[Masterenodes] CActiveMasternode::ManageState -- Not a masternode, returning\n");
        return;
    }

    if (Params().NetworkIDString() != CBaseChainParams::REGTEST && !masternodeSync.IsBlockchainSynced()) {
        nState = ACTIVE_MASTERNODE_SYNC_IN_PROCESS;
        LogPrint(BCLog::MN, "[Masterenodes] CActiveMasternode::ManageState -- %s: %s\n", GetStateString(), GetStatus());
        return;
    }
    ...
    LogPrint(BCLog::MN, "[Masterenodes] CActiveMasternode::ManageState -- status = %s, type = %s, pinger enabled = %d\n", GetStatus(), GetTypeString(), fPingerEnabled);
...
{
    if (!fPingerEnabled) {
        LogPrint(BCLog::MN, "[Masterenodes] CActiveMasternode::SendMasternodePing -- %s: masternode ping service is disabled, skipping...\n", GetStateString());
        return false;
    }
    ...
    if (!mnodeman.Has(outpoint)) {
        ...
        LogPrint(BCLog::MN, "[Masterenodes] CActiveMasternode::SendMasternodePing -- %s: %s\n", GetStateString(), strNotCapableReason);
        return false;
    }
    ...
    if (!mnp.Sign(keyMasternode, pubKeyMasternode)) {
        LogPrint(BCLog::MN, "[Masterenodes] CActiveMasternode::SendMasternodePing -- ERROR: Couldn't sign Masternode Ping\n");
        return false;
    }

    // Update lastPing for our masternode in Masternode list
    if (mnodeman.IsMasternodePingedWithin(outpoint, Params().GetConsensus().nMasternodeMinMnpSeconds, mnp.sigTime)) {
        LogPrint(BCLog::MN, "[Masterenodes] CActiveMasternode::SendMasternodePing -- Too early to send Masternode Ping\n");
        return false;
    }

    mnodeman.SetMasternodeLastPing(outpoint, mnp);

    LogPrint(BCLog::MN, "[Masterenodes] CActiveMasternode::SendMasternodePing -- Relaying ping, collateral=%s\n", outpoint.ToStringShort());
    mnp.Relay(connman);
...
{
    LogPrint(BCLog::MN, "[Masterenodes] CActiveMasternode::ManageStateInitial -- status = %s, type = %s, pinger enabled = %d\n", GetStatus(), GetTypeString(), fPingerEnabled);
    ...
    if (!fListen) {
        ...
        LogPrint(BCLog::MN, "[Masterenodes] CActiveMasternode::ManageStateInitial -- %s: %s\n", GetStateString(), strNotCapableReason);
        return;
    }
    ...
    LogPrint(BCLog::MN, "[Masterenodes] CActiveMasternode::ManageStateInitial -- Checking inbound connection to '%s'\n", service.ToString());
    ...
    LogPrint(BCLog::MN, "[Masterenodes] CActiveMasternode::ManageStateInitial -- End status = %s, type = %s, pinger enabled = %d\n", GetStatus(), GetTypeString(), fPingerEnabled);
}

void CActiveMasternode::ManageStateRemote()
{
    LogPrint(BCLog::MN, "[Masterenodes] CActiveMasternode::ManageStateRemote -- Start status = %s, type = %s, pinger enabled = %d, pubKeyMasternode.GetID() = %s\n", 
    ...
    if (mnodeman.GetMasternodeInfo(pubKeyMasternode, infoMn)) {
        if (infoMn.nProtocolVersion != PROTOCOL_VERSION) {
            ...
            LogPrint(BCLog::MN, "[Masterenodes] CActiveMasternode::ManageStateRemote -- %s: %s\n", GetStateString(), strNotCapableReason);
            return;
        }
        ...
        if (nState != ACTIVE_MASTERNODE_STARTED) {
            LogPrint(BCLog::MN, "[Masterenodes] CActiveMasternode::ManageStateRemote -- STARTED!\n");
