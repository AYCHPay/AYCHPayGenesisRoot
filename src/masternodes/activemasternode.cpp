...
{
    LogPrintG(BCLogLevel::LOG_DEBUG, BCLog::MN, "[Masternodes] CActiveMasternode::ManageState -- Start\n");
    if (!fMasternodeMode) {
        LogPrintG(BCLogLevel::LOG_DEBUG, BCLog::MN, "[Masternodes] CActiveMasternode::ManageState -- Not a masternode, returning\n");
        return;
    }

    if (Params().NetworkIDString() != CBaseChainParams::REGTEST && !masternodeSync.IsBlockchainSynced()) {
        nState = ACTIVE_MASTERNODE_SYNC_IN_PROCESS;
        LogPrintG(BCLogLevel::LOG_DEBUG, BCLog::MN, "[Masternodes] CActiveMasternode::ManageState -- %s: %s\n", GetStateString(), GetStatus());
        return;
    }
    ...
    LogPrintG(BCLogLevel::LOG_DEBUG, BCLog::MN, "[Masternodes] CActiveMasternode::ManageState -- status = %s, type = %s, pinger enabled = %d\n", GetStatus(), GetTypeString(), fPingerEnabled);
...
{
    if (!fPingerEnabled) {
        LogPrintG(BCLogLevel::LOG_DEBUG, BCLog::MN, "[Masternodes] CActiveMasternode::SendMasternodePing -- %s: masternode ping service is disabled, skipping...\n", GetStateString());
        return false;
    }
    ...
    if (!mnodeman.Has(outpoint)) {
        ...
        LogPrintG(BCLogLevel::LOG_DEBUG, BCLog::MN, "[Masternodes] CActiveMasternode::SendMasternodePing -- %s: %s\n", GetStateString(), strNotCapableReason);
        return false;
    }
    ...
    if (!mnp.Sign(keyMasternode, pubKeyMasternode)) {
        LogPrintG(BCLogLevel::LOG_DEBUG, BCLog::MN, "[Masternodes] CActiveMasternode::SendMasternodePing -- ERROR: Couldn't sign Masternode Ping\n");
        return false;
    }

    // Update lastPing for our masternode in Masternode list
    if (mnodeman.IsMasternodePingedWithin(outpoint, Params().GetConsensus().nMasternodeMinMnpSeconds, mnp.sigTime)) {
        LogPrintG(BCLogLevel::LOG_DEBUG, BCLog::MN, "[Masternodes] CActiveMasternode::SendMasternodePing -- Too early to send Masternode Ping\n");
        return false;
    }

    mnodeman.SetMasternodeLastPing(outpoint, mnp);

    LogPrintG(BCLogLevel::LOG_DEBUG, BCLog::MN, "[Masternodes] CActiveMasternode::SendMasternodePing -- Relaying ping, collateral=%s\n", outpoint.ToStringShort());
    mnp.Relay(connman);
...
{
    LogPrintG(BCLogLevel::LOG_DEBUG, BCLog::MN, "[Masternodes] CActiveMasternode::ManageStateInitial -- status = %s, type = %s, pinger enabled = %d\n", GetStatus(), GetTypeString(), fPingerEnabled);
    ...
    if (!fListen) {
        ...
        LogPrintG(BCLogLevel::LOG_DEBUG, BCLog::MN, "[Masternodes] CActiveMasternode::ManageStateInitial -- %s: %s\n", GetStateString(), strNotCapableReason);
        return;
    }
    ...
    LogPrintG(BCLogLevel::LOG_DEBUG, BCLog::MN, "[Masternodes] CActiveMasternode::ManageStateInitial -- Checking inbound connection to '%s'\n", service.ToString());
    ...
    LogPrintG(BCLogLevel::LOG_DEBUG, BCLog::MN, "[Masternodes] CActiveMasternode::ManageStateInitial -- End status = %s, type = %s, pinger enabled = %d\n", GetStatus(), GetTypeString(), fPingerEnabled);
}

void CActiveMasternode::ManageStateRemote()
{
    LogPrintG(BCLogLevel::LOG_DEBUG, BCLog::MN, "[Masternodes] CActiveMasternode::ManageStateRemote -- Start status = %s, type = %s, pinger enabled = %d, pubKeyMasternode.GetID() = %s\n", 
    ...
    if (mnodeman.GetMasternodeInfo(pubKeyMasternode, infoMn)) {
        if (infoMn.nProtocolVersion != PROTOCOL_VERSION) {
            ...
            LogPrintG(BCLogLevel::LOG_DEBUG, BCLog::MN, "[Masternodes] CActiveMasternode::ManageStateRemote -- %s: %s\n", GetStateString(), strNotCapableReason);
            return;
        }
        ...
        if (nState != ACTIVE_MASTERNODE_STARTED) {
            LogPrintG(BCLogLevel::LOG_DEBUG, BCLog::MN, "[Masternodes] CActiveMasternode::ManageStateRemote -- STARTED!\n");
