...
bool IsBlockValueValid(const CBlock& block, int nBlockHeight, CAmount blockReward, std::string& strErrorRet)
{
    ...
    LogPrintG(BCLogLevel::DEBUG, BCLog::MN, "[Masternodes] block.vtx[0]->GetValueOut() %lld <= blockReward %lld\n", block.vtx[0]->GetValueOut(), blockReward);
    ...
    LogPrintG(BCLogLevel::DEBUG, BCLog::GOV, "[Governance] block.vtx[0]->GetValueOut() %lld <= nGovernanceBlockMaxValue %lld\n", block.vtx[0]->GetValueOut(), nGovernanceBlockMaxValue);
    ...
    if (!masternodeSync.IsSynced() || fLiteMode) {
        ...
        if (CGovernanceBlock::IsValidBlockHeight(nBlockHeight)) {
            LogPrintG(BCLogLevel::DEBUG, BCLog::MN, "[Masternodes] IsBlockPayeeValid -- WARNING: Not enough data, checking governanceblock max bounds only\n");
    ...
    if (CGovernanceBlockManager::IsGovernanceBlockTriggered(nBlockHeight)) {
        if (CGovernanceBlockManager::IsValid(block.vtx[0], nBlockHeight, blockReward)) {
            LogPrintG(BCLogLevel::DEBUG, BCLog::GOV, "[Governance] IsBlockValueValid -- Valid governanceblock at height %d: %s", nBlockHeight, block.vtx[0]->ToString());
            // all checks are done in CGovernanceBlock::IsValid, nothing to do here
            return true;
        }

        // triggered but invalid? that's weird
        LogPrintG(BCLogLevel::DEBUG, BCLog::MN, "[Masternodes] IsBlockValueValid -- ERROR: Invalid governanceblock detected at height %d: %s", nBlockHeight, block.vtx[0]->ToString());
    ...
    LogPrintG(BCLogLevel::DEBUG, BCLog::GOV, "[Governance] IsBlockValueValid -- No triggered governanceblock detected at height %d\n", nBlockHeight);
    if (!isBlockRewardValueMet) {
...
bool IsBlockPayeeValid(const CTransactionRef& txNew, int nBlockHeight, CAmount blockReward)
{
    if (!masternodeSync.IsSynced() || fLiteMode) {
        ...
        LogPrintG(BCLogLevel::DEBUG, BCLog::MN, "[Masternodes] IsBlockPayeeValid -- WARNING: Not enough data, skipping block payee checks\n");
        return true;
    }
    ...
    LogPrintG(BCLogLevel::DEBUG, BCLog::GOV, "[Governance] IsBlockPayeeValid -- No triggered governanceblock detected at height %d\n", nBlockHeight);
    ...
    if (mnpayments.IsTransactionValid(txNew, nBlockHeight, blockReward)) {
        LogPrintG(BCLogLevel::DEBUG, BCLog::MN, "[Masternodes] IsBlockPayeeValid -- Valid masternode payment at height %d: %s", nBlockHeight, txNew->ToString());
        return true;
    }
...
void FillBlockPayments(CMutableTransaction& txNew, int nBlockHeight, CAmount blockReward, std::vector<CTxOut>& vtxoutMasternodeRet, std::vector<CTxOut>& vtxoutGovernanceRet)
{
    ...
    if (CGovernanceBlockManager::IsGovernanceBlockTriggered(nBlockHeight)) {
        LogPrintG(BCLogLevel::DEBUG, BCLog::GOV, "[Governance] FillBlockPayments -- triggered governanceblock creation at height %d\n", nBlockHeight);
    ...
    else
    {
        ...
        LogPrintG(BCLogLevel::DEBUG, BCLog::MN, "[Masternodes] FillBlockPayments -- triggered masternode block creation at height %d\n", nBlockHeight);
...
std::string GetRequiredPaymentsString(int nBlockHeight)
{
    ...
    if (CGovernanceBlockManager::IsGovernanceBlockTriggered(nBlockHeight)) {
        LogPrintG(BCLogLevel::DEBUG, BCLog::GOV, "[Governance] GetRequiredPaymentsString - getting governance payees at height %d\n", nBlockHeight);
        return CGovernanceBlockManager::GetRequiredPaymentsString(nBlockHeight);
    }
    else
    {
        // OTHERWISE, PAY MASTERNODE
        LogPrintG(BCLogLevel::DEBUG, BCLog::MN, "[Masternodes] GetRequiredPaymentsString - getting masternode payees at height %d\n", nBlockHeight);
...
void CMasternodePayments::FillBlockPayees(CMutableTransaction& txNew, int nBlockHeight, CAmount blockReward, std::vector<CTxOut>& vtxoutMasternodeRet) const
{
    ...
    if (!GetBlockPayees(nBlockHeight, payee, primaryPayeeActivationHeight)) {
        ...
        if (!mnodeman.GetNextMasternodesInQueueForPayment(nBlockHeight, true, nCount, mnInfo, secondaryMnInfoRet)) {
            ...
            LogPrintG(BCLogLevel::DEBUG, BCLog::MN, "[Masternodes] CMasternodePayments::FillBlockPayees -- Failed to detect masternode to pay\n");
            return;
        }
    ...
    LogPrintG(BCLogLevel::DEBUG, BCLog::MN, "[Masternodes] CMasternodePayments::FillBlockPayees -- Masternode payment %lld to %s\n", primaryMasternodePayment / COIN, EncodeDestination(CScriptID(payee)));
    ...
    if ((int)secondaryMnInfoRet.size() == 0)
    {
        ...
        if (!mnodeman.GetNextMasternodesInQueueForPayment(nBlockHeight, true, nCount, mnInfo, secondaryMnInfoRet)) {
            // ...and we can't calculate it on our own
            LogPrintG(BCLogLevel::DEBUG, BCLog::MN, "[Masternodes] CMasternodePayments::FillBlockPayees -- Failed to detect secondary masternode to pay\n");
            return;
        }
    }
    ...
    if ((int)secondaryMnInfoRet.size() > 0)
    {
        ...
        if ((int)secondaryPayees.size() > 0)
        {
            ...
            for(int i=0; i< (int)secondaryPayees.size(); ++i)
            {
                ...
                LogPrintG(BCLogLevel::DEBUG, BCLog::MN, "[Masternodes] CMasternodePayments::FillBlockPayees -- Secondary Masternode payment %lld to %s\n", amountToPaySecondary / COIN, EncodeDestination(CScriptID(secondaryPayees[i])));
            }
        }
    }
    LogPrintG(BCLogLevel::DEBUG, BCLog::MN, "[Masternodes] CMasternodePayments::FillBlockPayees -- Found %i secondary masternodes to pay\n", (int)secondaryPayees.size());
}
...
void CMasternodePayments::ProcessMessage(CNode* pfrom, const std::string& strCommand, CDataStream& vRecv, CConnman& connman)
{
    ...
    if (strCommand == NetMsgType::MASTERNODEPAYMENTSYNC) { //Masternode Payments Request Sync
        ...
        if (pfrom->nVersion < GetMinMasternodePaymentsProto()) {
            LogPrintG(BCLogLevel::DEBUG, BCLog::MN, "[Masternodes] MASTERNODEPAYMENTSYNC -- peer=%d using obsolete version %i\n", pfrom->GetId(), pfrom->nVersion);
        ...
        if (netfulfilledman.HasFulfilledRequest(pfrom->addr, NetMsgType::MASTERNODEPAYMENTSYNC)) {
            ...
            LogPrintG(BCLogLevel::DEBUG, BCLog::MN, "[Masternodes] MASTERNODEPAYMENTSYNC -- peer already asked me for the list, peer=%d\n", pfrom->GetId());
        ...
        if (pfrom->nVersion < GetMinMasternodePaymentsProto()) {
            LogPrintG(BCLogLevel::DEBUG, BCLog::MN, "[Masternodes] MASTERNODEPAYMENTVOTEPRIMARY -- peer=%d using obsolete version %i\n", pfrom->GetId(), pfrom->nVersion);
            connman.PushMessage(pfrom, CNetMsgMaker(pfrom->GetSendVersion()).Make(NetMsgType::REJECT, strCommand, REJECT_OBSOLETE,
                               strprintf("Version must be %d or greater", GetMinMasternodePaymentsProto())));
            return;
        }
...

        {
            ...
            if (!res.second && res.first->second.IsVerified()) {
                LogPrintG(BCLogLevel::DEBUG, BCLog::MN, "[Masternodes] MASTERNODEPAYMENTVOTEPRIMARY -- hash=%s, nBlockHeight=%d/%d seen\n",
        ...
        if (vote.nBlockHeight < nFirstBlock || vote.nBlockHeight > nCachedBlockHeight + 20) {
            LogPrintG(BCLogLevel::DEBUG, BCLog::MN, "[Masternodes] MASTERNODEPAYMENTVOTEPRIMARY -- vote out of range: nFirstBlock=%d, nBlockHeight=%d, nHeight=%d\n", nFirstBlock, vote.nBlockHeight, nCachedBlockHeight);
            return;
        }

        std::string strError = "";
        if (!vote.IsValid(pfrom, nCachedBlockHeight, strError, connman)) {
            LogPrintG(BCLogLevel::DEBUG, BCLog::MN, "[Masternodes] MASTERNODEPAYMENTVOTEPRIMARY -- invalid message, error: %s\n", strError);
            return;
        }
        ...
        if (!vote.CheckSignature(mnInfo.pubKeyMasternode, nCachedBlockHeight, nDos)) {
            if (nDos) {
                LOCK(cs_main);
                LogPrintG(BCLogLevel::DEBUG, BCLog::MN, "[Masternodes] MASTERNODEPAYMENTVOTEPRIMARY -- ERROR: invalid signature\n");
                Misbehaving(pfrom->GetId(), nDos);
            } else {
                // only warn about anything non-critical (i.e. nDos == 0) in debug mode
                LogPrintG(BCLogLevel::DEBUG, BCLog::MN, "[Masternodes] MASTERNODEPAYMENTVOTEPRIMARY -- WARNING: invalid signature\n");
            }
        ...
        LogPrintG(BCLogLevel::DEBUG, BCLog::MN, "[Masternodes] MASTERNODEPAYMENTVOTEPRIMARY -- vote: address=%s, nBlockHeight=%d, nHeight=%d, prevout=%s, hash=%s new\n",
...
bool CMasternodePaymentVote::Sign()
{
    ...
    if (chainActive.Height() > Params().GetConsensus().nMasternodeSignHashThreshold) {
        ...
        if (!CHashSigner::SignHash(hash, activeMasternode.keyMasternode, vchSig)) {
            LogPrintG(BCLogLevel::DEBUG, BCLog::MN, "[Masternodes] CMasternodePaymentVote::Sign -- SignHash() failed\n");
            return false;
        }

        if (!CHashSigner::VerifyHash(hash, activeMasternode.pubKeyMasternode, vchSig, strError)) {
            LogPrintG(BCLogLevel::DEBUG, BCLog::MN, "[Masternodes] CMasternodePaymentVote::Sign -- VerifyHash() failed, error: %s\n", strError);
            return false;
        }
    } else {
        ...
        if (!CMessageSigner::SignMessage(strMessage, vchSig, activeMasternode.keyMasternode)) {
            LogPrintG(BCLogLevel::DEBUG, BCLog::MN, "[Masternodes] CMasternodePaymentVote::Sign -- SignMessage() failed\n");
            return false;
        }

        if (!CMessageSigner::VerifyMessage(activeMasternode.pubKeyMasternode, vchSig, strMessage, strError)) {
            LogPrintG(BCLogLevel::DEBUG, BCLog::MN, "[Masternodes] CMasternodePaymentVote::Sign -- VerifyMessage() failed, error: %s\n", strError);
            return false;
        }
    }
...
bool CMasternodePayments::AddOrUpdatePaymentVote(const CMasternodePaymentVote& vote)
{
    ...
    LogPrintG(BCLogLevel::DEBUG, BCLog::MN, "[Masternodes] CMasternodePayments::AddOrUpdatePaymentVote -- added, hash=%s\n", nVoteHash.ToString());
...
bool CMasternodeBlockPayees::GetBestPayee(CScript& payeeRet, int& activationBlockHeightRet) const
{
    ...
    if (vecPayees.empty()) {
        LogPrintG(BCLogLevel::DEBUG, BCLog::MN, "[Masternodes] CMasternodeBlockPayees::GetBestPayee -- ERROR: couldn't find any payee\n");
        return false;
    }
...
bool CMasternodeBlockPayees::HasPayeeWithVotes(const CScript& payeeIn, int nVotesReq) const
{
    ...
    LogPrintG(BCLogLevel::DEBUG, BCLog::MN, "[Masternodes] CMasternodeBlockPayees::HasPayeeWithVotes -- ERROR: couldn't find any payee with %d+ votes\n", nVotesReq);
    return false;
}
...
bool CMasternodeBlockPayees::IsTransactionValid(const CTransactionRef& txNew, int nBlockHeight, CAmount blockReward) const
{
        ...
        if (payee.GetVoteCount() >= MNPAYMENTS_SIGNATURES_REQUIRED) {
            for (const auto& txout : txNew->vout) {
                if (payee.GetPayee() == txout.scriptPubKey && txout.nValue >= nMasternodePayment && txout.nValue <= (nMasternodePayment + CAmount(10000000))) {
                    LogPrintG(BCLogLevel::DEBUG, BCLog::MN, "[Masternodes] CMasternodeBlockPayees::IsTransactionValid -- Found required payment\n");
                    return true;
                }
            }
    ...
    LogPrintG(BCLogLevel::DEBUG, BCLog::MN, "[Masternodes] CMasternodeBlockPayees::IsTransactionValid -- ERROR: Missing required payment, possible payees: '%s', amount: %f GENX\n", strPayeesPossible, (float)nMasternodePayment / COIN);
    return false;
}
...
void CMasternodePayments::CheckAndRemove()
{
    ...
    while(it != mapMasternodePaymentVotesPrimary.end()) {
        ...
        if (nCachedBlockHeight - vote.nBlockHeight > nLimit) {
            LogPrintG(BCLogLevel::DEBUG, BCLog::MN, "[Masternodes] CMasternodePayments::CheckAndRemove -- Removing old Masternode payment: nBlockHeight=%d\n", vote.nBlockHeight);
    ...
    LogPrintG(BCLogLevel::DEBUG, BCLog::MN, "[Masternodes] CMasternodePayments::CheckAndRemove -- %s\n", ToString());
}
...
bool CMasternodePaymentVote::IsValid(CNode* pnode, int nValidationHeight, std::string& strError, CConnman& connman) const
{
    ...
    if (!mnodeman.GetMasternodeRank(masternodeOutpoint, nRank, nBlockHeight - 101, nMinRequiredProtocol)) {
        LogPrintG(BCLogLevel::DEBUG, BCLog::MN, "[Masternodes] CMasternodePaymentVote::IsValid -- Can't calculate rank for masternode %s\n",
    ...
    if (nRank > MNPAYMENTS_SIGNATURES_TOTAL) {
        ...
        if (nRank > MNPAYMENTS_SIGNATURES_TOTAL*2 && nBlockHeight > nValidationHeight) {
            ...
            LogPrintG(BCLogLevel::DEBUG, BCLog::MN, "[Masternodes] CMasternodePaymentVote::IsValid -- Error: %s\n", strError);
...
bool CMasternodePayments::ProcessBlock(int nBlockHeight, CConnman& connman)
{
    ...
    if (!mnodeman.GetMasternodeRank(activeMasternode.outpoint, nRank, nBlockHeight - 101, GetMinMasternodePaymentsProto())) {
        LogPrintG(BCLogLevel::DEBUG, BCLog::MN, "[Masternodes] CMasternodePayments::ProcessBlock -- Unknown Masternode\n");
        return false;
    }

    if (nRank > MNPAYMENTS_SIGNATURES_TOTAL) {
        LogPrintG(BCLogLevel::DEBUG, BCLog::MN, "[Masternodes] CMasternodePayments::ProcessBlock -- Masternode not in the top %d (%d)\n", MNPAYMENTS_SIGNATURES_TOTAL, nRank);
        return false;
    }
    ...
    LogPrintG(BCLogLevel::DEBUG, BCLog::MN, "[Masternodes] CMasternodePayments::ProcessBlock -- Start: nBlockHeight=%d, masternode=%s\n", nBlockHeight, activeMasternode.outpoint.ToStringShort());
    ...
    if (!mnodeman.GetNextMasternodesInQueueForPayment(nBlockHeight, true, nCount, mnInfo, secondaryMnInfoRet)) {
        LogPrintG(BCLogLevel::DEBUG, BCLog::MN, "[Masternodes] CMasternodePayments::ProcessBlock -- ERROR: Failed to find masternode to pay\n");
        return false;
    }

    LogPrintG(BCLogLevel::DEBUG, BCLog::MN, "[Masternodes] CMasternodePayments::ProcessBlock -- Masternode found by GetNextMasternodesInQueueForPayment(): %s\n", mnInfo.outpoint.ToStringShort());
    ...
    LogPrintG(BCLogLevel::DEBUG, BCLog::MN, "[Masternodes] CMasternodePayments::ProcessBlock -- vote: payee=%s, nBlockHeight=%d\n", EncodeDestination(address1), nBlockHeight);

    // SIGN MESSAGE TO NETWORK WITH OUR MASTERNODE KEYS

    LogPrintG(BCLogLevel::DEBUG, BCLog::MN, "[Masternodes] CMasternodePayments::ProcessBlock -- Signing vote\n");
    if (voteNew.Sign()) {
        LogPrintG(BCLogLevel::DEBUG, BCLog::MN, "[Masternodes] CMasternodePayments::ProcessBlock -- AddOrUpdatePaymentVote()\n");
...
void CMasternodePayments::CheckBlockVotes(int nBlockHeight)
{
    ...
    if (!mnodeman.GetMasternodeRanks(mns, nBlockHeight - 101, GetMinMasternodePaymentsProto())) {
        LogPrintG(BCLogLevel::DEBUG, BCLog::MN, "[Masternodes] CMasternodePayments::CheckBlockVotes -- nBlockHeight=%d, GetMasternodeRanks failed\n", nBlockHeight);
        return;
    }
    ...
    if (mapMasternodesDidNotVote.empty()) {
        LogPrintG(BCLogLevel::DEBUG, BCLog::MN, "[Masternodes] %s", debugStr);
        return;
    }
...
void CMasternodePaymentVote::Relay(CConnman& connman) const
{
    ...
    if (!masternodeSync.IsSynced()) {
        LogPrintG(BCLogLevel::DEBUG, BCLog::MN, "[Masternodes] CMasternodePayments::Relay -- won't relay until fully synced\n");
        return;
    }
...
void CMasternodePayments::Sync(CNode* pnode, CConnman& connman) const
{
    ...
    LogPrintG(BCLogLevel::DEBUG, BCLog::MN, "[Masternodes] CMasternodePayments::Sync -- Sent %d votes to peer=%d\n", nInvCount, pnode->GetId());
...
void CMasternodePayments::RequestLowDataPaymentBlocks(CNode* pnode, CConnman& connman) const
{
    ...
    while(nCachedBlockHeight - pindex->nHeight < nLimit) {
        ...
        if (it == mapMasternodeBlocksPrimary.end()) {
            ...
            if (vToFetch.size() == MAX_INV_SZ) {
                LogPrintG(BCLogLevel::DEBUG, BCLog::MN, "[Masternodes] CMasternodePayments::RequestLowDataPaymentBlocks -- asking peer=%d for %d blocks\n", pnode->GetId(), MAX_INV_SZ);
    ...
    while(it != mapMasternodeBlocksPrimary.end()) {
        ...
        // We should not violate GETDATA rules
        if (vToFetch.size() == MAX_INV_SZ) {
            LogPrintG(BCLogLevel::DEBUG, BCLog::MN, "[Masternodes] CMasternodePayments::RequestLowDataPaymentBlocks -- asking peer=%d for %d payment blocks\n", pnode->GetId(), MAX_INV_SZ);
            connman.PushMessage(pnode, msgMaker.Make(NetMsgType::GETDATA, vToFetch));
            // connman.PushMessage(pnode, NetMsgType::GETDATA, vToFetch);
            // Start filling new batch
            vToFetch.clear();
...
void CMasternodePayments::UpdatedBlockTip(const CBlockIndex *pindex, CConnman& connman)
{
    ...
    LogPrintG(BCLogLevel::DEBUG, BCLog::MN, "[Masternodes] CMasternodePayments::UpdatedBlockTip -- nCachedBlockHeight=%d\n", nCachedBlockHeight);
