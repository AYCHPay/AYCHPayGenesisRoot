...
void CMasternodeSync::BumpAssetLastTime(const std::string& strFuncName)
{
    ...
    nTimeLastBumped = GetTime();
    LogPrint(BCLog::MN, "[Masterenodes] CMasternodeSync::BumpAssetLastTime -- %s\n", strFuncName);
}
...
void CMasternodeSync::SwitchToNextAsset(CConnman& connman)
{
    switch(nRequestedMasternodeAssets)
    {
        ...
        case(MASTERNODE_SYNC_INITIAL):
            ...
            LogPrint(BCLog::MN, "[Masterenodes] CMasternodeSync::SwitchToNextAsset -- Starting %s\n", GetAssetName());
            break;
        case(MASTERNODE_SYNC_WAITING):
            LogPrint(BCLog::MN, "[Masterenodes] CMasternodeSync::SwitchToNextAsset -- Completed %s in %llds\n", GetAssetName(), GetTime() - nTimeAssetSyncStarted);
        ...
        case(MASTERNODE_SYNC_GOVERNANCE):
            ...
            LogPrint(BCLog::MN, "[Masterenodes] CMasternodeSync::SwitchToNextAsset -- Sync has finished\n");
...
void CMasternodeSync::ProcessMessage(CNode* pfrom, const std::string& strCommand, CDataStream& vRecv)
{
    if (strCommand == NetMsgType::SYNCSTATUSCOUNT) { //Sync status count
        ...
        LogPrint(BCLog::MN, "[Masterenodes] SYNCSTATUSCOUNT -- got inventory count: nItemID=%d  nCount=%d  peer=%d\n", nItemID, nCount, pfrom->GetId());
    }
}
...
void CMasternodeSync::ProcessTick(CConnman& connman)
{
    ...
    if (GetTime() - nTimeLastProcess > 60*60) {
        LogPrint(BCLog::MN, "[Masterenodes] CMasternodeSync::ProcessTick -- WARNING: no actions for too long, restarting sync...\n");
        Reset();
    ...
    if (IsFailed()) {
        if (nTimeLastFailure + (1*60) < GetTime()) { // 1 minute cooldown after failed sync
            LogPrint(BCLog::MN, "[Masterenodes] CMasternodeSync::ProcessTick -- WARNING: failed to sync, trying again...\n");
            Reset();
            SwitchToNextAsset(connman);
        }
        return;
    ...
    LogPrint(BCLog::MN, "[Masterenodes] CMasternodeSync::ProcessTick -- nTick %d nRequestedMasternodeAssets %d nRequestedMasternodeAttempt %d nSyncProgress %f\n", nTick, nRequestedMasternodeAssets, nRequestedMasternodeAttempt, nSyncProgress);
    ...
    for (auto& pnode : vNodesCopy)
    {
        ...
        // NORMAL NETWORK MODE - TESTNET/MAINNET
        {
            if (netfulfilledman.HasFulfilledRequest(pnode->addr, "full-sync")) {
                ...
                LogPrint(BCLog::MN, "[Masterenodes] CMasternodeSync::ProcessTick -- disconnecting from recently synced peer=%d\n", pnode->GetId());
                continue;
            }
            ...
            if (nRequestedMasternodeAssets == MASTERNODE_SYNC_LIST) {
                LogPrint(BCLog::MN, "[Masterenodes] CMasternodeSync::ProcessTick -- nTick %d nRequestedMasternodeAssets %d nTimeLastBumped %lld GetTime() %lld diff %lld\n", nTick, nRequestedMasternodeAssets, nTimeLastBumped, GetTime(), GetTime() - nTimeLastBumped);
                // check for timeout first
                if (GetTime() - nTimeLastBumped > MASTERNODE_SYNC_TIMEOUT_SECONDS) {
                    LogPrint(BCLog::MN, "[Masterenodes] CMasternodeSync::ProcessTick -- nTick %d nRequestedMasternodeAssets %d -- timeout\n", nTick, nRequestedMasternodeAssets);
                    if (nRequestedMasternodeAttempt == 0) {
                        LogPrint(BCLog::MN, "[Masterenodes] CMasternodeSync::ProcessTick -- ERROR: failed to sync %s\n", GetAssetName());
            ...
            if (nRequestedMasternodeAssets == MASTERNODE_SYNC_MNW) {
                ...
                if (nRequestedMasternodeAttempt > 1 && mnpayments.IsEnoughData()) {
                    LogPrint(BCLog::MN, "[Masterenodes] CMasternodeSync::ProcessTick -- nTick %d nRequestedMasternodeAssets %d -- found enough data\n", nTick, nRequestedMasternodeAssets);
                    SwitchToNextAsset(connman);
                    connman.ReleaseNodeVector(vNodesCopy);
                    return;
                }
            ...
            if (nRequestedMasternodeAssets == MASTERNODE_SYNC_GOVERNANCE) {
                ...
                if (GetTime() - nTimeLastBumped > MASTERNODE_SYNC_TIMEOUT_SECONDS) {
                    LogPrint(BCLog::MN, "[Masterenodes] CMasternodeSync::ProcessTick -- nTick %d nRequestedMasternodeAssets %d -- timeout\n", nTick, nRequestedMasternodeAssets);
                    if (nRequestedMasternodeAttempt == 0) {
                        LogPrint(BCLog::MN, "[Masterenodes] CMasternodeSync::ProcessTick -- WARNING: failed to sync %s\n", GetAssetName());
                ...
                if (netfulfilledman.HasFulfilledRequest(pnode->addr, "governance-sync")) {
                    ...
                    if (nObjsLeftToAsk == 0) {
                        ...
                        if (GetTime() - nTimeNoObjectsLeft > MASTERNODE_SYNC_TIMEOUT_SECONDS &&
                            ...
                            LogPrint(BCLog::MN, "[Masterenodes] CMasternodeSync::ProcessTick -- nTick %d nRequestedMasternodeAssets %d -- asked for all objects, nothing to do\n", nTick, nRequestedMasternodeAssets);
...
{
    LogPrint(BCLog::MN, "[Masterenodes] CMasternodeSync::AcceptedBlockHeader -- pindexNew->nHeight: %d\n", pindexNew->nHeight);
...
{
    LogPrint(BCLog::MN, "[Masterenodes] CMasternodeSync::NotifyHeaderTip -- pindexNew->nHeight: %d fInitialDownload=%d\n", pindexNew->nHeight, fInitialDownload);
...
{
    LogPrint(BCLog::MN, "[Masterenodes] CMasternodeSync::UpdatedBlockTip -- pindexNew->nHeight: %d fInitialDownload=%d\n", pindexNew->nHeight, fInitialDownload);
    ...
    LogPrint(BCLog::MN, "[Masterenodes] CMasternodeSync::UpdatedBlockTip -- pindexNew->nHeight: %d pindexBestHeader->nHeight: %d fInitialDownload=%d fReachedBestHeader=%d\n",
