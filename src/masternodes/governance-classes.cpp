...
bool CGovernanceTriggerManager::AddNewTrigger(uint256 nHash)
{
    ...
    try {
        ...
        LogPrintG(BCLogLevel::LOG_DEBUG, BCLog::GOV, "[Governance] CGovernanceTriggerManager::AddNewTrigger -- Error creating governance block: %s\n", e.what());
        return false;
    }
    catch (...) {
        LogPrintG(BCLogLevel::LOG_DEBUG, BCLog::GOV, "[Governance] CGovernanceTriggerManager::AddNewTrigger: Unknown Error creating governance block\n");
        return false;
    }
...
void CGovernanceTriggerManager::CleanAndRemove()
{
    ...
    while(it != mapTrigger.end()) {
        ...
        if (!pGovernanceBlock) {
            LogPrintG(BCLogLevel::LOG_DEBUG, BCLog::GOV, "[Governance] CGovernanceTriggerManager::CleanAndRemove -- NULL governance block marked for removal\n");
            remove = true;
        } else {
            pObj = governance.FindGovernanceObject(it->first);
            if (!pObj || pObj->GetObjectType() != GOVERNANCE_OBJECT_TRIGGER) {
                LogPrintG(BCLogLevel::LOG_DEBUG, BCLog::GOV, "[Governance] CGovernanceTriggerManager::CleanAndRemove -- Unknown or non-trigger governance block\n");
                pGovernanceBlock->SetStatus(SEEN_OBJECT_ERROR_INVALID);
            }
            LogPrintG(BCLogLevel::LOG_DEBUG, BCLog::GOV, "[Governance] CGovernanceTriggerManager::CleanAndRemove -- governance block status = %d\n", pGovernanceBlock->GetStatus());
            switch (pGovernanceBlock->GetStatus()) {
            case SEEN_OBJECT_ERROR_INVALID:
            case SEEN_OBJECT_UNKNOWN:
                LogPrintG(BCLogLevel::LOG_DEBUG, BCLog::GOV, "[Governance] CGovernanceTriggerManager::CleanAndRemove -- Unknown or invalid trigger found\n");
                remove = true;
                break;
        ...
        if (remove) {
            LogPrintG(BCLogLevel::LOG_DEBUG, BCLog::GOV, "[Governance] CGovernanceTriggerManager::CleanAndRemove -- Removing trigger object\n");
...
{
    LogPrintG(BCLogLevel::LOG_DEBUG, BCLog::GOV, "[Governance] CGovernanceBlockManager::IsGovernanceBlockTriggered -- Start nBlockHeight = %d\n", nBlockHeight);
    ...
    LogPrintG(BCLogLevel::LOG_DEBUG, BCLog::GOV, "[Governance] CGovernanceBlockManager::IsGovernanceBlockTriggered -- vecTriggers.size() = %d\n", vecTriggers.size());

    for (const auto& pGovernanceBlock : vecTriggers)
    {
        if (!pGovernanceBlock) {
            LogPrintG(BCLogLevel::LOG_DEBUG, BCLog::GOV, "[Governance] CGovernanceBlockManager::IsGovernanceBlockTriggered -- Non-governance block found, continuing\n");
            continue;
        }
        ...
        if (!pObj) {
            LogPrintG(BCLogLevel::LOG_DEBUG, BCLog::GOV, "[Governance] CGovernanceBlockManager::IsGovernanceBlockTriggered -- pObj == NULL, continuing\n");
            continue;
        }

        LogPrintG(BCLogLevel::LOG_DEBUG, BCLog::GOV, "[Governance] CGovernanceBlockManager::IsGovernanceBlockTriggered -- data = %s\n", pObj->GetDataAsPlainString());

        // note : 12.1 - is epoch calculation correct?

        if (nBlockHeight != pGovernanceBlock->GetBlockHeight()) {
            LogPrintG(BCLogLevel::LOG_DEBUG, BCLog::GOV, "[Governance] CGovernanceBlockManager::IsGovernanceBlockTriggered -- block height doesn't match nBlockHeight = %d, blockStart = %d, continuing\n",
                     nBlockHeight,
        ...
        if (pObj->IsSetCachedFunding()) {
            LogPrintG(BCLogLevel::LOG_DEBUG, BCLog::GOV, "[Governance] CGovernanceBlockManager::IsGovernanceBlockTriggered -- fCacheFunding = true, returning true\n");
            return true;
        }
        else  {
            LogPrintG(BCLogLevel::LOG_DEBUG, BCLog::GOV, "[Governance] CGovernanceBlockManager::IsGovernanceBlockTriggered -- fCacheFunding = false, continuing\n");
        }
    }
...
void CGovernanceBlockManager::CreateGovernanceBlock(CMutableTransaction& txNewRet, int nBlockHeight, std::vector<CTxOut>& vtxoutGovernanceRet)
{
    ...
    if (!CGovernanceBlockManager::GetBestGovernanceBlock(pGovernanceBlock, nBlockHeight)) {
        LogPrintG(BCLogLevel::LOG_DEBUG, BCLog::GOV, "[Governance] CGovernanceBlockManager::CreateGovernanceBlock -- Can't find governance block for height %d\n", nBlockHeight);
        return;
    }
    ...
    for (int i = 0; i < pGovernanceBlock->CountPayments(); i++) {
        ...
        if (pGovernanceBlock->GetPayment(i, payment)) {
            ...
            LogPrintG(BCLogLevel::LOG_DEBUG, BCLog::GOV, "[Governance] NEW GovernanceBlock : output %d (addr %s, amount %d)\n", i, EncodeDestination(address1), payment.nAmount);
        }
    }
}
      ...
      vecPayments()
{
    ...
    LogPrintG(BCLogLevel::LOG_DEBUG, BCLog::GOV, "[Governance] CGovernanceBlock -- nBlockHeight = %d, strAddresses = %s, strAmounts = %s, vecPayments.size() = %d\n",
...
CAmount CGovernanceBlock::GetPaymentsLimit(int nBlockHeight)
{
    ...
    LogPrintG(BCLogLevel::LOG_DEBUG, BCLog::GOV, "[Governance] CGovernanceBlock::GetPaymentsLimit -- Valid governance block height %d, payments max %lld\n", nBlockHeight, nPaymentsLimit);
...
bool CGovernanceBlock::IsValid(const CTransactionRef& txNew, int nBlockHeight, CAmount blockReward)
{
    ...
    // code is running.
    if (!IsValidBlockHeight(nBlockHeight)) {
        LogPrintG(BCLogLevel::LOG_DEBUG, BCLog::GOV, "[Governance] CGovernanceBlock::IsValid -- ERROR: Block invalid, incorrect block height\n");
        return false;
    }
    ...
    LogPrintG(BCLogLevel::LOG_DEBUG, BCLog::GOV, "[Governance] CGovernanceBlock::IsValid nOutputs = %d, nPayments = %d, GetDataAsHexString = %s\n",
    ...
    if (nMinerPayments < 0) {
        ...
        LogPrintG(BCLogLevel::LOG_DEBUG, BCLog::GOV, "[Governance] CGovernanceBlock::IsValid -- ERROR: Block invalid, too few governance block payments\n");
        return false;
    }
    ...
    if (nPaymentsTotalAmount > nPaymentsLimit) {
        LogPrintG(BCLogLevel::LOG_DEBUG, BCLog::GOV, "[Governance] CGovernanceBlock::IsValid -- ERROR: Block invalid, payments limit exceeded: payments %lld, limit %lld\n", nPaymentsTotalAmount, nPaymentsLimit);
        return false;
    }
    ...
    if (nBlockValue > blockReward + nPaymentsTotalAmount) {
        LogPrintG(BCLogLevel::LOG_DEBUG, BCLog::GOV, "[Governance] CGovernanceBlock::IsValid -- ERROR: Block invalid, block value limit exceeded: block %lld, limit %lld\n", nBlockValue, blockReward + nPaymentsTotalAmount);
        return false;
    }
    ...
    for (int i = 0; i < nPayments; i++) {
        ...
        if (!GetPayment(i, payment)) {
            ...
            LogPrintG(BCLogLevel::LOG_DEBUG, BCLog::GOV, "[Governance] CGovernanceBlock::IsValid -- WARNING: Failed to find payment: %d of %d total payments\n", i, nPayments);
            continue;
        }
        ...
        if (!fPaymentMatch) {
            ...
            CTxDestination address1;
            ExtractDestination(payment.script, address1);
            LogPrintG(BCLogLevel::LOG_DEBUG, BCLog::GOV, "[Governance] CGovernanceBlock::IsValid -- ERROR: Block invalid: %d payment %d to %s not found\n", i, payment.nAmount, EncodeDestination(address1));
...
bool CGovernanceBlock::IsExpired()
{
    ...
    if (governance.GetCachedBlockHeight() > nExpirationBlock) {
        LogPrintG(BCLogLevel::LOG_DEBUG, BCLog::GOV, "[Governance] CGovernanceBlock::IsExpired -- Outdated trigger found\n");
        fExpired = true;
        CGovernanceObject* pgovobj = GetGovernanceObject();
        if (pgovobj) {
            LogPrintG(BCLogLevel::LOG_DEBUG, BCLog::GOV, "[Governance] CGovernanceBlock::IsExpired -- Expiring outdated object: %s\n", pgovobj->GetHash().ToString());
...
std::string CGovernanceBlockManager::GetRequiredPaymentsString(int nBlockHeight)
{
    ...
    CGovernanceBlock_sptr pGovernanceBlock;
    if (!GetBestGovernanceBlock(pGovernanceBlock, nBlockHeight)) {
        LogPrintG(BCLogLevel::LOG_DEBUG, BCLog::GOV, "[Governance] CGovernanceBlockManager::GetRequiredPaymentsString -- Can't find governance block for height %d\n", nBlockHeight);
        return "error";
    }
