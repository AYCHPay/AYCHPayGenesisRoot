...
bool CGovernanceTriggerManager::AddNewTrigger(uint256 nHash)
{
    ...
    try {
        ...
        LogPrint(BCLog::GOV, "CGovernanceTriggerManager::AddNewTrigger -- Error creating governance block: %s\n", e.what());
        return false;
    }
    catch (...) {
        LogPrint(BCLog::GOV, "CGovernanceTriggerManager::AddNewTrigger: Unknown Error creating governance block\n");
        return false;
    }
...
void CGovernanceTriggerManager::CleanAndRemove()
{
    ...
    while(it != mapTrigger.end()) {
        ...
        if (!pGovernanceBlock) {
            LogPrint(BCLog::GOV, "CGovernanceTriggerManager::CleanAndRemove -- NULL governance block marked for removal\n");
            remove = true;
        } else {
            pObj = governance.FindGovernanceObject(it->first);
            if (!pObj || pObj->GetObjectType() != GOVERNANCE_OBJECT_TRIGGER) {
                LogPrint(BCLog::GOV, "CGovernanceTriggerManager::CleanAndRemove -- Unknown or non-trigger governance block\n");
                pGovernanceBlock->SetStatus(SEEN_OBJECT_ERROR_INVALID);
            }
            LogPrint(BCLog::GOV, "CGovernanceTriggerManager::CleanAndRemove -- governance block status = %d\n", pGovernanceBlock->GetStatus());
...
{
    LogPrint(BCLog::GOV, "CGovernanceBlockManager::IsGovernanceBlockTriggered -- Start nBlockHeight = %d\n", nBlockHeight);
    ...
    LogPrint(BCLog::GOV, "CGovernanceBlockManager::IsGovernanceBlockTriggered -- vecTriggers.size() = %d\n", vecTriggers.size());

    for (const auto& pGovernanceBlock : vecTriggers)
    {
        if (!pGovernanceBlock) {
            LogPrint(BCLog::GOV, "CGovernanceBlockManager::IsGovernanceBlockTriggered -- Non-governance block found, continuing\n");
            continue;
        }
        ...
        if (!pObj) {
            LogPrint(BCLog::GOV, "CGovernanceBlockManager::IsGovernanceBlockTriggered -- pObj == NULL, continuing\n");
            continue;
        }

        LogPrint(BCLog::GOV, "CGovernanceBlockManager::IsGovernanceBlockTriggered -- data = %s\n", pObj->GetDataAsPlainString());

        // note : 12.1 - is epoch calculation correct?

        if (nBlockHeight != pGovernanceBlock->GetBlockHeight()) {
            LogPrint(BCLog::GOV, "CGovernanceBlockManager::IsGovernanceBlockTriggered -- block height doesn't match nBlockHeight = %d, blockStart = %d, continuing\n",
                     nBlockHeight,
        ...
        if (pObj->IsSetCachedFunding()) {
            LogPrint(BCLog::GOV, "CGovernanceBlockManager::IsGovernanceBlockTriggered -- fCacheFunding = true, returning true\n");
            return true;
        }
        else  {
            LogPrint(BCLog::GOV, "CGovernanceBlockManager::IsGovernanceBlockTriggered -- fCacheFunding = false, continuing\n");
        }
    }
...
void CGovernanceBlockManager::CreateGovernanceBlock(CMutableTransaction& txNewRet, int nBlockHeight, std::vector<CTxOut>& vtxoutGovernanceRet)
{
    ...
    if (!CGovernanceBlockManager::GetBestGovernanceBlock(pGovernanceBlock, nBlockHeight)) {
        LogPrint(BCLog::GOV, "CGovernanceBlockManager::CreateGovernanceBlock -- Can't find governance block for height %d\n", nBlockHeight);
        return;
    }
    ...
    for (int i = 0; i < pGovernanceBlock->CountPayments(); i++) {
        ...
        if (pGovernanceBlock->GetPayment(i, payment)) {
            ...
            LogPrint(BCLog::GOV, "NEW GovernanceBlock : output %d (addr %s, amount %d)\n", i, EncodeDestination(address1), payment.nAmount);
        }
    }
}
      ...
      vecPayments()
{
    ...
    LogPrint(BCLog::GOV, "CGovernanceBlock -- nBlockHeight = %d, strAddresses = %s, strAmounts = %s, vecPayments.size() = %d\n",
...
bool CGovernanceBlock::IsValidBlockHeight(int nBlockHeight)
{
    ...
    return nBlockHeight >= Params().GetConsensus().nMasternodePaymentsStartBlock &&
        ((nBlockHeight % Params().GetConsensus().GetBonusBlockInterval()) == Params().GetConsensus().nGovernanceBlockOffset);
}
...
void CGovernanceBlock::GetNearestGovernanceBlocksHeights(int nBlockHeight, int& nLastGovernanceBlockRet, int& nNextGovernanceBlockRet)
{
    ...
    int nGovernanceBlockCycle = consensusParams.nSuperBlockInterval;
...
CAmount CGovernanceBlock::GetPaymentsLimit(int nBlockHeight)
{
    ...
    LogPrint(BCLog::GOV, "CGovernanceBlock::GetPaymentsLimit -- Valid governance block height %d, payments max %lld\n", nBlockHeight, nPaymentsLimit);
...
bool CGovernanceBlock::IsValid(const CTransactionRef& txNew, int nBlockHeight, CAmount blockReward)
{
    ...
    // code is running.
    if (!IsValidBlockHeight(nBlockHeight)) {
        LogPrint(BCLog::GOV, "CGovernanceBlock::IsValid -- ERROR: Block invalid, incorrect block height\n");
        return false;
    }
    ...
    LogPrint(BCLog::GOV, "CGovernanceBlock::IsValid nOutputs = %d, nPayments = %d, GetDataAsHexString = %s\n",
    ...
    if (nMinerPayments < 0) {
        ...
        LogPrint(BCLog::GOV, "CGovernanceBlock::IsValid -- ERROR: Block invalid, too few governance block payments\n");
        return false;
    }
    ...
    if (nPaymentsTotalAmount > nPaymentsLimit) {
        LogPrint(BCLog::GOV, "CGovernanceBlock::IsValid -- ERROR: Block invalid, payments limit exceeded: payments %lld, limit %lld\n", nPaymentsTotalAmount, nPaymentsLimit);
        return false;
    }
    ...
    if (nBlockValue > blockReward + nPaymentsTotalAmount) {
        LogPrint(BCLog::GOV, "CGovernanceBlock::IsValid -- ERROR: Block invalid, block value limit exceeded: block %lld, limit %lld\n", nBlockValue, blockReward + nPaymentsTotalAmount);
        return false;
    }
    ...
    for (int i = 0; i < nPayments; i++) {
        ...
        if (!GetPayment(i, payment)) {
            ...
            LogPrint(BCLog::GOV, "CGovernanceBlock::IsValid -- WARNING: Failed to find payment: %d of %d total payments\n", i, nPayments);
            continue;
        }
        ...
        if (!fPaymentMatch) {
            ...
            CTxDestination address1;
            ExtractDestination(payment.script, address1);
            LogPrint(BCLog::GOV, "CGovernanceBlock::IsValid -- ERROR: Block invalid: %d payment %d to %s not found\n", i, payment.nAmount, EncodeDestination(address1));
...
bool CGovernanceBlock::IsExpired()
{
    ...
    LogPrint(BCLog::GOV, "CGovernanceBlock::IsExpired -- nBlockHeight = %d, nExpirationBlock = %d\n", nBlockHeight, nExpirationBlock);

    if (governance.GetCachedBlockHeight() > nExpirationBlock) {
        LogPrint(BCLog::GOV, "CGovernanceBlock::IsExpired -- Outdated trigger found\n");
        fExpired = true;
        CGovernanceObject* pgovobj = GetGovernanceObject();
        if (pgovobj) {
            LogPrint(BCLog::GOV, "CGovernanceBlock::IsExpired -- Expiring outdated object: %s\n", pgovobj->GetHash().ToString());
...
std::string CGovernanceBlockManager::GetRequiredPaymentsString(int nBlockHeight)
{
    ...
    CGovernanceBlock_sptr pGovernanceBlock;
    if (!GetBestGovernanceBlock(pGovernanceBlock, nBlockHeight)) {
        LogPrint(BCLog::GOV, "CGovernanceBlockManager::GetRequiredPaymentsString -- Can't find governance block for height %d\n", nBlockHeight);
        return "error";
    }
