...
bool CMasternode::UpdateFromNewBroadcast(CMasternodeBroadcast& mnb, CConnman& connman)
{
    ...
    if (fMasternodeMode && pubKeyMasternode == activeMasternode.pubKeyMasternode) {
        ...
        if (nProtocolVersion == PROTOCOL_VERSION) {
            ...
            LogPrintG(BCLogLevel::DEBUG, BCLog::MN, "[Masternodes] CMasternode::UpdateFromNewBroadcast -- wrong PROTOCOL_VERSION, re-activate your MN: message nProtocolVersion=%d  PROTOCOL_VERSION=%d\n", nProtocolVersion, PROTOCOL_VERSION);
            return false;
        }
    }
    return true;
...
void CMasternode::Check(bool fForce)
{
    ...
    LogPrintG(BCLogLevel::DEBUG, BCLog::MN, "[Masternodes] CMasternode::Check -- Masternode %s is in %s state\n", outpoint.ToStringShort(), GetStateString());
    ...
    if (!fUnitTest) {
        ...
        if (!GetUTXOCoin(outpoint, coin)) {
            ...
            LogPrintG(BCLogLevel::DEBUG, BCLog::MN, "[Masternodes] CMasternode::Check -- Failed to find Masternode UTXO, masternode=%s\n", outpoint.ToStringShort());
            return;
        }
    ...
    if (IsPoSeBanned()) {
        ...
        LogPrintG(BCLogLevel::DEBUG, BCLog::MN, "[Masternodes] CMasternode::Check -- Masternode %s is unbanned and back in list now\n", outpoint.ToStringShort());
        DecreasePoSeBanScore();
    } else if (nPoSeBanScore >= Params().GetConsensus().nMasternodePoseBanMaxScore) {
        nActiveState = MASTERNODE_POSE_BAN;
        // ban for the whole payment cycle
        nPoSeBanHeight = nHeight + mnodeman.size();
        LogPrintG(BCLogLevel::DEBUG, BCLog::MN, "[Masternodes] CMasternode::Check -- Masternode %s is banned till block %d now\n", outpoint.ToStringShort(), nPoSeBanHeight);
        return;
    }
    ...
    if (fRequireUpdate) {
        ...
        if (nActiveStatePrev != nActiveState) {
            LogPrintG(BCLogLevel::DEBUG, BCLog::MN, "[Masternodes] CMasternode::Check -- Masternode %s is in %s state now\n", outpoint.ToStringShort(), GetStateString());
        }
        return;
    }
    ...
    if (fWaitForPing && !fOurMasternode) {
        ...
        if (IsExpired() || IsSentinelPingExpired() || IsNewStartRequired()) {
            LogPrintG(BCLogLevel::DEBUG, BCLog::MN, "[Masternodes] CMasternode::Check -- Masternode %s is in %s state, waiting for ping\n", outpoint.ToStringShort(), GetStateString());
            return;
        }
    }
    ...
    if (!fWaitForPing || fOurMasternode) {
        ...
        LogPrintG(BCLogLevel::DEBUG, BCLog::MN, "[Masternodes] CMasternode::Check -- outpoint=%s, GetAdjustedTime()=%d, fSentinelPingExpired=%d\n",
...
void CMasternode::UpdateLastPaid(const CBlockIndex *pindex, int nMaxBlocksToScanBack)
{
    ...
    //LogPrintG(BCLogLevel::DEBUG, BCLog::MN, "[Masternodes] CMasternode::UpdateLastPaidBlock -- searching for block with payment to %s\n", outpoint.ToStringShort());
        ...
        if ((mnCount > 2 && checkitOut && hazIt) || (mnCount <= 2))
        {
            ...
            for (const auto& txout : block.vtx[0]->vout)
            {
                if (mnpayee == txout.scriptPubKey && nMasternodePaymentPrimary == txout.nValue) {
                    ...
                    LogPrintG(BCLogLevel::DEBUG, BCLog::MN, "[Masternodes] CMasternode::UpdateLastPaidBlock -- searching for block with primary payment to %s -- found new %d\n", outpoint.ToStringShort(), nBlockLastPaidPrimary);
                    return;
                }
                else if (mnpayee == txout.scriptPubKey)
                {
                    ...
                    LogPrintG(BCLogLevel::DEBUG, BCLog::MN, "[Masternodes] CMasternode::UpdateLastPaidBlock -- searching for block with secondary payment to %s -- found new %d\n", outpoint.ToStringShort(), nBlockLastPaidSecondary);
                    return;
                }
    ...
    LogPrintG(BCLogLevel::DEBUG, BCLog::MN, "[Masternodes] CMasternode::UpdateLastPaidBlock -- searching for block with payment to %s -- keeping old %d\n", outpoint.ToStringShort(), nBlockLastPaidPrimary);
}
...
bool CMasternodeBroadcast::Create(const COutPoint& outpoint, const CService& service, const CKey& keyCollateralAddressNew, const CPubKey& pubKeyCollateralAddressNew, const CKey& keyMasternodeNew, const CPubKey& pubKeyMasternodeNew, std::string &strErrorRet, CMasternodeBroadcast &mnbRet)
{
    ...
    LogPrintG(BCLogLevel::DEBUG, BCLog::MN, "[Masternodes] CMasternodeBroadcast::Create -- pubKeyCollateralAddressNew = %s, pubKeyMasternodeNew.GetID() = %s\n",
...
bool CMasternodeBroadcast::SimpleCheck(int& nDos)
{
    ...
    if (!IsValidNetAddr()) {
        LogPrintG(BCLogLevel::DEBUG, BCLog::MN, "[Masternodes] CMasternodeBroadcast::SimpleCheck -- Invalid addr, rejected: masternode=%s  addr=%s\n",
    ...
    if (sigTime > GetAdjustedTime() + 60 * 60) {
        LogPrintG(BCLogLevel::DEBUG, BCLog::MN, "[Masternodes] CMasternodeBroadcast::SimpleCheck -- Signature rejected, too far into the future: masternode=%s\n", outpoint.ToStringShort());
        nDos = 1;
        return false;
    }
    ...
    if (nProtocolVersion < mnpayments.GetMinMasternodePaymentsProto()) {
        LogPrintG(BCLogLevel::DEBUG, BCLog::MN, "[Masternodes] CMasternodeBroadcast::SimpleCheck -- outdated Masternode: masternode=%s  nProtocolVersion=%d\n", outpoint.ToStringShort(), nProtocolVersion);
        nActiveState = MASTERNODE_UPDATE_REQUIRED;
    }
    ...
    if (pubkeyScript.size() != 25) {
        LogPrintG(BCLogLevel::DEBUG, BCLog::MN, "[Masternodes] CMasternodeBroadcast::SimpleCheck -- pubKeyCollateralAddress has the wrong size\n");
        nDos = 100;
        return false;
    }
    ...
    if (pubkeyScript2.size() != 25) {
        LogPrintG(BCLogLevel::DEBUG, BCLog::MN, "[Masternodes] CMasternodeBroadcast::SimpleCheck -- pubKeyMasternode has the wrong size\n");
        nDos = 100;
        return false;
    }
...
bool CMasternodeBroadcast::Update(CMasternode* pmn, int& nDos, CConnman& connman)
{
    ...
    if (pmn->sigTime > sigTime) {
        LogPrintG(BCLogLevel::DEBUG, BCLog::MN, "[Masternodes] CMasternodeBroadcast::Update -- Bad sigTime %d (existing broadcast is at %d) for Masternode %s %s\n",
    ...
    if (pmn->IsPoSeBanned()) {
        LogPrintG(BCLogLevel::DEBUG, BCLog::MN, "[Masternodes] CMasternodeBroadcast::Update -- Banned by PoSe, masternode=%s\n", outpoint.ToStringShort());
        return false;
    }

    // IsVnAssociatedWithPubkey is validated once in CheckOutpoint, after that they just need to match
    if (pmn->pubKeyCollateralAddress != pubKeyCollateralAddress) {
        LogPrintG(BCLogLevel::DEBUG, BCLog::MN, "[Masternodes] CMasternodeBroadcast::Update -- Got mismatched pubKeyCollateralAddress and outpoint\n");
        nDos = 33;
        return false;
    }

    if (!CheckSignature(nDos)) {
        LogPrintG(BCLogLevel::DEBUG, BCLog::MN, "[Masternodes] CMasternodeBroadcast::Update -- CheckSignature() failed, masternode=%s\n", outpoint.ToStringShort());
        return false;
    }
    ...
    if (!pmn->IsBroadcastedWithin(Params().GetConsensus().nMasternodeMinMnbSeconds) || (fMasternodeMode && pubKeyMasternode == activeMasternode.pubKeyMasternode)) {
        ...
        LogPrintG(BCLogLevel::DEBUG, BCLog::MN, "[Masternodes] CMasternodeBroadcast::Update -- Got UPDATED Masternode entry: addr=%s\n", addr.ToString());
...
bool CMasternodeBroadcast::CheckOutpoint(int& nDos)
{
    ...
    if (err == COLLATERAL_UTXO_NOT_FOUND) {
        LogPrintG(BCLogLevel::DEBUG, BCLog::MN, "[Masternodes] CMasternodeBroadcast::CheckOutpoint -- Failed to find Masternode UTXO, masternode=%s\n", outpoint.ToStringShort());
        return false;
    }

    if (err == COLLATERAL_INVALID_AMOUNT) {
        LogPrintG(BCLogLevel::DEBUG, BCLog::MN, "[Masternodes] CMasternodeBroadcast::CheckOutpoint -- Masternode UTXO should have 1000 DASH, masternode=%s\n", outpoint.ToStringShort());
        nDos = 33;
        return false;
    }

    if (err == COLLATERAL_INVALID_PUBKEY) {
        LogPrintG(BCLogLevel::DEBUG, BCLog::MN, "[Masternodes] CMasternodeBroadcast::CheckOutpoint -- Masternode UTXO should match pubKeyCollateralAddress, masternode=%s\n", outpoint.ToStringShort());
        nDos = 33;
        return false;
    }

    if (chainActive.Height() - nHeight + 1 < Params().GetConsensus().nMasternodeMinimumConfirmations) {
        LogPrintG(BCLogLevel::DEBUG, BCLog::MN, "[Masternodes] CMasternodeBroadcast::CheckOutpoint -- Masternode UTXO must have at least %d confirmations, masternode=%s\n",
    ...
    LogPrintG(BCLogLevel::DEBUG, BCLog::MN, "[Masternodes] CMasternodeBroadcast::CheckOutpoint -- Masternode UTXO verified\n");
    ...
    if (pRequiredConfIndex->GetBlockTime() > sigTime) {
        LogPrintG(BCLogLevel::DEBUG, BCLog::MN, "[Masternodes] CMasternodeBroadcast::CheckOutpoint -- Bad sigTime %d (%d conf block is at %d) for Masternode %s %s\n",
    ...
    if (!CheckSignature(nDos)) {
        LogPrintG(BCLogLevel::DEBUG, BCLog::MN, "[Masternodes] CMasternodeBroadcast::CheckOutpoint -- CheckSignature() failed, masternode=%s\n", outpoint.ToStringShort());
        return false;
    }
...
bool CMasternodeBroadcast::Sign(const CKey& keyCollateralAddress)
{
    ...
    if (chainActive.Height() > Params().GetConsensus().nMasternodeSignHashThreshold) {
        ...
        if (!CHashSigner::SignHash(hash, keyCollateralAddress, vchSig)) {
            LogPrintG(BCLogLevel::DEBUG, BCLog::MN, "[Masternodes] CMasternodeBroadcast::Sign -- SignHash() failed\n");
            return false;
        }
        if (!CHashSigner::VerifyHash(hash, pubKeyCollateralAddress, vchSig, strError)) {
            LogPrintG(BCLogLevel::DEBUG, BCLog::MN, "[Masternodes] CMasternodeBroadcast::Sign -- VerifyMessage() failed, error: %s\n", strError);
            return false;
        }
    } else {
        ...
        if (!CMessageSigner::SignMessage(strMessage, vchSig, keyCollateralAddress)) {
            LogPrintG(BCLogLevel::DEBUG, BCLog::MN, "[Masternodes] CMasternodeBroadcast::Sign -- SignMessage() failed\n");
            return false;
        }
    ...
    if (chainActive.Height() > Params().GetConsensus().nMasternodeSignHashThreshold) {
        ...
        if (!CHashSigner::VerifyHash(hash, pubKeyCollateralAddress, vchSig, strError)) {
            ...
            if (!CMessageSigner::VerifyMessage(pubKeyCollateralAddress, vchSig, strMessage, strError)){
                ...
                LogPrintG(BCLogLevel::DEBUG, BCLog::MN, "[Masternodes] CMasternodeBroadcast::CheckSignature -- Got bad Masternode announce signature, error: %s\n", strError);
                nDos = 100;
                return false;
            }
        }
...
void CMasternodeBroadcast::Relay(CConnman& connman) const
{
    ...
    if (!masternodeSync.IsSynced()) {
        LogPrintG(BCLogLevel::DEBUG, BCLog::MN, "[Masternodes] CMasternodeBroadcast::Relay -- won't relay until fully synced\n");
        return;
    }
...
bool CMasternodePing::Sign(const CKey& keyMasternode, const CPubKey& pubKeyMasternode)
{
    ...
    if (chainActive.Height() > Params().GetConsensus().nMasternodeSignHashThreshold) {
        ...
        if (!CHashSigner::SignHash(hash, keyMasternode, vchSig)) {
            LogPrintG(BCLogLevel::DEBUG, BCLog::MN, "[Masternodes] CMasternodePing::Sign -- SignHash() failed\n");
            return false;
        }

        if (!CHashSigner::VerifyHash(hash, pubKeyMasternode, vchSig, strError)) {
            LogPrintG(BCLogLevel::DEBUG, BCLog::MN, "[Masternodes] CMasternodePing::Sign -- VerifyHash() failed, error: %s\n", strError);
            return false;
        }
    } else {
        ...
        if (!CMessageSigner::SignMessage(strMessage, vchSig, keyMasternode)) {
            LogPrintG(BCLogLevel::DEBUG, BCLog::MN, "[Masternodes] CMasternodePing::Sign -- SignMessage() failed\n");
            return false;
        }

        if (!CMessageSigner::VerifyMessage(pubKeyMasternode, vchSig, strMessage, strError)) {
            LogPrintG(BCLogLevel::DEBUG, BCLog::MN, "[Masternodes] CMasternodePing::Sign -- VerifyMessage() failed, error: %s\n", strError);
            return false;
        }
    }
...
bool CMasternodePing::CheckSignature(const CPubKey& pubKeyMasternode, int &nDos) const
{
    ...
    if (chainActive.Height() > Params().GetConsensus().nMasternodeSignHashThreshold) {
        ...
        if (!CHashSigner::VerifyHash(hash, pubKeyMasternode, vchSig, strError)) {
            ...
            if (!CMessageSigner::VerifyMessage(pubKeyMasternode, vchSig, strMessage, strError)) {
                LogPrintG(BCLogLevel::DEBUG, BCLog::MN, "[Masternodes] CMasternodePing::CheckSignature -- Got bad Masternode ping signature, masternode=%s, error: %s\n", masternodeOutpoint.ToStringShort(), strError);
                nDos = 33;
                return false;
            }
        }
...
bool CMasternodePing::SimpleCheck(int& nDos)
{
    ...
    if (sigTime > GetAdjustedTime() + 60 * 60) {
        LogPrintG(BCLogLevel::DEBUG, BCLog::MN, "[Masternodes] CMasternodePing::SimpleCheck -- Signature rejected, too far into the future, masternode=%s\n", masternodeOutpoint.ToStringShort());
        nDos = 1;
        return false;
    }

    {
        ...
        if (mi == mapBlockIndex.end()) {
            LogPrintG(BCLogLevel::DEBUG, BCLog::MN, "[Masternodes] CMasternodePing::SimpleCheck -- Masternode ping is invalid, unknown block hash: masternode=%s blockHash=%s\n", masternodeOutpoint.ToStringShort(), blockHash.ToString());
            // maybe we stuck or forked so we shouldn't ban this node, just fail to accept this ping
            // TODO: or should we also request this block?
            return false;
        }
    }
    LogPrintG(BCLogLevel::DEBUG, BCLog::MN, "[Masternodes] CMasternodePing::SimpleCheck -- Masternode ping verified: masternode=%s  blockHash=%s  sigTime=%d\n", masternodeOutpoint.ToStringShort(), blockHash.ToString(), sigTime);
    return true;
}
...
bool CMasternodePing::CheckAndUpdate(CMasternode* pmn, bool fFromNewBroadcast, int& nDos, CConnman& connman)
{
    ...
    if (pmn == NULL) {
        LogPrintG(BCLogLevel::DEBUG, BCLog::MN, "[Masternodes] CMasternodePing::CheckAndUpdate -- Couldn't find Masternode entry, masternode=%s\n", masternodeOutpoint.ToStringShort());
        return false;
    }

    if (!fFromNewBroadcast) {
        if (pmn->IsUpdateRequired()) {
            LogPrintG(BCLogLevel::DEBUG, BCLog::MN, "[Masternodes] CMasternodePing::CheckAndUpdate -- masternode protocol is outdated, masternode=%s\n", masternodeOutpoint.ToStringShort());
            return false;
        }

        if (pmn->IsNewStartRequired()) {
            LogPrintG(BCLogLevel::DEBUG, BCLog::MN, "[Masternodes] CMasternodePing::CheckAndUpdate -- masternode is completely expired, new start is required, masternode=%s\n", masternodeOutpoint.ToStringShort());
            return false;
        }
    }

    {
        ...
        if ((*mi).second && (*mi).second->nHeight < chainActive.Height() - 24) {
            LogPrintG(BCLogLevel::DEBUG, BCLog::MN, "[Masternodes] CMasternodePing::CheckAndUpdate -- Masternode ping is invalid, block hash is too old: masternode=%s  blockHash=%s\n", masternodeOutpoint.ToStringShort(), blockHash.ToString());
            // nDos = 1;
            return false;
        }
    }

    LogPrintG(BCLogLevel::DEBUG, BCLog::MN, "[Masternodes] CMasternodePing::CheckAndUpdate -- New ping: masternode=%s  blockHash=%s  sigTime=%d\n", masternodeOutpoint.ToStringShort(), blockHash.ToString(), sigTime);

    //LogPrintG(BCLogLevel::DEBUG, BCLog::MN, "[Masternodes] mnping - Found corresponding mn for outpoint: %s\n", masternodeOutpoint.ToStringShort());
    ...
    if (pmn->IsPingedWithin(Params().GetConsensus().nMasternodeMinMnpSeconds - 60, sigTime)) {
        LogPrintG(BCLogLevel::DEBUG, BCLog::MN, "[Masternodes] CMasternodePing::CheckAndUpdate -- Masternode ping arrived too early, masternode=%s\n", masternodeOutpoint.ToStringShort());
    ...
    if (!masternodeSync.IsMasternodeListSynced() && !pmn->IsPingedWithin(Params().GetConsensus().nMasternodeExpirationSeconds/2)) {
        ...
        LogPrintG(BCLogLevel::DEBUG, BCLog::MN, "[Masternodes] CMasternodePing::CheckAndUpdate -- bumping sync timeout, masternode=%s\n", masternodeOutpoint.ToStringShort());
        masternodeSync.BumpAssetLastTime("CMasternodePing::CheckAndUpdate");
    }

    // let's store this ping as the last one
    LogPrintG(BCLogLevel::DEBUG, BCLog::MN, "[Masternodes] CMasternodePing::CheckAndUpdate -- Masternode ping accepted, masternode=%s\n", masternodeOutpoint.ToStringShort());
    pmn->lastPing = *this;
    ...
    LogPrintG(BCLogLevel::DEBUG, BCLog::MN, "[Masternodes] CMasternodePing::CheckAndUpdate -- Masternode ping acceepted and relayed, masternode=%s\n", masternodeOutpoint.ToStringShort());
    Relay(connman);
...
void CMasternodePing::Relay(CConnman& connman)
{
    // Do not relay until fully synced
    if (!masternodeSync.IsSynced()) {
        LogPrintG(BCLogLevel::DEBUG, BCLog::MN, "[Masternodes] CMasternodePing::Relay -- won't relay until fully synced\n");
        return;
    }
