...
bool CGovernanceObject::Sign(const CKey& keyMasternode, const CPubKey& pubKeyMasternode)
{
    ...
    if (chainActive.Height() > Params().GetConsensus().nMasternodeSignHashThreshold) {
        ...
        if (!CHashSigner::SignHash(hash, keyMasternode, vchSig)) {
            LogPrintG(BCLogLevel::LOG_DEBUG, BCLog::GOV, "[Governance] CGovernanceObject::Sign -- SignHash() failed\n");
            return false;
        }

        if (!CHashSigner::VerifyHash(hash, pubKeyMasternode, vchSig, strError)) {
            LogPrintG(BCLogLevel::LOG_DEBUG, BCLog::GOV, "[Governance] CGovernanceObject::Sign -- VerifyHash() failed, error: %s\n", strError);
            return false;
        }
    } else {
        std::string strMessage = GetSignatureMessage();
        if (!CMessageSigner::SignMessage(strMessage, vchSig, keyMasternode)) {
            LogPrintG(BCLogLevel::LOG_DEBUG, BCLog::GOV, "[Governance] CGovernanceObject::Sign -- SignMessage() failed\n");
            return false;
        }

        if (!CMessageSigner::VerifyMessage(pubKeyMasternode, vchSig, strMessage, strError)) {
            LogPrintG(BCLogLevel::LOG_DEBUG, BCLog::GOV, "[Governance] CGovernanceObject::Sign -- VerifyMessage() failed, error: %s\n", strError);
            return false;
        }
    }

    LogPrintG(BCLogLevel::LOG_DEBUG, BCLog::GOV, "[Governance] CGovernanceObject::Sign -- pubkey id = %s, masternode = %s\n",
    ...
    if (chainActive.Height() > Params().GetConsensus().nMasternodeSignHashThreshold) {
        ...
        if (!CHashSigner::VerifyHash(hash, pubKeyMasternode, vchSig, strError)) {
            ...
            if (!CMessageSigner::VerifyMessage(pubKeyMasternode, vchSig, strMessage, strError)) {
                ...
                LogPrintG(BCLogLevel::LOG_DEBUG, BCLog::GOV, "[Governance] CGovernance::CheckSignature -- VerifyMessage() failed, error: %s\n", strError);
                return false;
            }
        } else {
            std::string strMessage = GetSignatureMessage();
...
bool CGovernanceObject::IsCollateralValid(std::string& strError, bool& fMissingConfirmations) const
{
    ...
    if (!GetTransaction(nCollateralHash, txCollateral, Params().GetConsensus(), nBlockHash, true)){
        ...
        LogPrintG(BCLogLevel::LOG_DEBUG, BCLog::GOV, "[Governance] CGovernanceObject::IsCollateralValid -- %s\n", strError);
        return false;
    }

    if (nBlockHash == uint256()) {
        //strError = strprintf("Collateral tx %s is not mined yet", txCollateral->ToString());
        LogPrintG(BCLogLevel::LOG_DEBUG, BCLog::GOV, "[Governance] CGovernanceObject::IsCollateralValid\n");
        return false;
    }
...
void CGovernanceObject::Relay(CConnman& connman)
{
    ...
    if (!masternodeSync.IsSynced()) {
        LogPrintG(BCLogLevel::LOG_DEBUG, BCLog::GOV, "[Governance] CGovernanceObject::Relay -- won't relay until fully synced\n");
        return;
    }
...
void CGovernanceObject::CheckOrphanVotes(CConnman& connman)
{
    ...
    while(it != listVotes.end()) {
        ...
        if (!ProcessVote(NULL, vote, exception, connman)) {
            LogPrintG(BCLogLevel::LOG_DEBUG, BCLog::GOV, "[Governance] CGovernanceObject::CheckOrphanVotes -- Failed to add orphan vote: %s\n", exception.what());
        }
        else {
            vote.Relay(connman);
            fRemove = true;
