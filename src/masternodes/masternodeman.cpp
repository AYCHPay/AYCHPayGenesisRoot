...
bool CMasternodeMan::Add(CMasternode &mn)
{
    ...
    if (Has(mn.outpoint))
    {
        LogPrintG(BCLogLevel::LOG_DEBUG, BCLog::MN, "[Masternodes] CMasternodeMan::Add -- NOT Adding new Masternode (already exists): addr=%s, %i now\n", mn.addr.ToString(), size());
        return false;
    } 

    LogPrintG(BCLogLevel::LOG_DEBUG, BCLog::MN, "[Masternodes] CMasternodeMan::Add -- Adding new Masternode: addr=%s, %i now\n", mn.addr.ToString(), size() + 1);
...
void CMasternodeMan::AskForMN(CNode* pnode, const COutPoint& outpoint, CConnman& connman)
{
    if (!pnode)
    {
        LogPrintG(BCLogLevel::LOG_DEBUG, BCLog::MN, "[Masternodes] CMasternodeMan::AskForMN -- pnode is invalid \n");
        return;
    } 
    ...
    if (it1 != mWeAskedForMasternodeListEntry.end()) {
        ...
        if (it2 != it1->second.end()) {
            ...
            LogPrintG(BCLogLevel::LOG_DEBUG, BCLog::MN, "[Masternodes] CMasternodeMan::AskForMN -- Asking same peer %s for missing masternode entry again: %s\n", addrSquashed.ToString(), outpoint.ToStringShort());
        } else {
...
bool CMasternodeMan::PoSeBan(const COutPoint &outpoint)
{
    ...
    if (!pmn) {
        LogPrintG(BCLogLevel::LOG_DEBUG, BCLog::MN, "[Masternodes] CMasternodeMan::PoSeBan -- Masternode not found \n");
        return false;
    }
    pmn->PoSeBan();
...
void CMasternodeMan::Check()
{
    ...
    LogPrintG(BCLogLevel::LOG_DEBUG, BCLog::MN, "[Masternodes] CMasternodeMan::Check -- nLastSentinelPingTime=%d, IsSentinelPingActive()=%d\n", nLastSentinelPingTime, IsSentinelPingActive());
...
void CMasternodeMan::CheckAndRemove(CConnman& connman)
{
    ...
    {
        LogPrintG(BCLogLevel::LOG_DEBUG, BCLog::MN, "[Masternodes] CMasternodeMan::CheckAndRemove -- Masternode list is not synced \n");
        return;
    } 

    {
        ...
        while (it != mapMasternodes.end()) {
            ...
            if (it->second.IsOutpointSpent()) {
                LogPrintG(BCLogLevel::LOG_DEBUG, BCLog::MN, "[Masternodes] CMasternodeMan::CheckAndRemove -- Removing Masternode: %s  addr=%s  %i now\n", it->second.GetStateString(), it->second.addr.ToString(), size() - 1);
                ...
                if (fAsk) {
                    ...
                    for(int i = 0; setRequested.size() < MNB_RECOVERY_QUORUM_TOTAL && i < (int)vecMasternodeRanks.size(); i++) {
                        ...
                        { 
                            LogPrintG(BCLogLevel::LOG_DEBUG, BCLog::MN, "[Masternodes] CMasternodeMan::CheckAndRemove -- Avoiding banning, masternode=%s\n", it->first.ToStringShort());
                            continue; 
                        }
                    ...
                    if (fAskedForMnbRecovery) {
                        LogPrintG(BCLogLevel::LOG_DEBUG, BCLog::MN, "[Masternodes] CMasternodeMan::CheckAndRemove -- Recovery initiated, masternode=%s\n", it->first.ToStringShort());
                        nAskForMnbRecovery--;
                    }
        ...
        LogPrintG(BCLogLevel::LOG_DEBUG, BCLog::MN, "[Masternodes] CMasternodeMan::CheckAndRemove -- mMnbRecoveryGoodReplies size=%d\n", (int)mMnbRecoveryGoodReplies.size());
        ...
        while(itMnbReplies != mMnbRecoveryGoodReplies.end()){
            if (mMnbRecoveryRequests[itMnbReplies->first].first < GetTime()) {
                ...
                if (itMnbReplies->second.size() >= MNB_RECOVERY_QUORUM_REQUIRED) {
                    ...
                    LogPrintG(BCLogLevel::LOG_DEBUG, BCLog::MN, "[Masternodes] CMasternodeMan::CheckAndRemove -- reprocessing mnb, masternode=%s\n", itMnbReplies->second[0].outpoint.ToStringShort());
                ...
                LogPrintG(BCLogLevel::LOG_DEBUG, BCLog::MN, "[Masternodes] CMasternodeMan::CheckAndRemove -- removing mnb recovery reply, masternode=%s, size=%d\n", itMnbReplies->second[0].outpoint.ToStringShort(), (int)itMnbReplies->second.size());
    ...
    }
    {
        ...
        while(it4 != mapSeenMasternodePing.end()){
            if ((*it4).second.IsExpired()) {
                LogPrintG(BCLogLevel::LOG_DEBUG, BCLog::MN, "[Masternodes] CMasternodeMan::CheckAndRemove -- Removing expired Masternode ping: hash=%s\n", (*it4).second.GetHash().ToString());
        ...
        while(itv2 != mapSeenMasternodeVerification.end()){
            if ((*itv2).second.nBlockHeight < nCachedBlockHeight - MAX_POSE_BLOCKS){
                LogPrintG(BCLogLevel::LOG_DEBUG, BCLog::MN, "[Masternodes] CMasternodeMan::CheckAndRemove -- Removing expired Masternode verification: hash=%s\n", (*itv2).first.ToString());
...
int CMasternodeMan::CountMasternodes(int nProtocolVersion)
{
    ...
    for (const auto& mnpair : mapMasternodes) {
        ...
        { 
            LogPrintG(BCLogLevel::LOG_DEBUG, BCLog::MN, "[Masternodes] CMasternodeMan::CountMasternodes -- Skip (Protocol version too low)\n");
            continue; 
        }
        nCount++;
    }
...
void CMasternodeMan::DsegUpdate(CNode* pnode, CConnman& connman)
{
    ...
    if (Params().NetworkIDString() == CBaseChainParams::MAIN) {
        if (!(pnode->addr.IsRFC1918() || pnode->addr.IsLocal())) {
            ...
            if (it != mWeAskedForMasternodeList.end() && GetTime() < (*it).second) {
                LogPrintG(BCLogLevel::LOG_DEBUG, BCLog::MN, "[Masternodes] CMasternodeMan::DsegUpdate -- we already asked %s for the list; skipping...\n", addrSquashed.ToString());
                return;
            }
        }
    }
    ...
    LogPrintG(BCLogLevel::LOG_DEBUG, BCLog::MN, "[Masternodes] CMasternodeMan::DsegUpdate -- asked %s for the list\n", pnode->addr.ToString());
    LogPrintG(BCLogLevel::LOG_DEBUG, BCLog::MN, "[Masternodes] CMasternodeMan::DsegUpdate -- %s", pnode->addr.ToString());
}
...
bool CMasternodeMan::GetNextMasternodesInQueueForPayment(int nBlockHeight, bool fFilterSigTime, int& nCountRet, masternode_info_t& mnInfoRet, std::vector<masternode_info_t>& vSecondaryMnInfoRet)
{
    ...
    if (!masternodeSync.IsWinnersListSynced()) {
        ...
        LogPrintG(BCLogLevel::LOG_DEBUG, BCLog::MN, "[Masternodes] CMasternodeMan::GetNextMasternodesInQueueForPayment -- Skip (Winners list not synced)\n");
        return false;
    }
    ...
    */
    ...
    for (const auto& mnpair : mapMasternodes) {
        ...
        { 
            //LogPrintG(BCLogLevel::LOG_DEBUG, BCLog::MN, "[Masternodes] \n");
        ...
        //         LogPrintG(BCLogLevel::LOG_DEBUG, BCLog::MN, "[Masternodes] CMasternodePayments::FillBlockPayees -- Primary payee nTimeLastPaidPrimary is irrationally zero \n");
        //         continue; 
        //     }
        // }
    ...
    for (const auto& mnpair : mapMasternodes) {
        ...
        // if (mnpayments.IsScheduled(mnpair.second, nBlockHeight))
        // { 
        //     LogPrintG(BCLogLevel::LOG_DEBUG, BCLog::MN, "[Masternodes] CMasternodeMan::GetNextMasternodesInQueueForPayment secondary -- Skip (Scheduled for payment) \n");
        //     continue; 
        // }
    ...
    if (!GetBlockHash(blockHash, nBlockHeight - (Params().GetConsensus().nCoinbaseMaturity + 1))) {
        LogPrintG(BCLogLevel::LOG_DEBUG, BCLog::MN, "[Masternodes] CMasternode::GetNextMasternodesInQueueForPayment -- ERROR: GetBlockHash() failed at nBlockHeight %d\n", nBlockHeight - (Params().GetConsensus().nCoinbaseMaturity + 1));
        return false;
    }
...
masternode_info_t CMasternodeMan::FindRandomNotInVec(const std::vector<COutPoint> &vecToExclude, int nProtocolVersion)
{
    ...
    LogPrintG(BCLogLevel::LOG_DEBUG, BCLog::MN, "[Masternodes] CMasternodeMan::FindRandomNotInVec -- %d enabled masternodes, %d masternodes to choose from\n", nCountEnabled, nCountNotExcluded);
    ...
    for (const auto& pmn : vpMasternodesShuffled) {
        ...
        {
            LogPrintG(BCLogLevel::LOG_DEBUG, BCLog::MN, "[Masternodes] CMasternodeMan::FindRandomNotInVec -- Skip (Protocol Version Too Low) \n");
            continue; 
        }
        else if (!pmn->IsEnabled())
        { 
            LogPrintG(BCLogLevel::LOG_DEBUG, BCLog::MN, "[Masternodes] CMasternodeMan::FindRandomNotInVec -- Skip (Not enabled) \n");
            continue; 
        }
        fExclude = false;
        ...
        if (fExclude)
        { 
            LogPrintG(BCLogLevel::LOG_DEBUG, BCLog::MN, "[Masternodes] CMasternodeMan::FindRandomNotInVec -- Skip (Excluded) \n");
            continue; 
        }

        // found the one not in vecToExclude
        LogPrintG(BCLogLevel::LOG_DEBUG, BCLog::MN, "[Masternodes] CMasternodeMan::FindRandomNotInVec -- found, masternode=%s\n", pmn->outpoint.ToStringShort());
        return pmn->GetInfo();
    }

    LogPrintG(BCLogLevel::LOG_DEBUG, BCLog::MN, "[Masternodes] CMasternodeMan::FindRandomNotInVec -- failed\n");
...
bool CMasternodeMan::GetMasternodeScores(const uint256& nBlockHash, CMasternodeMan::score_pair_vec_t& vecMasternodeScoresRet, int nMinProtocol)
{
    ...
    if (!masternodeSync.IsMasternodeListSynced())
    {
        LogPrintG(BCLogLevel::LOG_DEBUG, BCLog::MN, "[Masternodes] CMasternodeMan::GetMasternodeScores -- Skip (Masternode list not synced)\n");
        return false;
    }
    ...
    {
        LogPrintG(BCLogLevel::LOG_DEBUG, BCLog::MN, "[Masternodes] CMasternodeMan::GetMasternodeScores -- Skip (Masternode list empty)\n");
        return false;
    }
...
bool CMasternodeMan::GetMasternodeRank(const COutPoint& outpoint, int& nRankRet, int nBlockHeight, int nMinProtocol)
{
    ...
    if (!masternodeSync.IsMasternodeListSynced())
    {
        LogPrintG(BCLogLevel::LOG_DEBUG, BCLog::MN, "[Masternodes] CMasternodeMan::GetMasternodeRank -- Skip (Masternode list not synced)\n");
        return false;
    }
    ...
    if (!GetBlockHash(nBlockHash, nBlockHeight)) {
        LogPrintG(BCLogLevel::LOG_DEBUG, BCLog::MN, "[Masternodes] CMasternodeMan::%s -- ERROR: GetBlockHash() failed at nBlockHeight %d\n", __func__, nBlockHeight);
        return false;
    }
    ...
    {
        LogPrintG(BCLogLevel::LOG_DEBUG, BCLog::MN, "[Masternodes] CMasternodeMan::GetMasternodeRank -- Skip (Unable to get masternode scores)\n");
        return false;
    }
...
bool CMasternodeMan::GetMasternodeRanks(CMasternodeMan::rank_pair_vec_t& vecMasternodeRanksRet, int nBlockHeight, int nMinProtocol)
{
    ...
    if (!masternodeSync.IsMasternodeListSynced())
    {
        LogPrintG(BCLogLevel::LOG_DEBUG, BCLog::MN, "[Masternodes] CMasternodeMan::GetMasternodeRanks -- Skip (Masternode list not synced)\n");
        return false;
    }
    // make sure we know about this block
    uint256 nBlockHash = uint256();
    if (!GetBlockHash(nBlockHash, nBlockHeight)) {
        LogPrintG(BCLogLevel::LOG_DEBUG, BCLog::MN, "[Masternodes] CMasternodeMan::GetMasternodeRanks %s -- ERROR: GetBlockHash() failed at nBlockHeight %d\n", __func__, nBlockHeight);
        return false;
    }
    ...
    if (!GetMasternodeScores(nBlockHash, vecMasternodeScores, nMinProtocol))
    {
        LogPrintG(BCLogLevel::LOG_DEBUG, BCLog::MN, "[Masternodes] CMasternodeMan::GetMasternodeRanks -- Skip (Unable to get masternode scores)\n");
        return false;
    }
...
void CMasternodeMan::ProcessMasternodeConnections(CConnman& connman)
{
    ...
    {
        LogPrintG(BCLogLevel::LOG_DEBUG, BCLog::MN, "[Masternodes] CMasternodeMan::ProcessMasternodeConnections -- Skipped (RegTest) \n");
        return;
    }
    ...
    connman.ForEachNode(CConnman::AllNodes, [](CNode* pnode) {
        ...
        if (pnode->fMasternode) {
#endif // ENABLE_WALLET
            LogPrintG(BCLogLevel::LOG_DEBUG, BCLog::MN, "[Masternodes] Closing Masternode connection: peer=%d, addr=%s\n", pnode->GetId(), pnode->addr.ToString());
...
void CMasternodeMan::ProcessPendingMnbRequests(CConnman& connman)
{
    ...
    while (itPendingMNB != mapPendingMNB.end()) {
        ...
        if (fDone || (GetTime() - nTimeAdded > 15)) {
            if (!fDone) {
                LogPrintG(BCLogLevel::LOG_DEBUG, BCLog::MN, "[Masternodes] CMasternodeMan::%s -- failed to connect to %s\n", __func__, itPendingMNB->first.ToString());
            }
...
{  
    ...
    if (strCommand == NetMsgType::MNANNOUNCE) { //Masternode Broadcast
        ...
        {
            LogPrintG(BCLogLevel::LOG_DEBUG, BCLog::MN, "[Masternodes] CMasternodeMan::ProcessMessage (MNANNOUNCE) -- Skipped (Blockchain not synced) \n");
            return;
        }
        ...
        if (!masternodeSync.IsBlockchainSynced())
        {
            LogPrintG(BCLogLevel::LOG_DEBUG, BCLog::MN, "[Masternodes] CMasternodeMan::ProcessMessage (MNPING) -- Skipped (Blockchain not synced) \n");
            return;
        }
        ...
        {
            LogPrintG(BCLogLevel::LOG_DEBUG, BCLog::MN, "[Masternodes] CMasternodeMan::ProcessMessage (MNPING) -- Skipped (Seen) \n");
            return;
        } //seen
        ...
        {
            LogPrintG(BCLogLevel::LOG_DEBUG, BCLog::MN, "[Masternodes] CMasternodeMan::ProcessMessage (MNPING) -- Skipped (Too late, new MNANNOUNCE required) \n");
            return;
        }
        ...
        if (nDos > 0) {
            ...
            LogPrintG(BCLogLevel::LOG_DEBUG, BCLog::MN, "[Masternodes] CMasternodeMan::ProcessMessage (MNPING) -- Node is acting suspicious \n");
            Misbehaving(pfrom->GetId(), nDos);
        } else if (pmn != NULL) {
...
void CMasternodeMan::SyncAll(CNode* pnode, CConnman& connman)
{
    ...
    for (const auto& mnpair : mapMasternodes) {
        ...
        { 
            LogPrintG(BCLogLevel::LOG_DEBUG, BCLog::MN, "[Masternodes] CMasternodeMan::SyncAll -- Skip (IsRFC1918) \n");
            continue; 
        } 
        else if (mnpair.second.addr.IsLocal())
        { 
            LogPrintG(BCLogLevel::LOG_DEBUG, BCLog::MN, "[Masternodes] CMasternodeMan::SyncAll -- Skip (IsLocal) \n");
            continue; 
        } // do not send local network masternode
        // NOTE: send masternode regardless of its current state, the other node will need it to verify old votes.
...
void CMasternodeMan::DoFullVerificationStep(CConnman& connman)
{
    ...
    {
        LogPrintG(BCLogLevel::LOG_DEBUG, BCLog::MN, "[Masternodes] CMasternodeMan::DoFullVerificationStep -- Skipped (Outpoint is Null) \n");
        return;
    }
    if (!masternodeSync.IsSynced())
    {
    ...
    while(it != vecMasternodeRanks.end()) {
        if (it->first > MAX_POSE_RANK) {
            LogPrintG(BCLogLevel::LOG_DEBUG, BCLog::MN, "[Masternodes] CMasternodeMan::DoFullVerificationStep -- Must be in top %d to send verify request\n",
                        (int)MAX_POSE_RANK);
            return;
        }
        if (it->second.outpoint == activeMasternode.outpoint) {
            nMyRank = it->first;
            LogPrintG(BCLogLevel::LOG_DEBUG, BCLog::MN, "[Masternodes] CMasternodeMan::DoFullVerificationStep -- Found self at rank %d/%d, verifying up to %d masternodes\n",
    ...
    while(it != vecMasternodeRanks.end()) {
        if (it->second.IsPoSeVerified() || it->second.IsPoSeBanned()) {
            LogPrintG(BCLogLevel::LOG_DEBUG, BCLog::MN, "[Masternodes] CMasternodeMan::DoFullVerificationStep -- Already %s%s%s masternode %s address %s, skipping...\n",
        ...
        LogPrintG(BCLogLevel::LOG_DEBUG, BCLog::MN, "[Masternodes] CMasternodeMan::DoFullVerificationStep -- Verifying masternode %s rank %d/%d address %s\n",
        ...
        for (const auto& pmn : vSortedByAddr) {
            ...
            if (pmn->addr == pprevMasternode->addr) {
                if (pverifiedMasternode) {
                    ...
                    LogPrintG(BCLogLevel::LOG_DEBUG, BCLog::MN, "[Masternodes] CMasternodeMan::CheckSameAddr -- Another masternode with the same ip is verified, ban this one \n");
                    vBan.push_back(pmn);
...
bool CMasternodeMan::SendVerifyRequest(const CAddress& addr, const std::vector<const CMasternode*>& vSortedByAddr, CConnman& connman)
{
    if (netfulfilledman.HasFulfilledRequest(addr, strprintf("%s", NetMsgType::MNVERIFY)+"-request")) {
        ...
        LogPrintG(BCLogLevel::LOG_DEBUG, BCLog::MN, "[Masternodes] CMasternodeMan::SendVerifyRequest -- too many requests, skipping... addr=%s\n", addr.ToString());
        return false;
    }
    ...
    LogPrintG(BCLogLevel::LOG_DEBUG, BCLog::MN, "[Masternodes] CMasternodeMan::SendVerifyRequest -- verifying node using nonce %d addr=%s\n", mnv.nonce, addr.ToString());
...
void CMasternodeMan::ProcessPendingMnvRequests(CConnman& connman)
{
    ...
    while (itPendingMNV != mapPendingMNV.end()) {
        bool fDone = connman.ForNode(itPendingMNV->first, [&](CNode* pnode) {
            ...
            LogPrintG(BCLogLevel::LOG_DEBUG, BCLog::MN, "[Masternodes] -- verifying node using nonce %d addr=%s\n", itPendingMNV->second.second.nonce, pnode->addr.ToString());
...
void CMasternodeMan::SendVerifyReply(CNode* pnode, CMasternodeVerification& mnv, CConnman& connman)
{
    ...
    if (!fMasternodeMode) {
        ...
        LogPrintG(BCLogLevel::LOG_DEBUG, BCLog::MN, "[Masternodes] CMasternodeMan::SendVerifyReply -- Only masternodes can sign a verified reply, but I am not a masternode... \n");
        return;
    }

    if (netfulfilledman.HasFulfilledRequest(pnode->addr, strprintf("%s", NetMsgType::MNVERIFY)+"-reply")) {
        // peer should not ask us that often
        LogPrintG(BCLogLevel::LOG_DEBUG, BCLog::MN, "[Masternodes] MasternodeMan::SendVerifyReply -- ERROR: peer already asked me recently, peer=%d\n", pnode->GetId());
    ...
    if (!GetBlockHash(blockHash, mnv.nBlockHeight)) {
        LogPrintG(BCLogLevel::LOG_DEBUG, BCLog::MN, "[Masternodes] MasternodeMan::SendVerifyReply -- can't get block hash for unknown block height %d, peer=%d\n", mnv.nBlockHeight, pnode->GetId());
        return;
    }
    ...
    if (chainActive.Height() > Params().GetConsensus().nMasternodeSignHashThreshold) {
        ...
        if (!CHashSigner::SignHash(hash, activeMasternode.keyMasternode, mnv.vchSig1)) {
            LogPrintG(BCLogLevel::LOG_DEBUG, BCLog::MN, "[Masternodes] CMasternodeMan::SendVerifyReply -- SignHash() failed\n");
            return;
        }
        if (!CHashSigner::VerifyHash(hash, activeMasternode.pubKeyMasternode, mnv.vchSig1, strError)) {
            LogPrintG(BCLogLevel::LOG_DEBUG, BCLog::MN, "[Masternodes] CMasternodeMan::SendVerifyReply -- VerifyHash() failed, error: %s\n", strError);
            return;
        }
    } else {
        ...
        if (!CMessageSigner::SignMessage(strMessage, mnv.vchSig1, activeMasternode.keyMasternode)) {
            LogPrintG(BCLogLevel::LOG_DEBUG, BCLog::MN, "[Masternodes] CMasternodeMan::SendVerifyReply -- SignMessage() failed\n");
            return;
        }

        if (!CMessageSigner::VerifyMessage(activeMasternode.pubKeyMasternode, mnv.vchSig1, strMessage, strError)) {
            LogPrintG(BCLogLevel::LOG_DEBUG, BCLog::MN, "[Masternodes] CMasternodeMan::SendVerifyReply -- VerifyMessage() failed, error: %s\n", strError);
            return;
        }
    }
...
void CMasternodeMan::ProcessVerifyReply(CNode* pnode, CMasternodeVerification& mnv)
{
    ...
    if (!netfulfilledman.HasFulfilledRequest(pnode->addr, strprintf("%s", NetMsgType::MNVERIFY)+"-request")) {
        LogPrintG(BCLogLevel::LOG_DEBUG, BCLog::MN, "[Masternodes] CMasternodeMan::ProcessVerifyReply -- ERROR: we didn't ask for verification of %s, peer=%d\n", pnode->addr.ToString(), pnode->GetId());
        Misbehaving(pnode->GetId(), 20);
        return;
    }
    ...
    if (mWeAskedForVerification[pnode->addr].nonce != mnv.nonce) {
        LogPrintG(BCLogLevel::LOG_DEBUG, BCLog::MN, "[Masternodes] CMasternodeMan::ProcessVerifyReply -- ERROR: wrong nounce: requested=%d, received=%d, peer=%d\n",
    ...
    if (mWeAskedForVerification[pnode->addr].nBlockHeight != mnv.nBlockHeight) {
        LogPrintG(BCLogLevel::LOG_DEBUG, BCLog::MN, "[Masternodes] CMasternodeMan::ProcessVerifyReply -- ERROR: wrong nBlockHeight: requested=%d, received=%d, peer=%d\n",
    ...
    if (!GetBlockHash(blockHash, mnv.nBlockHeight)) {
        ...
        LogPrintG(BCLogLevel::LOG_DEBUG, BCLog::MN, "[Masternodes] MasternodeMan::ProcessVerifyReply -- can't get block hash for unknown block height %d, peer=%d\n", mnv.nBlockHeight, pnode->GetId());
        return;
    }

    // we already verified this address, why node is spamming?
    if (netfulfilledman.HasFulfilledRequest(pnode->addr, strprintf("%s", NetMsgType::MNVERIFY)+"-done")) {
        LogPrintG(BCLogLevel::LOG_DEBUG, BCLog::MN, "[Masternodes] CMasternodeMan::ProcessVerifyReply -- ERROR: already verified %s recently\n", pnode->addr.ToString());
        Misbehaving(pnode->GetId(), 20);
        return;
    }

    {
        ...
        for (auto& mnpair : mapMasternodes) {
            if (CAddress(mnpair.second.addr, NODE_NETWORK) == pnode->addr) {
                ...
                if (fFound) {
                    ...
                    if (activeMasternode.outpoint.IsNull())
                    { 
                        LogPrintG(BCLogLevel::LOG_DEBUG, BCLog::MN, "[Masternodes] CMasternodeMan::ProcessVerifyReply -- Skip (Active masternode outpoint is null) \n");
                        continue; 
                    }
                    // update ...
                    ...
                    if (chainActive.Height() > Params().GetConsensus().nMasternodeSignHashThreshold) {
                        ...
                        if (!CHashSigner::SignHash(hash2, activeMasternode.keyMasternode, mnv.vchSig2)) {
                            LogPrintG(BCLogLevel::LOG_DEBUG, BCLog::MN, "[Masternodes] CMasternodeMan::ProcessVerifyReply -- SignHash() failed\n");
                            return;
                        }

                        if (!CHashSigner::VerifyHash(hash2, activeMasternode.pubKeyMasternode, mnv.vchSig2, strError)) {
                            LogPrintG(BCLogLevel::LOG_DEBUG, BCLog::MN, "[Masternodes] CMasternodeMan::ProcessVerifyReply -- VerifyHash() failed, error: %s\n", strError);
                            return;
                        }
                    } else {
                        ...
                        if (!CMessageSigner::SignMessage(strMessage2, mnv.vchSig2, activeMasternode.keyMasternode)) {
                            LogPrintG(BCLogLevel::LOG_DEBUG, BCLog::MN, "[Masternodes] CMasternodeMan::ProcessVerifyReply -- SignMessage() failed\n");
                            return;
                        }

                        if (!CMessageSigner::VerifyMessage(activeMasternode.pubKeyMasternode, mnv.vchSig2, strMessage2, strError)) {
                            LogPrintG(BCLogLevel::LOG_DEBUG, BCLog::MN, "[Masternodes] CMasternodeMan::ProcessVerifyReply -- VerifyMessage() failed, error: %s\n", strError);
                            return;
                        }
                    }
        ...
        if (!prealMasternode) {
            ...
            LogPrintG(BCLogLevel::LOG_DEBUG, BCLog::MN, "[Masternodes] CMasternodeMan::ProcessVerifyReply -- ERROR: no real masternode found for addr %s\n", pnode->addr.ToString());
            Misbehaving(pnode->GetId(), 20);
            return;
        }
        LogPrintG(BCLogLevel::LOG_DEBUG, BCLog::MN, "[Masternodes] CMasternodeMan::ProcessVerifyReply -- verified real masternode %s for addr %s\n",
...
void CMasternodeMan::ProcessVerifyBroadcast(CNode* pnode, const CMasternodeVerification& mnv)
{
    ...
    if (mapSeenMasternodeVerification.find(mnv.GetHash()) != mapSeenMasternodeVerification.end()) {
        // we already have one
        LogPrintG(BCLogLevel::LOG_DEBUG, BCLog::MN, "[Masternodes] CMasternodeMan::ProcessVerifyBroadcast -- Skip (We already have a verification) \n");
        return;
    }
    ...
    if (mnv.nBlockHeight < nCachedBlockHeight - MAX_POSE_BLOCKS) {
        LogPrintG(BCLogLevel::LOG_DEBUG, BCLog::MN, "[Masternodes] CMasternodeMan::ProcessVerifyBroadcast -- Outdated: current block %d, verification block %d, peer=%d\n",
                    nCachedBlockHeight, mnv.nBlockHeight, pnode->GetId());
        return;
    }

    if (mnv.masternodeOutpoint1 == mnv.masternodeOutpoint2) {
        LogPrintG(BCLogLevel::LOG_DEBUG, BCLog::MN, "[Masternodes] CMasternodeMan::ProcessVerifyBroadcast -- ERROR: same outpoints %s, peer=%d\n",
    ...
    if (!GetBlockHash(blockHash, mnv.nBlockHeight)) {
        // this shouldn't happen...
        LogPrintG(BCLogLevel::LOG_DEBUG, BCLog::MN, "[Masternodes] CMasternodeMan::ProcessVerifyBroadcast -- Can't get block hash for unknown block height %d, peer=%d\n", mnv.nBlockHeight, pnode->GetId());
        return;
    }
    ...
    if (!GetMasternodeRank(mnv.masternodeOutpoint2, nRank, mnv.nBlockHeight, MIN_POSE_PROTO_VERSION)) {
        LogPrintG(BCLogLevel::LOG_DEBUG, BCLog::MN, "[Masternodes] CMasternodeMan::ProcessVerifyBroadcast -- Can't calculate rank for masternode %s\n",
                    mnv.masternodeOutpoint2.ToStringShort());
        return;
    }

    if (nRank > MAX_POSE_RANK) {
        LogPrintG(BCLogLevel::LOG_DEBUG, BCLog::MN, "[Masternodes] CMasternodeMan::ProcessVerifyBroadcast -- Masternode %s is not in top %d, current rank %d, peer=%d\n",
...

    {
        ...
        if (!pmn1) {
            LogPrintG(BCLogLevel::LOG_DEBUG, BCLog::MN, "[Masternodes] CMasternodeMan::ProcessVerifyBroadcast -- can't find masternode1 %s\n", mnv.masternodeOutpoint1.ToStringShort());
            return;
        }

        CMasternode* pmn2 = Find(mnv.masternodeOutpoint2);
        if (!pmn2) {
            LogPrintG(BCLogLevel::LOG_DEBUG, BCLog::MN, "[Masternodes] CMasternodeMan::ProcessVerifyBroadcast -- can't find masternode2 %s\n", mnv.masternodeOutpoint2.ToStringShort());
            return;
        }

        if (pmn1->addr != mnv.addr) {
            LogPrintG(BCLogLevel::LOG_DEBUG, BCLog::MN, "[Masternodes] CMasternodeMan::ProcessVerifyBroadcast -- addr %s does not match %s\n", mnv.addr.ToString(), pmn1->addr.ToString());
            return;
        }
        ...
        if (chainActive.Height() > Params().GetConsensus().nMasternodeSignHashThreshold) {
            ...
            if (!CHashSigner::VerifyHash(hash1, pmn1->pubKeyMasternode, mnv.vchSig1, strError)) {
                LogPrintG(BCLogLevel::LOG_DEBUG, BCLog::MN, "[Masternodes] CMasternodeMan::ProcessVerifyBroadcast -- VerifyHash() failed, error: %s\n", strError);
                return;
            }
            ...
            if (!CMessageSigner::VerifyMessage(pmn1->pubKeyMasternode, mnv.vchSig1, strMessage1, strError)) {
                LogPrintG(BCLogLevel::LOG_DEBUG, BCLog::MN, "[Masternodes] CMasternodeMan::ProcessVerifyBroadcast -- VerifyMessage() for masternode1 failed, error: %s\n", strError);
                return;
            }

            if (!CMessageSigner::VerifyMessage(pmn2->pubKeyMasternode, mnv.vchSig2, strMessage2, strError)) {
                LogPrintG(BCLogLevel::LOG_DEBUG, BCLog::MN, "[Masternodes] CMasternodeMan::ProcessVerifyBroadcast -- VerifyMessage() for masternode2 failed, error: %s\n", strError);
                return;
            }
        }
        ...
        for (auto& mnpair : mapMasternodes) {
            ...
            nCount++;
            LogPrintG(BCLogLevel::LOG_DEBUG, BCLog::MN, "[Masternodes] CMasternodeMan::ProcessVerifyBroadcast -- increased PoSe ban score for %s addr %s, new score %d\n",
                        mnpair.first.ToStringShort(), mnpair.second.addr.ToString(), mnpair.second.nPoSeBanScore);
        }
        if (nCount)
            LogPrintG(BCLogLevel::LOG_DEBUG, BCLog::MN, "[Masternodes] CMasternodeMan::ProcessVerifyBroadcast -- PoSe score increased for %d fake masternodes, addr %s\n",
...
bool CMasternodeMan::CheckMnbAndUpdateMasternodeList(CNode* pfrom, CMasternodeBroadcast mnb, int& nDos, CConnman& connman)
{
...

    {
        ...
        if (!mnb.SimpleCheck(nDos)) {
            LogPrintG(BCLogLevel::LOG_DEBUG, BCLog::MN, "[Masternodes] CMasternodeMan::CheckMnbAndUpdateMasternodeList -- SimpleCheck() failed, masternode=%s\n", mnb.outpoint.ToStringShort());
            return false;
        }
        ...
        if (pmn) {
            ...
            if (!mnb.Update(pmn, nDos, connman)) {
                LogPrintG(BCLogLevel::LOG_DEBUG, BCLog::MN, "[Masternodes] CMasternodeMan::CheckMnbAndUpdateMasternodeList -- Update() failed, masternode=%s\n", mnb.outpoint.ToStringShort());
                return false;
            }
    ...
    if (mnb.CheckOutpoint(nDos)) {
        ...
        if (fMasternodeMode && mnb.pubKeyMasternode == activeMasternode.pubKeyMasternode) {
            ...
            if (mnb.nProtocolVersion == PROTOCOL_VERSION) {
                ...
                LogPrintG(BCLogLevel::LOG_DEBUG, BCLog::MN, "[Masternodes] CMasternodeMan::CheckMnbAndUpdateMasternodeList -- Got NEW Masternode entry: masternode=%s  sigTime=%lld  addr=%s\n",
                            mnb.outpoint.ToStringShort(), mnb.sigTime, mnb.addr.ToString());
                activeMasternode.ManageState(connman);
            } else {
                // ... otherwise we need to reactivate our node, do not add it to the list and do not relay
                // but also do not ban the node we get this message from
                LogPrintG(BCLogLevel::LOG_DEBUG, BCLog::MN, "[Masternodes] CMasternodeMan::CheckMnbAndUpdateMasternodeList -- wrong PROTOCOL_VERSION, re-activate your MN: message nProtocolVersion=%d  PROTOCOL_VERSION=%d\n", mnb.nProtocolVersion, PROTOCOL_VERSION);
                return false;
            }
        }
        mnb.Relay(connman);
    } else {
        LogPrintG(BCLogLevel::LOG_DEBUG, BCLog::MN, "[Masternodes] CMasternodeMan::CheckMnbAndUpdateMasternodeList -- Rejected Masternode entry: %s  addr=%s\n", mnb.outpoint.ToStringShort(), mnb.addr.ToString());
        return false;
    }
...
bool CMasternodeMan::AddGovernanceVote(const COutPoint& outpoint, uint256 nGovernanceObjectHash)
{
    ...
    CMasternode* pmn = Find(outpoint);
    if (!pmn) {
        LogPrintG(BCLogLevel::LOG_DEBUG, BCLog::MN, "[Masternodes] CMasternodeMan::AddGovernanceVote -- Skip (Masternode not found) \n");
        return false;
    }
...
void CMasternodeMan::SetMasternodeLastPing(const COutPoint& outpoint, const CMasternodePing& mnp)
{
    ...
    CMasternode* pmn = Find(outpoint);
    if (!pmn) {
        LogPrintG(BCLogLevel::LOG_DEBUG, BCLog::MN, "[Masternodes] CMasternodeMan::SetMasternodeLastPing -- Skip (Masternode not found) \n");
        return;
    }
    pmn->lastPing = mnp;
...
void CMasternodeMan::UpdatedBlockTip(const CBlockIndex *pindex, bool lock)
{
    ...
    LogPrintG(BCLogLevel::LOG_DEBUG, BCLog::MN, "[Masternodes] CMasternodeMan::UpdatedBlockTip -- nCachedBlockHeight=%d\n", nCachedBlockHeight);
...
void CMasternodeMan::WarnMasternodeDaemonUpdates()
{
    ...
    if (fWarned)
    {
        LogPrintG(BCLogLevel::LOG_DEBUG, BCLog::MN, "[Masternodes] CMasternodeMan::WarnMasternodeDaemonUpdates -- Skip (Warned) \n");
        return;
    }

    if (!size())
    {
        LogPrintG(BCLogLevel::LOG_DEBUG, BCLog::MN, "[Masternodes] CMasternodeMan::WarnMasternodeDaemonUpdates -- Skip (Invalid Size) \n");
        return;
    }

    if (!masternodeSync.IsMasternodeListSynced())
    {
        LogPrintG(BCLogLevel::LOG_DEBUG, BCLog::MN, "[Masternodes] CMasternodeMan::WarnMasternodeDaemonUpdates -- Skip (Masternode list not synced) \n");
        return;
    }
    ...
    {
        LogPrintG(BCLogLevel::LOG_DEBUG, BCLog::MN, "[Masternodes] CMasternodeMan::WarnMasternodeDaemonUpdates -- Skip (Not enough updated masternodes to do a meaningful update) \n");
        return;
    }
