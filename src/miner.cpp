...
std::unique_ptr<CBlockTemplate> BlockAssembler::CreateNewBlock(const CScript& scriptPubKeyIn, bool fMineWitnessTx)
{
    ...
    LogPrintG(BCLogLevel::DEBUG, BCLog::BENCH, "[Benchmarking] CreateNewBlock() packages: %.2fms (%d packages, %d updated descendants), validity: %.2fms (total %.2fms)\n", 0.001 * (nTime1 - nTimeStart), nPackagesSelected, nDescendantsUpdated, 0.001 * (nTime2 - nTime1), 0.001 * (nTime2 - nTimeStart));
...
{
    LogPrintG(BCLogLevel::DEBUG, BCLog::POW, "[ProofOfWork] %s\n", pblock->ToString());
...
void BlockAssembler::AddToBlock(CTxMemPool::txiter iter)
{
    ...
    if (fPrintPriority) {
        LogPrintG(BCLogLevel::DEBUG, BCLog::POW, "[ProofOfWork] fee %s txid %s\n",
...
{
    LogPrintG(BCLogLevel::DEBUG, BCLog::POW, "[ProofOfWork] Genesis Miner started\n");
    ...
    LogPrintG(BCLogLevel::DEBUG, BCLog::POW, "[ProofOfWork] Using Equihash solver \"%s\" with n = %u, k = %u\n", solver, n, k);
    ...
    try 
    {
        while (true) 
        {
            if (chainparams.MiningRequiresPeers()) 
            {
                ...
                //miningTimer.stop();
                LogPrintG(BCLogLevel::DEBUG, BCLog::POW, "[ProofOfWork] Checking for peers before mining commences\n");
                ...
                do 
                {
                    ...
                    {
                        // LogPrintG(BCLogLevel::DEBUG, BCLog::POW, "[ProofOfWork] Waiting for peers...\n");
                    }
                    MilliSleep(1000);
                } while (true);
                //miningTimer.start();
            ...
            {
                LogPrintG(BCLogLevel::DEBUG, BCLog::POW, "[ProofOfWork] Waiting for masternode sync to complete before mining commences\n");
                do 
                {
                    bool isMnStuffSynced = masternodeSync.IsSynced();
                    if (isMnStuffSynced)
                    {
                        LogPrintG(BCLogLevel::DEBUG, BCLog::POW, "[ProofOfWork] Masternode sync has completed. Commencing mining...\n");
                        break;
                    }
                    else
                    {
                        // LogPrintG(BCLogLevel::DEBUG, BCLog::POW, "[ProofOfWork] Masternode sync has still not completed. Can not commence mining...\n");
                    }
            ...
            if (!pblocktemplate.get())
            {
                if (gArgs.GetArg("-mineraddress", "").empty()) 
                {
                    LogPrintG(BCLogLevel::DEBUG, BCLog::POW, "[ProofOfWork] Error in Genesis Miner: Keypool ran out, please call keypoolrefill before restarting the mining thread\n");
                } 
                else 
                {
                    // Should never reach here, because -mineraddress validity is checked in init.cpp
                    LogPrintG(BCLogLevel::DEBUG, BCLog::POW, "[ProofOfWork] Error in Genesis Miner: Invalid -mineraddress\n");
                }
                return;
            }
            ...
            LogPrintG(BCLogLevel::DEBUG, BCLog::POW, "[ProofOfWork] Running Genesis Miner with %u transactions in block (%u bytes)\n", pblock->vtx.size(), ::GetSerializeSize(*pblock, SER_NETWORK, PROTOCOL_VERSION));
            ...
            while (true) 
            {
                ...
                {
                    EhInitialiseState(n, k, state, "SafeCash");
                }
                        ...
                        (std::vector<unsigned char> soln) 
                {
                    ...
                    LogPrintG(BCLogLevel::DEBUG, BCLog::POW, "[ProofOfWork] Genesis Miner:\n");
                    LogPrintG(BCLogLevel::DEBUG, BCLog::POW, "[ProofOfWork] proof-of-work found  \n  hash: %s  \ntarget: %s\n", pblock->GetHash().GetHex(), hashTarget.GetHex());
                ...
                {
                    LogPrintG(BCLogLevel::DEBUG, BCLog::POW, "[ProofOfWork] Equihash solver broke out of the loop, as GetNodeCount is zero and mining requires peers \n");
                    break;
                }
                if ((UintToArith256(pblock->nNonce) & 0xffff) == 0xffff)
                {
                    LogPrintG(BCLogLevel::DEBUG, BCLog::POW, "[ProofOfWork] Equihash solver broke out of the loop \n");
                    break;
                }
                if (mempool.GetTransactionsUpdated() != nTransactionsUpdatedLast && GetTime() - nStart > 60)
                {
                    LogPrintG(BCLogLevel::DEBUG, BCLog::POW, "[ProofOfWork] Equihash solver broke out of the loop, because GetTransactionsUpdated was incorrect \n");
                    break;
                }
                if (pindexPrev != chainActive.Tip())
                {
                    LogPrintG(BCLogLevel::DEBUG, BCLog::POW, "[ProofOfWork] Equihash solver broke out of the loop, because the previous index is not the active tip \n");
                    break;
                }
    ...
    catch (const boost::thread_interrupted&)
    {
        //miningTimer.stop();
        //c.disconnect();
        LogPrintG(BCLogLevel::DEBUG, BCLog::POW, "[ProofOfWork] Genesis Miner thread terminated\n");
        throw;
    }
    catch (const std::runtime_error &e)
    {
        //miningTimer.stop();
        //c.disconnect();
        LogPrintG(BCLogLevel::DEBUG, BCLog::POW, "[ProofOfWork] Genesis Miner runtime error: %s\n", e.what());
        return;
    }
    //miningTimer.stop();
    //c.disconnect();
