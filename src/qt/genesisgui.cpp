...
const std::string GenesisGUI::DEFAULT_UIPLATFORM =
#if defined(Q_OS_MAC)
        "macosx"
...
const QString GenesisGUI::DEFAULT_WALLET = "~Default";

GenesisGUI::GenesisGUI(const PlatformStyle *_platformStyle, const NetworkStyle *networkStyle, QWidget *parent) :
    QMainWindow(parent),
    enableWallet(false),
    clientModel(0),
    walletFrame(0),
...
void GenesisGUI::createActions()
{
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a Genesis address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and genesis: URIs)"));
...
#ifdef ENABLE_WALLET
    QSettings settings;
    bool settingsShowMasternodesTab = settings.value("fShowMasternodesTab").toBool();
    if (!fLiteMode && settingsShowMasternodesTab) {
...
void GenesisGUI::createToolBars()
{
    if (walletFrame)
    {
...
void GenesisGUI::setClientModel(ClientModel *_clientModel)
{
...
#ifdef ENABLE_WALLET
bool GenesisGUI::addWallet(const QString& name, WalletModel *walletModel)
{
    if (!walletFrame)
        return false;
...
bool GenesisGUI::setCurrentWallet(const QString& name)
{
    if (!walletFrame)
        return false;
...
void GenesisGUI::removeAllWallets()
{
    if (!walletFrame)
        return;
    setWalletActionsEnabled(false);
...
void GenesisGUI::setWalletActionsEnabled(bool enabled)
{
...
void GenesisGUI::createTrayIcon(const NetworkStyle *networkStyle)
{
#ifndef Q_OS_MAC
...
void GenesisGUI::createTrayIconMenu()
{
#ifndef Q_OS_MAC
...
#ifndef Q_OS_MAC
void GenesisGUI::trayIconActivated(QSystemTrayIcon::ActivationReason reason)
{
...
void GenesisGUI::optionsClicked()
{
...
void GenesisGUI::gotoOverviewPage()
{
    overviewAction->setChecked(true);
    if (walletFrame) walletFrame->gotoOverviewPage();
}

void GenesisGUI::gotoHistoryPage()
{
    historyAction->setChecked(true);
    if (walletFrame) walletFrame->gotoHistoryPage();
}

void GenesisGUI::gotoMasternodePage()
{
    QSettings settings;
    if (!fLiteMode && settings.value("fShowMasternodesTab").toBool() && masternodeAction) {
...
void GenesisGUI::gotoReceiveCoinsPage()
{
    receiveCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoReceiveCoinsPage();
}

void GenesisGUI::gotoSendCoinsPage(QString addr)
{
    sendCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoSendCoinsPage(addr);
}

void GenesisGUI::gotoSignMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoSignMessageTab(addr);
}

void GenesisGUI::gotoVerifyMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoVerifyMessageTab(addr);
}
#endif // ENABLE_WALLET

void GenesisGUI::updateNetworkState()
{
    ...
    if (clientModel->getNetworkActive()) {
        tooltip = tr("%n active connection(s) to Genesis network", "", count) + QString(".<br>") + tr("Click to disable network activity.");
    } else {
...
void GenesisGUI::setNumConnections(int count)
{
    updateNetworkState();
}

void GenesisGUI::setNetworkActive(bool networkActive)
{
    updateNetworkState();
}

void GenesisGUI::updateHeadersSyncProgressLabel()
{
...
void GenesisGUI::setNumBlocks(int count, const QDateTime& blockDate, double nVerificationProgress, bool header)
{
    if (modalOverlay)
    {
        if (header)
...
void GenesisGUI::setAdditionalDataSyncProgress(double nSyncProgress)
{
...
void GenesisGUI::message(const QString &title, const QString &message, unsigned int style, bool *ret)
{
    QString strTitle = tr("Genesis"); // default title
...
void GenesisGUI::changeEvent(QEvent *e)
{
...
void GenesisGUI::closeEvent(QCloseEvent *event)
{
#ifndef Q_OS_MAC // Ignored on Mac
    if (clientModel && clientModel->getOptionsModel())
    {
...
#ifdef ENABLE_WALLET
void GenesisGUI::incomingTransaction(const QString& date, int unit, const CAmount& amount, const QString& type, const QString& address, const QString& label)
{
    // On new transaction, make an info balloon
    QString msg = tr("Date: %1\n").arg(date) +
                  tr("Amount: %1\n").arg(GenesisUnits::formatWithUnit(unit, amount, true)) +
                  tr("Type: %1\n").arg(type);
    if (!label.isEmpty())
        msg += tr("Label: %1\n").arg(label);
...
void GenesisGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void GenesisGUI::dropEvent(QDropEvent *event)
{
    if (event->mimeData()->hasUrls())
    {
...
bool GenesisGUI::eventFilter(QObject *object, QEvent *event)
{
...
#ifdef ENABLE_WALLET
bool GenesisGUI::handlePaymentRequest(const SendCoinsRecipient& recipient)
{
    // URI has to be valid
...
void GenesisGUI::setHDStatus(int hdEnabled)
{
...
void GenesisGUI::setEncryptionStatus(int status)
{
    switch(status)
    {
...
void GenesisGUI::showNormalIfMinimized(bool fToggleHidden)
{
    if (!clientModel)
        return;
...
void GenesisGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void GenesisGUI::detectShutdown()
{
...
void GenesisGUI::showProgress(const QString &title, int nProgress)
{
    if (nProgress == 0)
    {
...
void GenesisGUI::setTrayIconVisible(bool fHideTrayIcon)
{
    if (trayIcon)
    {
...
void GenesisGUI::showModalOverlay()
{
    if (modalOverlay && (progressBar->isVisible() || modalOverlay->isLayerVisible()))
        modalOverlay->toggleVisibility();
}

static bool ThreadSafeMessageBox(GenesisGUI *gui, const std::string& message, const std::string& caption, unsigned int style)
{
...
void GenesisGUI::subscribeToCoreSignals()
{
...
void GenesisGUI::unsubscribeFromCoreSignals()
{
...
void GenesisGUI::toggleNetworkActive()
{
    if (clientModel) {
    ...
    menu(0)
{
    ...
    QList<GenesisUnits::Unit> units = GenesisUnits::availableUnits();
    int max_width = 0;
    const QFontMetrics fm(font());
    for (const GenesisUnits::Unit unit : units)
    {
        max_width = qMax(max_width, fm.width(GenesisUnits::longName(unit)));
    }
    setMinimumSize(max_width, 0);
    setAlignment(Qt::AlignRight | Qt::AlignVCenter);
    setStyleSheet(QString("QLabel { color : #1EA1EF; }"));
}
...
void UnitDisplayStatusBarControl::createContextMenu()
{
    ...
    {
        QAction *menuAction = new QAction(QString(GenesisUnits::longName(u)), this);
...
{
    setText(GenesisUnits::longName(newUnits));
}
