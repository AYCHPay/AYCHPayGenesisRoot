...
UniValue gobject(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp  ||
        ...
        throw std::runtime_error(
                ...
                "  vote-conf          - Vote on a governance object by masternode configured in genesis.conf\n"
    ...
    */
    ...
    if (strCommand == "vote-conf")
    {
        ...
        if (!fMnFound) {
            ...
            resultsObj.push_back(Pair("genesis.conf", statusObj));
...
UniValue getgovernanceinfo(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 0) {
        throw std::runtime_error(
            ...
            "  \"governanceblockcycle\": xxxxx,               (numeric) the number of blocks between governance blocks\n"
            "  \"lastgovernanceblock\": xxxxx,                (numeric) the block number of the last governance block\n"
            "  \"nextgovernanceblock\": xxxxx,                (numeric) the block number of the next governance block\n"
    ...
    int nLastGovernanceBlock = 0, nNextGovernanceBlock = 0;
    int nBlockHeight = chainActive.Height();

    CGovernanceBlock::GetNearestGovernanceBlocksHeights(nBlockHeight, nLastGovernanceBlock, nNextGovernanceBlock);

    UniValue obj(UniValue::VOBJ);
    obj.push_back(Pair("governanceminquorum", Params().GetConsensus().nGovernanceMinQuorum));
    obj.push_back(Pair("masternodewatchdogmaxseconds", Params().GetConsensus().nMasternodeSentinelPingMaxSeconds));
    obj.push_back(Pair("sentinelpingmaxseconds", Params().GetConsensus().nMasternodeSentinelPingMaxSeconds));
    obj.push_back(Pair("proposalfee", ValueFromAmount(GOVERNANCE_PROPOSAL_FEE_TX)));
    obj.push_back(Pair("governanceblockcycle", Params().GetConsensus().nSuperBlockInterval));
    obj.push_back(Pair("lastgovernanceblock", nLastGovernanceBlock));
    obj.push_back(Pair("nextgovernanceblock", nNextGovernanceBlock));
    obj.push_back(Pair("maxgovobjdatasize", MAX_GOVERNANCE_OBJECT_DATA_SIZE));

    return obj;
}

UniValue getgovernanceblockbudget(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 1) {
        throw std::runtime_error(
            "getgovernanceblockbudget index\n"
            "\nReturns the absolute maximum sum of governanceblock payments allowed.\n"
            "\nArguments:\n"
            "1. index         (numeric, required) The block index\n"
            "\nResult:\n"
            "n                (numeric) The absolute maximum sum of governance block payments allowed, in " + CURRENCY_UNIT + "\n"
            "\nExamples:\n"
            + HelpExampleCli("getgovernanceblockbudget", "1000")
            + HelpExampleRpc("getgovernanceblockbudget", "1000")
        );
    }
    ...
    CAmount nBudget = CGovernanceBlock::GetPaymentsLimit(nBlockHeight);
...
{ //  category              name                      actor (function)         argNames
  //  --------------------- ------------------------  -----------------------  ----------
    ...
    { "governance",         "getgovernanceblockbudget",    &getgovernanceblockbudget,    {"index"} },
