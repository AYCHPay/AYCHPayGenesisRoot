...
void PushNodeVersion(CNode *pnode, CConnman* connman, int64_t nTime)
{
    ...
    if (fLogIPs) {
        LogPrintG(BCLogLevel::LOG_NOTICE, BCLog::NET, "[Networking] send version message: version %d, blocks=%d, us=%s, them=%s, peer=%d\n", PROTOCOL_VERSION, nNodeStartingHeight, addrMe.ToString(), addrYou.ToString(), nodeid);
    } else {
        LogPrintG(BCLogLevel::LOG_NOTICE, BCLog::NET, "[Networking] send version message: version %d, blocks=%d, us=%s, peer=%d\n", PROTOCOL_VERSION, nNodeStartingHeight, addrMe.ToString(), nodeid);
    }
}
...
void FindNextBlocksToDownload(NodeId nodeid, unsigned int count, std::vector<const CBlockIndex*>& vBlocks, NodeId& nodeStaller, const Consensus::Params& consensusParams) {
    ...
    {
        //LogPrintG(BCLogLevel::LOG_NOTICE, BCLog::NET, "[Networking] Node (%s) has a null index for the best known block \n", state->name);
        return;
    }
    else if (state->pindexBestKnownBlock->nChainWork < chainActive.Tip()->nChainWork)
    {
        //LogPrintG(BCLogLevel::LOG_NOTICE, BCLog::NET, "[Networking] Node (%s) best known block's chain work is less than the current tip's chain work \n", state->name);
        return;        
    } 
    else if (state->pindexBestKnownBlock->nChainWork < nMinimumChainWork) 
    {
        //LogPrintG(BCLogLevel::LOG_NOTICE, BCLog::NET, "[Networking] Node (%s) best known block's chain work is less than the minimum allowed chain work \n", state->name);
        return;                
    }
...
bool AddOrphanTx(const CTransactionRef& tx, NodeId peer) EXCLUSIVE_LOCKS_REQUIRED(g_cs_orphans)
{
    ...
    {
        LogPrintG(BCLogLevel::LOG_NOTICE, BCLog::MEMPOOL, "[MemPool] ignoring large orphan tx (size: %u, hash: %s)\n", sz, hash.ToString());
        return false;
    }
    ...
    LogPrintG(BCLogLevel::LOG_NOTICE, BCLog::MEMPOOL, "[MemPool] stored orphan tx %s (mapsz %u outsz %u)\n", hash.ToString(),
...
void EraseOrphansFor(NodeId peer)
{
    ...
    if (nErased > 0) LogPrintG(BCLogLevel::LOG_NOTICE, BCLog::MEMPOOL, "[MemPool] Erased %d orphan tx from peer=%d\n", nErased, peer);
}
...
unsigned int LimitOrphanTxSize(unsigned int nMaxOrphans)
{
    ...
    if (nNextSweep <= nNow) {
        ...
        if (nErased > 0) LogPrintG(BCLogLevel::LOG_NOTICE, BCLog::MEMPOOL, "[MemPool] Erased %d orphan tx due to expiration\n", nErased);
    }
...
void Misbehaving(NodeId pnode, int howmuch)
{
    ...
    {
        LogPrintG(BCLogLevel::LOG_NOTICE, BCLog::NET, "[Networking] %s: %s peer=%d (%d -> %d) BAN THRESHOLD EXCEEDED\n", __func__, state->name, pnode, state->nMisbehavior-howmuch, state->nMisbehavior);
        state->fShouldBan = true;
    } else
        LogPrintG(BCLogLevel::LOG_NOTICE, BCLog::NET, "[Networking] %s: %s peer=%d (%d -> %d)\n", __func__, state->name, pnode, state->nMisbehavior-howmuch, state->nMisbehavior);
}
...
void PeerLogicValidation::BlockConnected(const std::shared_ptr<const CBlock>& pblock, const CBlockIndex* pindex, const std::vector<CTransactionRef>& vtxConflicted) {
    ...
    if (vOrphanErase.size()) {
        ...
        LogPrintG(BCLogLevel::LOG_NOTICE, BCLog::MEMPOOL, "[MemPool] Erased %d orphan tx included or conflicted by block\n", nErased);
    }
...
void static ProcessGetBlockData(CNode* pfrom, const Consensus::Params& consensusParams, const CInv& inv, CConnman* connman, const std::atomic<bool>& interruptMsgProc)
{
    ...
    if (mi != mapBlockIndex.end()) {
        ...
        if (!send) {
            LogPrintG(BCLogLevel::LOG_NOTICE, BCLog::NET, "[Networking] %s: ignoring request from peer=%i for old block that isn't in the main chain\n", __func__, pfrom->GetId());
        }
    }
    ...
    if (send && !pfrom->fWhitelisted && (
        ...
        LogPrintG(BCLogLevel::LOG_NOTICE, BCLog::NET, "[Networking] Ignore block request below NODE_NETWORK_LIMITED threshold from peer=%d\n", pfrom->GetId());
    ...
    const CNetMsgMaker msgMaker(pfrom->GetSendVersion());
    {
        ...
        while (it != pfrom->vRecvGetData.end() && (it->type == MSG_TX || it->type == MSG_WITNESS_TX || it->type == MSG_MASTERNODE_PAYMENT_VOTE_PRIMARY || it->type == MSG_MASTERNODE_PAYMENT_VOTE_SECONDARY || it->type == MSG_MASTERNODE_PAYMENT_BLOCK_PRIMARY || it->type == MSG_MASTERNODE_PAYMENT_BLOCK_SECONDARY || it->type == MSG_MASTERNODE_QUORUM || it->type == MSG_MASTERNODE_ANNOUNCE || it->type == MSG_MASTERNODE_PING || it->type == MSG_GOVERNANCE_OBJECT || it->type == MSG_GOVERNANCE_OBJECT_VOTE || it->type == MSG_MASTERNODE_VERIFY)) {
            ...
            if (!push && inv.type == MSG_GOVERNANCE_OBJECT) {
                LogPrintG(BCLogLevel::LOG_NOTICE, BCLog::NET, "ProcessGetData -- MSG_GOVERNANCE_OBJECT: inv = %s\n", inv.ToString());
                ...
                LogPrintG(BCLogLevel::LOG_NOTICE, BCLog::NET, "ProcessGetData -- MSG_GOVERNANCE_OBJECT: topush = %d, inv = %s\n", topush, inv.ToString());
                if (topush) {
...
inline void static SendBlockTransactions(const CBlock& block, const BlockTransactionsRequest& req, CNode* pfrom, CConnman* connman) {
    ...
    for (size_t i = 0; i < req.indexes.size(); i++) {
        if (req.indexes[i] >= block.vtx.size()) {
            ...
            LogPrintG(BCLogLevel::LOG_NOTICE, BCLog::NET, "[Networking] Peer %d sent us a getblocktxn with out-of-bounds tx indices", pfrom->GetId());
            return;
        }
...
bool static ProcessHeadersMessage(CNode *pfrom, CConnman *connman, const std::vector<CBlockHeader>& headers, const CChainParams& chainparams, bool punish_duplicate_invalid)
{
    ...
    const CBlockIndex *pindexLast = nullptr;
    {
        ...
        if (mapBlockIndex.find(headers[0].hashPrevBlock) == mapBlockIndex.end() && nCount < MAX_BLOCKS_TO_ANNOUNCE) {
            ...
            LogPrintG(BCLogLevel::LOG_NOTICE, BCLog::NET, "[Networking] received header %s: missing prev block %s, sending getheaders (%d) to end (peer=%d, nUnconnectingHeaders=%d)\n",
...

    {
        ...
        if (nodestate->nUnconnectingHeaders > 0) {
            LogPrintG(BCLogLevel::LOG_NOTICE, BCLog::NET, "[Networking] peer=%d: resetting nUnconnectingHeaders (%d -> 0)\n", pfrom->GetId(), nodestate->nUnconnectingHeaders);
        }
        ...
        if (nCount == MAX_HEADERS_RESULTS) {
            ...
            // from there instead.
            LogPrintG(BCLogLevel::LOG_NOTICE, BCLog::NET, "[Networking] more getheaders (%d) to end to peer=%d (startheight:%d)\n", pindexLast->nHeight, pfrom->GetId(), pfrom->nStartingHeight);
        ...
        if (fCanDirectFetch && pindexLast->IsValid(BLOCK_VALID_TREE) && chainActive.Tip()->nChainWork <= pindexLast->nChainWork) {
            ...
            if (!chainActive.Contains(pindexWalk)) {
                ...
                for (const CBlockIndex *pindex : reverse_iterate(vToFetch)) {
                    ...
                    LogPrintG(BCLogLevel::LOG_NOTICE, BCLog::NET, "[Networking] Requesting block %s from  peer=%d\n",
                            pindex->GetBlockHash().ToString(), pfrom->GetId());
                }
                if (vGetData.size() > 1) {
                    LogPrintG(BCLogLevel::LOG_NOTICE, BCLog::NET, "[Networking] Downloading blocks toward %s (%d) via headers direct fetch\n",
        ...
        if (IsInitialBlockDownload() && nCount != MAX_HEADERS_RESULTS) {
            ...
            if (nodestate->pindexBestKnownBlock && nodestate->pindexBestKnownBlock->nChainWork < nMinimumChainWork) {
                ...
                if (IsOutboundDisconnectionCandidate(pfrom)) {
                    LogPrintG(BCLogLevel::LOG_NOTICE, BCLog::NET, "[Networking] Disconnecting outbound peer %d -- headers chain has insufficient work\n", pfrom->GetId());
                    pfrom->fDisconnect = true;
                }
            }
        }
        ...
        if (!pfrom->fDisconnect && IsOutboundDisconnectionCandidate(pfrom) && nodestate->pindexBestKnownBlock != nullptr) {
            ...
            if (g_outbound_peers_with_protect_from_disconnect < MAX_OUTBOUND_PEERS_TO_PROTECT_FROM_DISCONNECT && nodestate->pindexBestKnownBlock->nChainWork >= chainActive.Tip()->nChainWork && !nodestate->m_chain_sync.m_protect) {
                LogPrintG(BCLogLevel::LOG_NOTICE, BCLog::NET, "[Networking] Protecting outbound peer=%d from eviction\n", pfrom->GetId());
...
{
    LogPrintG(BCLogLevel::LOG_NOTICE, BCLog::NET, "[Networking] received: %s (%u bytes) peer=%d\n", SanitizeString(strCommand), vRecv.size(), pfrom->GetId());
    if (gArgs.IsArgSet("-dropmessagestest") && GetRand(gArgs.GetArg("-dropmessagestest", 0)) == 0)
    {
        LogPrintG(BCLogLevel::LOG_NOTICE, BCLog::NET, "[Networking] dropmessagestest DROPPING RECV MESSAGE\n");
        return true;
    }
    ...
    if (strCommand == NetMsgType::REJECT)
    {
        if (LogAcceptCategory(BCLog::NET)) {
            try {
                ...
                LogPrintG(BCLogLevel::LOG_NOTICE, BCLog::NET, "[Networking] Reject %s\n", SanitizeString(ss.str()));
            } catch (const std::ios_base::failure&) {
                // Avoid feedback loops by preventing reject messages from triggering a new reject message.
                LogPrintG(BCLogLevel::LOG_NOTICE, BCLog::NET, "[Networking] Unparseable reject message received\n");
            }
        }
    }
    ...
    else if (strCommand == NetMsgType::VERSION)
    {
        ...
        {
            LogPrintG(BCLogLevel::LOG_NOTICE, BCLog::NET, "[Networking] peer=%d does not offer the expected services (%08x offered, %08x expected); disconnecting\n", pfrom->GetId(), nServices, GetDesirableServiceFlags(nServices));
        ...
        if (nVersion < MIN_PEER_PROTO_VERSION)
        {
            ...
            LogPrintG(BCLogLevel::LOG_NOTICE, BCLog::NET, "[Networking] peer=%d using obsolete version %i; disconnecting\n", pfrom->GetId(), nVersion);
    ...
    else if (strCommand == NetMsgType::GETBLOCKS)
    {
        ...
        for (; pindex; pindex = chainActive.Next(pindex))
        {
            ...
            {
                LogPrintG(BCLogLevel::LOG_NOTICE, BCLog::NET, "[Networking] getblocks stopping, pruned or too old block at %d %s\n", pindex->nHeight, pindex->GetBlockHash().ToString());
                break;
            }
    ...
    else if (strCommand == NetMsgType::GETBLOCKTXN)
    {
        ...
        if (it == mapBlockIndex.end() || !(it->second->nStatus & BLOCK_HAVE_DATA)) {
            LogPrintG(BCLogLevel::LOG_NOTICE, BCLog::NET, "[Networking] Peer %d sent us a getblocktxn for a block we don't have", pfrom->GetId());
            return true;
        }
        ...
        if (it->second->nHeight < chainActive.Height() - MAX_BLOCKTXN_DEPTH) {
            ...
            LogPrintG(BCLogLevel::LOG_NOTICE, BCLog::NET, "[Networking] Peer %d sent us a getblocktxn for a block > %i deep", pfrom->GetId(), MAX_BLOCKTXN_DEPTH);
            CInv inv;
    ...
    else if (strCommand == NetMsgType::GETHEADERS)
    {
        ...
        if (IsInitialBlockDownload() && !pfrom->fWhitelisted) {
            LogPrintG(BCLogLevel::LOG_NOTICE, BCLog::NET, "[Networking] Ignoring getheaders from peer=%d because node is in initial block download\n", pfrom->GetId());
            return true;
        }
        ...
        if (locator.IsNull())
        {
            ...
            if (!BlockRequestAllowed(pindex, chainparams.GetConsensus())) {
                LogPrintG(BCLogLevel::LOG_NOTICE, BCLog::NET, "[Networking] %s: ignoring request from peer=%i for old block header that isn't in the main chain\n", __func__, pfrom->GetId());
                return true;
            }
        }
        else
        ...
        LogPrintG(BCLogLevel::LOG_NOTICE, BCLog::NET, "[Networking] getheaders %d to %s from peer=%d\n", (pindex ? pindex->nHeight : -1), hashStop.IsNull() ? "end" : hashStop.ToString(), pfrom->GetId());
        for (; pindex; pindex = chainActive.Next(pindex))
        {
    ...
    else if (strCommand == NetMsgType::TX)
    {
        ...
        {
            LogPrintG(BCLogLevel::LOG_NOTICE, BCLog::NET, "[Networking] Transaction sent in violation of protocol peer=%d\n", pfrom->GetId());
            return true;
        }
        ...
        if (!AlreadyHave(inv) &&
            ...
            LogPrintG(BCLogLevel::LOG_NOTICE, BCLog::MEMPOOL, "[MemPool] AcceptToMemoryPool: peer=%d: accepted %s (poolsz %u txn, %u kB)\n",
                pfrom->GetId(),
                tx.GetHash().ToString(),
            ...
            while (!vWorkQueue.empty()) {
                     ...
                     ++mi)
                {
                    ...
                    if (AcceptToMemoryPool(mempool, stateDummy, porphanTx, &fMissingInputs2, &lRemovedTxn, false /* bypass_limits */, 0 /* nAbsurdFee */)) {
                        LogPrintG(BCLogLevel::LOG_NOTICE, BCLog::MEMPOOL, "[MemPool] Accepted orphan tx %s\n", orphanHash.ToString());
                    ...
                    else if (!fMissingInputs2)
                    {
                        ...
                        if (stateDummy.IsInvalid(nDos) && nDos > 0)
                        {
                            ...
                            LogPrintG(BCLogLevel::LOG_NOTICE, BCLog::MEMPOOL, "[MemPool] Invalid orphan tx %s\n", orphanHash.ToString());
                        }
                        // Has inputs but not accepted to mempool
                        // Probably non-standard or insufficient fee
                        LogPrintG(BCLogLevel::LOG_NOTICE, BCLog::MEMPOOL, "[MemPool] Removed orphan tx %s\n", orphanHash.ToString());
                        vEraseQueue.push_back(orphanHash);
        ...
        else if (fMissingInputs)
        {
            ...
            if (!fRejectedParents) {
                ...
                if (nEvicted > 0) {
                    LogPrintG(BCLogLevel::LOG_NOTICE, BCLog::MEMPOOL, "[MemPool] mapOrphan overflow, removed %u tx\n", nEvicted);
                }
            } else {
                LogPrintG(BCLogLevel::LOG_NOTICE, BCLog::MEMPOOL, "[MemPool] Not keeping orphan with rejected parents %s\n",tx.GetHash().ToString());
            ...
            if (pfrom->fWhitelisted && gArgs.GetBoolArg("-whitelistforcerelay", DEFAULT_WHITELISTFORCERELAY)) {
                ...
                if (!state.IsInvalid(nDoS) || nDoS == 0) {
                    LogPrintG(BCLogLevel::LOG_NOTICE, BCLog::MEMPOOL, "[MemPool] Force relaying tx %s from whitelisted peer=%d\n", tx.GetHash().ToString(), pfrom->GetId());
                    RelayTransaction(tx, connman);
                } else {
                    LogPrintG(BCLogLevel::LOG_NOTICE, BCLog::MEMPOOLREJ, "[MemPoolRejects] Not relaying invalid transaction %s from whitelisted peer=%d (%s)\n", tx.GetHash().ToString(), pfrom->GetId(), FormatStateMessage(state));
                }
            }
        }
        ...
        if (state.IsInvalid(nDoS))
        {
            LogPrintG(BCLogLevel::LOG_NOTICE, BCLog::MEMPOOLREJ, "[MemPoolRejects] %s from peer=%d was not accepted: %s\n", tx.GetHash().ToString(),
                pfrom->GetId(),
    ...
    else if (strCommand == NetMsgType::CMPCTBLOCK && !fImporting && !fReindex) // Ignore blocks received while importing
    {
        ...
        if (!ProcessNewBlockHeaders({cmpctblock.header}, state, chainparams, &pindex)) {
            ...
            if (state.IsInvalid(nDoS)) {
                if (nDoS > 0) {
                    LogPrintG(BCLogLevel::LOG_NOTICE, BCLog::NET, "[Networking] Peer %d sent us invalid header via cmpctblock\n", pfrom->GetId());
                    LOCK(cs_main);
                    Misbehaving(pfrom->GetId(), nDoS);
                } else {
                    LogPrintG(BCLogLevel::LOG_NOTICE, BCLog::NET, "[Networking] Peer %d sent us invalid header via cmpctblock\n", pfrom->GetId());
            ...
            if ((!fAlreadyInFlight && nodestate->nBlocksInFlight < MAX_BLOCKS_IN_TRANSIT_PER_PEER) ||
                ...
                if (!MarkBlockAsInFlight(pfrom->GetId(), pindex->GetBlockHash(), pindex, &queuedBlockIt)) {
                    if (!(*queuedBlockIt)->partialBlock)
                        ...
                        LogPrintG(BCLogLevel::LOG_NOTICE, BCLog::NET, "[Networking] eer sent us compact block we were already syncing!\n");
                        return true;
                    }
                }
                ...
                if (status == READ_STATUS_INVALID) {
                    ...
                    Misbehaving(pfrom->GetId(), 100);
                    LogPrintG(BCLogLevel::LOG_NOTICE, BCLog::NET, "[Networking] Peer %d sent us invalid compact block\n", pfrom->GetId());
                    return true;
    ...
    else if (strCommand == NetMsgType::BLOCKTXN && !fImporting && !fReindex) // Ignore blocks received while importing
    {
        ...
        bool fBlockRead = false;
        {
            ...
            if (it == mapBlocksInFlight.end() || !it->second.second->partialBlock ||
                ...
                LogPrintG(BCLogLevel::LOG_NOTICE, BCLog::NET, "[Networking] Peer %d sent us block transactions for block we weren't expecting\n", pfrom->GetId());
                return true;
            }
            ...
            if (status == READ_STATUS_INVALID) {
                ...
                Misbehaving(pfrom->GetId(), 100);
                LogPrintG(BCLogLevel::LOG_NOTICE, BCLog::NET, "[Networking] Peer %d sent us invalid compact block/non-matching block transactions\n", pfrom->GetId());
                return true;
            } else if (status == READ_STATUS_FAILED) {
    ...
    else if (strCommand == NetMsgType::BLOCK && !fImporting && !fReindex) // Ignore blocks received while importing
    {
        ...
        LogPrintG(BCLogLevel::LOG_NOTICE, BCLog::NET, "[Networking] Received block %s peer=%d\n", pblock->GetHash().ToString(), pfrom->GetId());
    ...
    else if (strCommand == NetMsgType::GETADDR)
    {
        ...
        if (!pfrom->fInbound) {
            LogPrintG(BCLogLevel::LOG_NOTICE, BCLog::NET, "[Networking] Ignoring \"getaddr\" from outbound connection. peer=%d\n", pfrom->GetId());
            return true;
        }
        ...
        if (pfrom->fSentAddr) {
            LogPrintG(BCLogLevel::LOG_NOTICE, BCLog::NET, "[Networking] Ignoring repeated \"getaddr\". peer=%d\n", pfrom->GetId());
            return true;
        }
    ...
    else if (strCommand == NetMsgType::MEMPOOL)
    {
        ...
        {
            LogPrintG(BCLogLevel::LOG_NOTICE, BCLog::NET, "[Networking] Mempool request with bloom filters disabled, disconnect peer=%d\n", pfrom->GetId());
            pfrom->fDisconnect = true;
            return true;
        }
        ...
        {
            LogPrintG(BCLogLevel::LOG_NOTICE, BCLog::NET, "[Networking] Mempool request with bandwidth limit reached, disconnect peer=%d\n", pfrom->GetId());
            pfrom->fDisconnect = true;
            return true;
        }
    ...
    else if (strCommand == NetMsgType::PONG)
    {
        ...
        if (!(sProblem.empty())) {
            LogPrintG(BCLogLevel::LOG_NOTICE, BCLog::NET, "[Networking] Pong peer=%d: %s, %x expected, %x received, %u bytes\n",
                pfrom->GetId(),
                sProblem,
    ...
    else if (strCommand == NetMsgType::FILTERCLEAR)
    {
        ...
        if (MoneyRange(newFeeFilter)) {
            ...
            LogPrintG(BCLogLevel::LOG_NOTICE, BCLog::NET, "[Networking] Received: feefilter of %s from peer=%d\n", CFeeRate(newFeeFilter).ToString(), pfrom->GetId());
        }
    }
    ...
    else
    {
        ...
        else
        {
            ...
            LogPrintG(BCLogLevel::LOG_NOTICE, BCLog::NET, "[Networking] Unknown command \"%s\" from peer=%d\n", SanitizeString(strCommand), pfrom->GetId());
        }
    }
...
static bool SendRejectsAndCheckIfBanned(CNode* pnode, CConnman* connman)
{
    ...
    if (state.fShouldBan) {
        ...
        if (pnode->fWhitelisted)
            LogPrintG(BCLogLevel::LOG_NOTICE, BCLog::NET, "[Networking] Warning: Not punishing whitelisted peer %s!\n", pnode->addr.ToString());
        else if (pnode->m_manual_connection)
            LogPrintG(BCLogLevel::LOG_NOTICE, BCLog::NET, "[Networking] Warning: Not punishing manually-connected peer %s!\n", pnode->addr.ToString());
        else {
            pnode->fDisconnect = true;
            if (pnode->addr.IsLocal())
                LogPrintG(BCLogLevel::LOG_NOTICE, BCLog::NET, "[Networking] Warning: Not banning local peer %s!\n", pnode->addr.ToString());
            else
            {
...
bool PeerLogicValidation::ProcessMessages(CNode* pfrom, std::atomic<bool>& interruptMsgProc)
{
    ...
    if (memcmp(msg.hdr.pchMessageStart, chainparams.MessageStart(), CMessageHeader::MESSAGE_START_SIZE) != 0) {
        LogPrintG(BCLogLevel::LOG_NOTICE, BCLog::NET, "[Networking] PROCESSMESSAGE: INVALID MESSAGESTART %s peer=%d\n", SanitizeString(msg.hdr.GetCommand()), pfrom->GetId());
        pfrom->fDisconnect = true;
        return false;
    }
    ...
    {
        LogPrintG(BCLogLevel::LOG_NOTICE, BCLog::NET, "[Networking] PROCESSMESSAGE: ERRORS IN HEADER %s peer=%d\n", SanitizeString(hdr.GetCommand()), pfrom->GetId());
        return fMoreWork;
    }
    ...
    {
        LogPrintG(BCLogLevel::LOG_NOTICE, BCLog::NET, "[Networking] %s(%s, %u bytes): CHECKSUM ERROR expected %s was %s\n", __func__,
    ...
    if (!fRet) {
        LogPrintG(BCLogLevel::LOG_NOTICE, BCLog::NET, "[Networking] %s(%s, %u bytes) FAILED peer=%d\n", __func__, SanitizeString(strCommand), nMessageSize, pfrom->GetId());
    }
...
bool PeerLogicValidation::SendMessages(CNode* pto, std::atomic<bool>& interruptMsgProc)
{
    const Consensus::Params& consensusParams = Params().GetConsensus();
    {
        ...
        if (state.nStallingSince && state.nStallingSince < nNow - 1000000 * BLOCK_STALLING_TIMEOUT) {
            ...
            LogPrintG(BCLogLevel::LOG_NOTICE, BCLog::NET, "[Networking] Peer=%d is stalling block download, disconnecting\n", pto->GetId());
            pto->fDisconnect = true;
            return true;
        }
        ...
        if (state.vBlocksInFlight.size() > 0) {
            ...
            if (nNow > state.nDownloadingSince + consensusParams.nPowTargetSpacing * (BLOCK_DOWNLOAD_TIMEOUT_BASE + BLOCK_DOWNLOAD_TIMEOUT_PER_PEER * nOtherPeersWithValidatedDownloads)) {
                LogPrintG(BCLogLevel::LOG_NOTICE, BCLog::NET, "[Networking] Timeout downloading block %s from peer=%d, disconnecting\n", queuedBlock.hash.ToString(), pto->GetId());
                pto->fDisconnect = true;
                return true;
            }
        }
        ...
        if (state.fSyncStarted && state.nHeadersSyncTimeout < std::numeric_limits<int64_t>::max()) {
            ...
            if (pindexBestHeader->GetBlockTime() <= GetAdjustedTime() - 24*60*60) {
                if (nNow > state.nHeadersSyncTimeout && nSyncStarted == 1 && (nPreferredDownload - state.fPreferredDownload >= 1)) {
                    ...
                    if (!pto->fWhitelisted) {
                        LogPrintG(BCLogLevel::LOG_NOTICE, BCLog::NET, "[Networking] Timeout downloading headers from peer=%d, disconnecting\n", pto->GetId());
                        pto->fDisconnect = true;
                        return true;
                    } else {
                        LogPrintG(BCLogLevel::LOG_NOTICE, BCLog::NET, "[Networking] Timeout downloading headers from whitelisted peer=%d, not disconnecting\n", pto->GetId());
        ...
        if (!pto->fClient && (fFetch || !IsInitialBlockDownload()) && state.nBlocksInFlight < MAX_BLOCKS_IN_TRANSIT_PER_PEER) {
            ...
            for (const CBlockIndex *pindex : vToDownload) {
                ...
                LogPrintG(BCLogLevel::LOG_NOTICE, BCLog::NET, "[Networking] Requesting block %s (%d) peer=%d\n", pindex->GetBlockHash().ToString(),
                    pindex->nHeight, pto->GetId());
            }
            if (state.nBlocksInFlight == 0 && staller != -1) {
                if (State(staller)->nStallingSince == 0) {
                    State(staller)->nStallingSince = nNow;
                    LogPrintG(BCLogLevel::LOG_NOTICE, BCLog::NET, "[Networking] Stall started peer=%d\n", staller);
                }
            }
        }
        ...
        while (!pto->mapAskFor.empty() && (*pto->mapAskFor.begin()).first <= nNow)
        {
            ...
            if (!AlreadyHave(inv))
            {
                LogPrintG(BCLogLevel::LOG_NOTICE, BCLog::NET, "[Networking] Requesting %s peer=%d\n", inv.ToString(), pto->GetId());
