...
#include <masternodes/flat-database.h>
 ...
 * between genesisd, and genesis-qt and non-server tools.
*/
...
public:
    ...
    bool GetCoin(const COutPoint &outpoint, Coin &coin) const override {
        try {
            ...
            LogPrintG(BCLogLevel::LOG_NOTICE, BCLog::LEVELDB, "[LevelDB] Error reading from database: %s\n", e.what());
...
void Shutdown()
{
    LogPrintG(BCLogLevel::LOG_NOTICE, BCLog::LEVELDB, "[LevelDB] %s: In progress...\n", __func__);
    ...
    if (fFeeEstimatesInitialized)
    {
        ...
        if (!est_fileout.IsNull())
            ...
            LogPrintG(BCLogLevel::LOG_NOTICE, BCLog::ESTIMATEFEE, "[FeeEstimation] %s: Failed to write fee estimates to %s\n", __func__, est_path.string());
...
void OnRPCStopped()
{
    ...
    LogPrintG(BCLogLevel::LOG_NOTICE, BCLog::RPC, "[RPC] Stopped.\n");
}
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf(_("Specify configuration file (default: %s)"), GENESIS_CONF_FILENAME));
    if (mode == HMM_GENESISD)
    {
#if HAVE_DECL_DAEMON
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), GENESIS_PID_FILENAME));
#endif
    ...
    strUsage += HelpMessageOpt("-litemode=<n>", strprintf(_("Disable all Genesis Masternode specific functionality (Masternodes, Governance) (0-1, default: %u)"), 0));
...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/genesisofficial/genesis>";
    const std::string URL_WEBSITE = "<https://genesisnetwork.io/>";
...
void ThreadImport(std::vector<fs::path> vImportFiles)
{
    ...
    if (fReindex) {
        ...
        while (true) {
            ...
            LogPrintG(BCLogLevel::LOG_NOTICE, BCLog::REINDEX, "[Reindex] Reindexing block file blk%05u.dat...\n", (unsigned int)nFile);
            LoadExternalBlockFile(chainparams, file, &pos);
            nFile++;
        }
        pblocktree->WriteReindexing(false);
        fReindex = false;
        LogPrintG(BCLogLevel::LOG_NOTICE, BCLog::REINDEX, "[Reindex] Reindexing finished\n");
...
/** Sanity checks
 *  Ensure that Genesis is running in a usable environment with all
...
bool AppInitParameterInteraction()
{
    ...
    if (nPruneArg == 1) {  // manual pruning: -prune=1
        LogPrintG(BCLogLevel::LOG_NOTICE, BCLog::PRUNE, "[Prune] Block pruning enabled. Use RPC call pruneblockchain(height) to manually prune block and undo files.\n");
        ...
        LogPrintG(BCLogLevel::LOG_NOTICE, BCLog::PRUNE, "[Prune] Configured to target %uMiB on disk for block and undo files.\n", nPruneTarget / 1024 / 1024);
        fPruneMode = true;
    }
...
{
    // Make sure only a single Genesis process is using the data directory.
...
bool AppInitMain()
{
    ...
    LogPrintf("Using config file %s\n", GetConfigFile(gArgs.GetArg("-conf", GENESIS_CONF_FILENAME)).string());
    LogPrintf("Using at most %i automatic connections (%i file descriptors available)\n", nMaxConnections, nFD);

    // Warn about relative -datadir path.
    if (gArgs.IsArgSet("-datadir") && !fs::path(gArgs.GetArg("-datadir", "")).is_absolute()) {
        LogPrintf("Warning: relative datadir option '%s' specified, which will be interpreted relative to the "
                  "current working directory '%s'. This is fragile, because if Genesis Official is started in the future "
                  "from a different location, it will be unable to locate the current data files. There could "
                  "also be data loss if Genesis Official is started while in a temporary directory.\n",
            ...
            try {
                ...
                if (!is_coinsview_empty) {
                    ...
                    if (fHavePruned && gArgs.GetArg("-checkblocks", DEFAULT_CHECKBLOCKS) > MIN_BLOCKS_TO_KEEP) {
                        LogPrintG(BCLogLevel::LOG_NOTICE, BCLog::PRUNE, "[Prune] Pruned datadir may not have more than %d blocks; only checking available blocks",
                            MIN_BLOCKS_TO_KEEP);
                    }
        ...
        if (!fLoaded && !fRequestShutdown) {
            ...
            if (!fReset) {
                ...
                if (fRet) {
                    ...
                    LogPrintG(BCLogLevel::LOG_NOTICE, BCLog::REINDEX, "[Reindex] Aborted block database rebuild. Exiting.\n");
                    return false;
                }
            } else {
    ...
    if (fPruneMode) {
        LogPrintG(BCLogLevel::LOG_NOTICE, BCLog::PRUNE, "[Prune] Unsetting NODE_NETWORK on prune mode\n");
    ...
    //lite mode disables all Genesis Masternode specific functionality
    fLiteMode = gArgs.GetBoolArg("-litemode", false);

    if (fLiteMode) {
        InitWarning(_("You are starting in lite mode, all Genesis Masternode specific functionality is disabled."));
    }
    ...
    // ********************************************************* Step 11c: update block tip in Genesis Masternode modules
...
void ThreadCheckMasternode(CConnman& connman)
{
    ...
    while (true)
    {
        ...
        if (masternodeSync.IsBlockchainSynced() && !ShutdownRequested()) {
            ...
            if (nTick % Params().GetConsensus().nMasternodeMinMnpSeconds == 15)
