...
int Equihash<N,K>::InitialiseState(eh_HashState& base_state, const std::string personalizationString)
{
    ...
    //LogPrintG(BCLogLevel::DEBUG, BCLog::POW, "[ProofOfWork] Equihash Initialise State called with %s\n", personalizationString);
                               ...
                               const std::function<bool(EhSolverCancelCheck)> cancelled)
{
    ...
    // LogPrintG(BCLogLevel::DEBUG, BCLog::POW, "[ProofOfWork] Generating first list\n");
    ...
    for (unsigned int r = 1; r < K && X.size() > 0; r++) {
        // LogPrintG(BCLogLevel::DEBUG, BCLog::POW, "[ProofOfWork] Round %u:\n", r);
        // 2a) Sort the list
        // LogPrintG(BCLogLevel::DEBUG, BCLog::POW, "[ProofOfWork] - Sorting list\n");
        std::sort(X.begin(), X.end(), CompareSR(CollisionByteLength));
        if (cancelled(ListSorting)) throw solver_cancelled;

        // LogPrintG(BCLogLevel::DEBUG, BCLog::POW, "[ProofOfWork] - Finding collisions\n");
        size_t i = 0;
        size_t posFree = 0;
    ...
    // LogPrintG(BCLogLevel::DEBUG, BCLog::POW, "[ProofOfWork] Final round:\n");
    if (X.size() > 1) {
        // LogPrintG(BCLogLevel::DEBUG, BCLog::POW, "[ProofOfWork] - Sorting list\n");
    ...
    {
        // LogPrintG(BCLogLevel::DEBUG, BCLog::POW, "[ProofOfWork] - List is empty\n");
    }
                                   ...
                                   const std::function<bool(EhSolverCancelCheck)> cancelled)
{
    ...
    size_t invalidCount = 0;
    {
        ...
        for (size_t r = 1; r < K && Xt.size() > 0; r++) {
            //LogPrintG(BCLogLevel::DEBUG, BCLog::POW, "[ProofOfWork] Round %zu:\n", r);
            // 2a) Sort the list
            //LogPrintG(BCLogLevel::DEBUG, BCLog::POW, "[ProofOfWork] - Sorting list\n");
    ...
    //LogPrintG(BCLogLevel::DEBUG, BCLog::POW, "[ProofOfWork] Found %d partial solutions\n", partialSolns.size());

    // Now for each solution run the algorithm again to recreate the indices
    //LogPrintG(BCLogLevel::DEBUG, BCLog::POW, "[ProofOfWork] Culling solutions\n");
...
invalidsolution:
    ...
    //LogPrintG(BCLogLevel::DEBUG, BCLog::POW, "[ProofOfWork] - Number of invalid solutions found: %zu\n", invalidCount);
...
bool Equihash<N,K>::IsValidSolution(const eh_HashState& base_state, std::vector<unsigned char> soln)
{
    if (soln.size() != SolutionWidth) {
        // LogPrintG(BCLogLevel::DEBUG, BCLog::POW, "[ProofOfWork] Invalid solution length: %d (expected %d)\n", soln.size(), SolutionWidth);
        return false;
    }
    ...
    while (X.size() > 1) {
        ...
        for (size_t i = 0; i < X.size(); i += 2) {
            if (!HasCollision(X[i], X[i+1], CollisionByteLength)) {
                ...
                // LogPrintG(BCLogLevel::DEBUG, BCLog::POW, "[ProofOfWork] Invalid solution: invalid collision length between StepRows\n");
                // LogPrintG(BCLogLevel::DEBUG, BCLog::POW, "[ProofOfWork] X[i]   = %s\n", X[i].GetHex(hashLen));
                // LogPrintG(BCLogLevel::DEBUG, BCLog::POW, "[ProofOfWork] X[i+1] = %s\n", X[i+1].GetHex(hashLen));
                return false;
            }
            if (X[i+1].IndicesBefore(X[i], hashLen, lenIndices)) {
                // LogPrintG(BCLogLevel::DEBUG, BCLog::POW, "[ProofOfWork] Invalid solution: Index tree incorrectly ordered\n");
                return false;
            }
            if (!DistinctIndices(X[i], X[i+1], hashLen, lenIndices)) {
                // LogPrintG(BCLogLevel::DEBUG, BCLog::POW, "[ProofOfWork] Invalid solution: duplicate indices\n");
                return false;
            }
