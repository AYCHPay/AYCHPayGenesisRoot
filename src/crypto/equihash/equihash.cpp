                                   ...
                                   const std::function<bool(EhSolverCancelCheck)> cancelled)
{
    ...
    //LogPrintG(BCLogLevel::LOG_DEBUG, BCLog::POW, "[ProofOfWork] Found %d partial solutions\n", partialSolns.size());

    // Now for each solution run the algorithm again to recreate the indices
    //LogPrintG(BCLogLevel::LOG_DEBUG, BCLog::POW, "[ProofOfWork] Culling solutions\n");
...
invalidsolution:
    ...
    //LogPrintG(BCLogLevel::LOG_DEBUG, BCLog::POW, "[ProofOfWork] - Number of invalid solutions found: %zu\n", invalidCount);
...
bool Equihash<N,K>::IsValidSolution(const eh_HashState& base_state, std::vector<unsigned char> soln)
{
    if (soln.size() != SolutionWidth) {
        // LogPrintG(BCLogLevel::LOG_DEBUG, BCLog::POW, "[ProofOfWork] Invalid solution length: %d (expected %d)\n", soln.size(), SolutionWidth);
        return false;
    }
    ...
    while (X.size() > 1) {
        ...
        for (size_t i = 0; i < X.size(); i += 2) {
            if (!HasCollision(X[i], X[i+1], CollisionByteLength)) {
                ...
                // LogPrintG(BCLogLevel::LOG_DEBUG, BCLog::POW, "[ProofOfWork] Invalid solution: invalid collision length between StepRows\n");
                // LogPrintG(BCLogLevel::LOG_DEBUG, BCLog::POW, "[ProofOfWork] X[i]   = %s\n", X[i].GetHex(hashLen));
                // LogPrintG(BCLogLevel::LOG_DEBUG, BCLog::POW, "[ProofOfWork] X[i+1] = %s\n", X[i+1].GetHex(hashLen));
                return false;
            }
            if (X[i+1].IndicesBefore(X[i], hashLen, lenIndices)) {
                // LogPrintG(BCLogLevel::LOG_DEBUG, BCLog::POW, "[ProofOfWork] Invalid solution: Index tree incorrectly ordered\n");
                return false;
            }
            if (!DistinctIndices(X[i], X[i+1], hashLen, lenIndices)) {
                // LogPrintG(BCLogLevel::LOG_DEBUG, BCLog::POW, "[ProofOfWork] Invalid solution: duplicate indices\n");
                return false;
            }
