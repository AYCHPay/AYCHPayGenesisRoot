...
UniValue importaddress(const JSONRPCRequest& request)
{
...

    {
        ...
        if (IsValidDestination(dest)) {
            ...
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid Genesis address or script");
        }
    }
    if (fRescan)
    {
        ...
        while (file.good()) {
            ...
            if (vchSecret.SetString(vstr[0])) {
                ...
                if (pwallet->HaveKey(keyid)) {
                    LogPrintG(BCLogLevel::LOG_DEBUG, BCLog::RPC, "[RPC] Skipping import of %s (key already present)\n", EncodeDestination(keyid));
                    continue;
                }
                ...
                LogPrintG(BCLogLevel::LOG_DEBUG, BCLog::RPC, "[RPC] Importing %s...\n", EncodeDestination(keyid));
                if (!pwallet->AddKeyPubKey(key, pubkey)) {
                    fGood = false;
                    continue;
                }
               ...
               if (pwallet->HaveCScript(script)) {
                   LogPrintG(BCLogLevel::LOG_DEBUG, BCLog::RPC, "[RPC] Skipping import of %s (script already present)\n", vstr[0]);
                   continue;
               }
               if(!pwallet->AddCScript(script)) {
                   LogPrintG(BCLogLevel::LOG_DEBUG, BCLog::RPC, "[RPC] Error importing script %s\n", vstr[0]);
                   fGood = false;
                   continue;
               }
...
UniValue dumpprivkey(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            ...
            "\nArguments:\n"
            "1. \"address\"   (string, required) The Genesis address for the private key\n"
            "\nResult:\n"
    ...
    return CGenesisSecret(vchSecret).ToString();
}
...
UniValue dumpwallet(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            ...
            "\nArguments:\n"
            "1. \"filename\"    (string, required) The filename with path (either absolute or relative to genesisd)\n"
            "\nResult:\n"
            "{                           (json object)\n"
    ...
    // produce output
    file << strprintf("# Wallet dump created by Genesis %s\n", CLIENT_BUILD);
    ...
    for (std::vector<std::pair<int64_t, CKeyID> >::const_iterator it = vKeyBirth.begin(); it != vKeyBirth.end(); it++) {
        ...
        if (pwallet->GetKey(keyid, key)) {
            file << strprintf("%s %s ", CGenesisSecret(key).ToString(), strTime);
                    ...
                    result.pushKV(
                        ...
                        JSONRPCError(
                            ...
                            strprintf("Rescan failed for key with creation timestamp %d. There was an error reading a "
                                      ...
                                      "caused by pruning or data corruption (see genesisd log for details) and could "
